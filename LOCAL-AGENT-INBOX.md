...
- Source/runner status: `QS3DBRCROUNDTRIPPROBE`, `scripts/test-bricscad-v25-brc-quantity-roundtrip.ps1` and `scripts/preflight-brc-quantity-roundtrip.py` are implemented. The runner is automation-only, requires an explicit disposable `*.reference-copy.dwg`, refuses a pre-existing sidecar, produces a new workbook, validates complete Excel-to-CAD selection, restores process environment variables and records before/after DWG hashes. Exact committed-SHA evidence is still required before this item can move out of `IN_PROGRESS`.
