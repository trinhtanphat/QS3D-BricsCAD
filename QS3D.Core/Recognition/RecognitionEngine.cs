...
=======
            LayerTerms = NormalizeTerms(layerTerms); TextTerms = NormalizeTerms(textTerms); EntityTypes = NormalizeTerms(entityTypes);
...
=======
        public RecognitionResult(EntitySnapshot snapshot, IReadOnlyList<RecognitionCandidate> candidates) { Snapshot = snapshot ?? throw new ArgumentNullException(nameof(snapshot)); Candidates = candidates ?? throw new ArgumentNullException(nameof(candidates)); }
>>>>>>> origin/agent/full-domain-integrate-20260810
...
=======
        public bool RequiresReview { get { var top = TopCandidate; return top == null || top.Confidence < .82d || Margin < .15d; } }
>>>>>>> origin/agent/full-domain-integrate-20260810
...
=======
        public RecognitionBatch(IEnumerable<RecognitionResult> results, double autoAcceptConfidence = .92d, double minimumMargin = .15d)
        {
            if (results == null) throw new ArgumentNullException(nameof(results)); if (autoAcceptConfidence < 0d || autoAcceptConfidence > 1d) throw new ArgumentOutOfRangeException(nameof(autoAcceptConfidence));
            Results = results.ToList(); AutoAccepted = Results.Where(x => x.TopCandidate is RecognitionCandidate c && c.Confidence >= autoAcceptConfidence && x.Margin >= minimumMargin).ToList(); ReviewRequired = Results.Except(AutoAccepted).ToList();
>>>>>>> origin/agent/full-domain-integrate-20260810
        }
...
=======
        ...
        {
            if (snapshot == null) throw new ArgumentNullException(nameof(snapshot));
            var layer = RecognitionText.Normalize(snapshot.Layer); var entityType = RecognitionText.Normalize(snapshot.EntityType); var text = RecognitionText.Normalize(string.Join(" ", snapshot.Metadata.Where(x => IsTextMetadata(x.Key)).Select(x => x.Value)));
            var candidates = new List<RecognitionCandidate>(); foreach (var rule in _rules) { var candidate = Score(rule, layer, text, entityType); if (candidate.Confidence >= .20d) candidates.Add(candidate); }
            return new RecognitionResult(snapshot, candidates.OrderByDescending(x => x.Confidence).ThenBy(x => x.RuleId, StringComparer.OrdinalIgnoreCase).ToList());
        }
        public RecognitionBatch SuggestBatch(IEnumerable<EntitySnapshot> snapshots, double autoAcceptConfidence = .92d, double minimumMargin = .15d) => new RecognitionBatch((snapshots ?? throw new ArgumentNullException(nameof(snapshots))).Select(Suggest), autoAcceptConfidence, minimumMargin);
        public static IReadOnlyList<RecognitionRule> DefaultRules() => new[]
        {
            new RecognitionRule("beam", ElementCategory.Beam, new[]{"beam","dam","kc-dam"}, new[]{"beam","dam"}, new[]{"line","polyline"}),
>>>>>>> origin/agent/full-domain-integrate-20260810
...
=======
        ...
        private static RecognitionCandidate Score(RecognitionRule rule, string layer, string text, string entityType)
        {
            var result = new RecognitionCandidate { RuleId = rule.Id, Category = rule.Category }; var layerMatch = BestTerm(rule.LayerTerms, layer); var textMatch = BestTerm(rule.TextTerms, text); var typeMatch = rule.EntityTypes.Count == 0 || rule.EntityTypes.Any(x => string.Equals(x, entityType, StringComparison.OrdinalIgnoreCase) || entityType.Contains(x));
            if (!string.IsNullOrEmpty(layerMatch)) { result.Confidence += .62d; result.Evidence.Add("layer:" + layerMatch); } if (!string.IsNullOrEmpty(textMatch)) { result.Confidence += .28d; result.Evidence.Add("text:" + textMatch); } if (typeMatch && (!string.IsNullOrEmpty(layerMatch) || !string.IsNullOrEmpty(textMatch))) { result.Confidence += .10d; result.Evidence.Add("type:" + entityType); } result.Confidence = Math.Min(1d, result.Confidence); return result;
        }
>>>>>>> origin/agent/full-domain-integrate-20260810
...
=======
            if (string.IsNullOrWhiteSpace(value)) return string.Empty; var source = (value ?? string.Empty).Trim().ToLowerInvariant().Replace('đ', 'd'); var normalized = source.Normalize(NormalizationForm.FormD); var builder = new StringBuilder(normalized.Length);
            foreach (var ch in normalized) if (CharUnicodeInfo.GetUnicodeCategory(ch) != UnicodeCategory.NonSpacingMark) builder.Append(char.IsLetterOrDigit(ch) ? ch : ' ');
>>>>>>> origin/agent/full-domain-integrate-20260810
