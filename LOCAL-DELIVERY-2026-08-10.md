...
- Reconciled remote baseline: `c2edfc5` (`fix(geometry): clear stale metadata after dependent invalidation`).
- Rebased B4D/ED2 commit: `c30c1ab` (`feat: add B4D scan and Excel handle round-trip`).
...
- All seventeen repository `preflight*.py` scripts: PASS.
