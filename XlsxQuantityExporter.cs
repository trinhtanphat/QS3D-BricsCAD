...
=======
            ...
            if (!string.IsNullOrEmpty(directory)) Directory.CreateDirectory(directory); if (File.Exists(fullPath)) File.Delete(fullPath);
            using (var stream = File.Create(fullPath)) using (var archive = new ZipArchive(stream, ZipArchiveMode.Create, false, Encoding.UTF8))
            {
                WriteEntry(archive, "[Content_Types].xml", ContentTypesXml); WriteEntry(archive, "_rels/.rels", RootRelationshipsXml); WriteEntry(archive, "xl/workbook.xml", WorkbookXml);
                WriteEntry(archive, "xl/_rels/workbook.xml.rels", WorkbookRelationshipsXml); WriteEntry(archive, "xl/styles.xml", StylesXml); WriteEntry(archive, "xl/worksheets/sheet1.xml", BuildSheet(rows));
>>>>>>> origin/agent/full-domain-20260810
        ...
        {
            var headers = new[] { "Tầng", "Loại", "Tên cấu kiện", "SL", "BT gộp (m³)", "Trừ giao (m³)", "BT còn (m³)", "Cốp pha (m²)", "Dài (m)", "Chu vi ngoài (m)", "Chu vi trong (m)", "DT cửa (m²)", "Thành bên (m²)", "DT đáy (m²)", "DT đỉnh (m²)", "DT khác (m²)", "Thép (kg)" };
            var lastRow = Math.Max(1, rows.Count + 1); var range = "A1:Q" + lastRow.ToString(CultureInfo.InvariantCulture); var sb = new StringBuilder();
            sb.Append("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><worksheet xmlns=\"http://schemas.openxmlformats.org/spreadsheetml/2006/main\"><dimension ref=\"").Append(range).Append("\"/>");
            sb.Append("<sheetViews><sheetView workbookViewId=\"0\"><pane ySplit=\"1\" topLeftCell=\"A2\" activePane=\"bottomLeft\" state=\"frozen\"/></sheetView></sheetViews><sheetData><row r=\"1\">");
            for (var c = 0; c < headers.Length; c++) AppendInlineStringCell(sb, CellRef(c, 1), headers[c], 1); sb.Append("</row>");
            for (var i = 0; i < rows.Count; i++)
            {
                var row = rows[i]; var r = i + 2; sb.Append("<row r=\"").Append(r).Append("\">");
                AppendInlineStringCell(sb, CellRef(0, r), row.Floor, 0); AppendInlineStringCell(sb, CellRef(1, r), row.Category, 0); AppendInlineStringCell(sb, CellRef(2, r), row.FamilyName, 0);
                AppendNumberCell(sb, CellRef(3, r), row.Count); AppendNumberCell(sb, CellRef(4, r), row.GrossConcreteM3); AppendNumberCell(sb, CellRef(5, r), row.DeductionM3); AppendNumberCell(sb, CellRef(6, r), row.NetConcreteM3);
                AppendNumberCell(sb, CellRef(7, r), row.FormworkM2); AppendNumberCell(sb, CellRef(8, r), row.LengthM); AppendNumberCell(sb, CellRef(9, r), row.OuterPerimeterM); AppendNumberCell(sb, CellRef(10, r), row.InnerPerimeterM);
                AppendNumberCell(sb, CellRef(11, r), row.DoorAreaM2); AppendNumberCell(sb, CellRef(12, r), row.SideAreaM2); AppendNumberCell(sb, CellRef(13, r), row.BottomAreaM2); AppendNumberCell(sb, CellRef(14, r), row.TopAreaM2);
                AppendNumberCell(sb, CellRef(15, r), row.OtherAreaM2); AppendNumberCell(sb, CellRef(16, r), row.SteelWeightKg); sb.Append("</row>");
            }
<<<<<<< origin/main
