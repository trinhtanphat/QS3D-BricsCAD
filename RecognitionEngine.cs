...
=======
            Snapshot = snapshot ?? throw new ArgumentNullException(nameof(snapshot)); Candidates = candidates ?? throw new ArgumentNullException(nameof(candidates));
...
=======
        public static IReadOnlyList<RecognitionRule> DefaultRules()
        {
            return new[]
            {
                new RecognitionRule("beam", ElementCategory.Beam, new[]{"beam","dam","d-beam","kc-dam"}, new[]{"beam","dam"}, new[]{"line","polyline"}),
                ...
                new RecognitionRule("foundation", ElementCategory.Foundation, new[]{"foundation","mong","footing"}, new[]{"foundation","mong","footing"}, new[]{"polyline","region"}),
                new RecognitionRule("earthwork", ElementCategory.Earthwork, new[]{"earth","excav","dao-dat"}, new[]{"earthwork","excavation","dao dat"}, new[]{"polyline","hatch","region"}),
                new RecognitionRule("rebar", ElementCategory.Rebar, new[]{"rebar","thep","steel-bar"}, new[]{"rebar","cot thep","d10","d12","d16","d20","phi"}, new[]{"line","polyline","mtext","dbtext"})
            };
        }
>>>>>>> origin/agent/full-domain-20260810
...
=======
            var source = (value ?? string.Empty).Trim().ToLowerInvariant().Replace('đ', 'd');
            var normalized = source.Normalize(NormalizationForm.FormD);
            var builder = new StringBuilder(normalized.Length);
            foreach (var ch in normalized)
            {
