...
var noProjectIdentity = new ClaimsIdentity(new[] {
    new Claim(ClaimTypes.NameIdentifier, "erp"),
    new Claim("scope", "quantity:read"),
    new Claim("tenant_id", "tenant-a") }, "smoke");
var noProjectPrincipal = new ClaimsPrincipal(noProjectIdentity);
Require(await Status(ApiAuthorization.AuthorizeAccess(noProjectPrincipal, new ResourceAccess("project-a", "tenant-a"))) == 403,
    "project-bound resource must reject caller without project_id claim");
