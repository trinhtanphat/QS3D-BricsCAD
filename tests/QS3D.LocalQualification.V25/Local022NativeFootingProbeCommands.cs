...
{
    /// <summary>
    /// Automation-only LOCAL-022 native evidence probe.  It loads the published product unchanged
    ...
    public sealed class Local022NativeFootingProbeCommands
    {
        ...
        private const string ProductVariable = "QS3D_LOCAL022_PRODUCT_DLL";
        private const string ProbeVariable = "QS3D_LOCAL022_PROBE_DLL";
        private const string PhaseVariable = "QS3D_LOCAL022_PHASE";
        ...
        private static Context BindContext(string expectedPhase)
        {
            ...
            if (!string.Equals(product.GetName().Name, "QS3D.BricsCAD.V25", StringComparison.Ordinal)) throw new ProbeException("product_identity_mismatch");
            ...
            int nativeBaseline)
        {
            ...
            {
                "schema=QS3D_LOCAL022_CONTINUITY_V1",
        ...
        private static ContinuityState RequireContinuity(Context context, ProjectState project)
        {
            ...
            if (values.Count != 5 || !values.TryGetValue("schema", out var schema) ||
                !string.Equals(schema, "QS3D_LOCAL022_CONTINUITY_V1", StringComparison.Ordinal) ||
        ...
        private static void RequireHostMajor25() { if (Process.GetCurrentProcess().MainModule == null || Process.GetCurrentProcess().MainModule.FileVersionInfo.FileMajorPart != 25) throw new ProbeException("host_major_not_25"); }
        private static Assembly ProductAssembly() => AppDomain.CurrentDomain.GetAssemblies().SingleOrDefault(x => string.Equals(x.GetName().Name, "QS3D.BricsCAD.V25", StringComparison.OrdinalIgnoreCase)) ?? throw new ProbeException("product_not_loaded");
