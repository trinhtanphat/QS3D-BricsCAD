    ...
    internal static class AuditTrailIntegritySmoke
    {
        ...
        private static void NonUtcHistoryFailsClosed()
        {
            foreach (var kind in new[] { DateTimeKind.Local, DateTimeKind.Unspecified })
            {
                ...
                var beforeVersion = project.ChangeVersion;
                var trail = AuditTrail.ForProject(project);
                Assert(ExpectInvalidOperation(() => { _ = trail.Events; }).Message.Contains("non-UTC", StringComparison.Ordinal), "Audit read must reject non-UTC stored timestamps.");
                Assert(ExpectInvalidOperation(() => trail.Record("Updated", "E-2", "valid")).Message.Contains("non-UTC", StringComparison.Ordinal), "Audit record must reject non-UTC existing timestamps.");
                Assert(ExpectInvalidOperation(trail.Clear).Message.Contains("non-UTC", StringComparison.Ordinal), "Audit clear must reject non-UTC stored timestamps.");
        ...
        private static void AssertXmlInvalidStoredField(Action<AuditEvent> mutate, string expectedMessage)
        {
            ...
            var beforeVersion = project.ChangeVersion;
            var trail = AuditTrail.ForProject(project);
            Assert(ExpectInvalidOperation(() => { _ = trail.Events; }).Message.Contains(expectedMessage, StringComparison.Ordinal), "Audit read should identify XML-invalid stored field.");
            Assert(ExpectInvalidOperation(() => trail.Record("Updated", "E-2", "valid")).Message.Contains(expectedMessage, StringComparison.Ordinal), "Audit record should identify XML-invalid existing field.");
            Assert(ExpectInvalidOperation(trail.Clear).Message.Contains(expectedMessage, StringComparison.Ordinal), "Audit clear should identify XML-invalid existing field.");
