...
namespace QS3D.Core.SmokeTests
{
    internal static class BomReleaseGuardSmoke
    {
        ...
        private static void ControlFamiliesAndXmlInvalidKeysFailClosed()
        {
            ...
            if (issues.Any(x => x.Code == "BOM_PROPERTY_KEY_INVALID" && (x.Message ?? string.Empty).IndexOf("VậtLiệu😀", StringComparison.Ordinal) >= 0) ||
                issues.Any(x => x.Code == "BOM_QUANTITY_KEY_INVALID" && (x.Message ?? string.Empty).IndexOf("KhốiLượng😀", StringComparison.Ordinal) >= 0))
                throw new Exception("Valid Unicode/supplementary-Unicode keys must remain accepted.");
        }
