...
{
    internal static class RegenerationPreviewTargetCountCurrentIntegritySmoke
    {
        ...
        private static void KnownCountOverrunRejectsBeforeUnexpectedCurrent()
        {
            ...
            var error = Throws<InvalidOperationException>(() => Service().PreviewSubset(ProjectWithElements("E1", "E2"), source));
            Contains(error.Message, "known count");
        ...
        private static void MoveNextTransientGrowthRejectsBeforeCurrent()
        {
            ...
            var error = Throws<InvalidOperationException>(() => Service().PreviewSubset(ProjectWithElements("E1"), source));
            Contains(error.Message, "known count");
        ...
        private static void ExactKnownCountRemainsAccepted()
        {
            ...
            var preview = Service().PreviewSubset(ProjectWithElements("E1", "E2"), source);
            Equal(2, preview.TargetElementIds.Count);
            Equal("E1", preview.TargetElementIds[0]);
            Equal("E2", preview.TargetElementIds[1]);
        ...
        {
            var preview = Service().PreviewSubset(ProjectWithElements("E1", "E2"), Streaming("E2", "E1"));
            Equal(2, preview.TargetElementIds.Count);
            Equal("E1", preview.TargetElementIds[0]);
            Equal("E2", preview.TargetElementIds[1]);
        }
        ...
        {
            var project = new ProjectState("regen-preview-count-integrity", "Regeneration Preview Count Integrity");
        ...
        private static RegenerationPreviewService Service() => new RegenerationPreviewService();
    ...
    internal static class RegenerationPreviewTargetCountCurrentIntegritySmokeRegistration
    {
        [ModuleInitializer]
        internal static void Initialize() => RegenerationPreviewTargetCountCurrentIntegritySmoke.Run();
    }
}
