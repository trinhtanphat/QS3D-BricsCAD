...
{
    internal static class MeasurementSnapshotNegativeKnownCountSmoke
    {
        ...
        {
            var source = new NegativeGenericCount<MeasurementTrace>();
            var error = Capture<ArgumentException>(() => Snapshot(source));
            ...
            Contains("negative", error.Message, "Negative ICollection<T>.Count must fail closed explicitly.");
        }

        private static void ReadOnlyCountFailsBeforeEnumeration()
        {
            var source = new NegativeReadOnlyCount<MeasurementTrace>();
            var error = Capture<ArgumentException>(() => Snapshot(source));
            ...
            Contains("negative", error.Message, "Negative IReadOnlyCollection<T>.Count must fail closed explicitly.");
        }

        private static void NonGenericCountFailsBeforeEnumeration()
        {
            var source = new NegativeNonGenericCount<MeasurementTrace>();
            var error = Capture<ArgumentException>(() => Snapshot(source));
            ...
            Contains("negative", error.Message, "Negative ICollection.Count must fail closed explicitly.");
        }

        private static MeasurementSnapshot Snapshot(IEnumerable<MeasurementTrace> traces)
        {
            return new MeasurementSnapshot(traces);
        }
    ...
    internal static class MeasurementSnapshotNegativeKnownCountRegistration
    {
        [ModuleInitializer]
        internal static void Initialize()
        {
            MeasurementSnapshotNegativeKnownCountSmoke.Run();
        }
    }
}
