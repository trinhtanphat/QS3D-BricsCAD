...
namespace QS3D.Core.SmokeTests
{
    internal static class ProjectZoneSemanticCanonicalitySmoke
    {
        ...
        internal static void Initialize()
        {
            ...
            PaddedStoredActiveZoneRejectsBeforeCreateOrDeleteMutation();
        }
        ...
        private static void PaddedStoredActiveZoneRejectsBeforeCreateOrDeleteMutation()
        {
            var project = new ProjectState("P-ZONE-STORED-ACTIVE", "Zone stored active canonicality");
            var zoneA = ProjectZoneService.Create(project, "zone-a", "Zone A");
            var zoneB = ProjectZoneService.Create(project, "zone-b", "Zone B");
            ...
            Throws<InvalidOperationException>(() => ProjectZoneService.Create(project, "zone-c", "Zone C"));
            Throws<InvalidOperationException>(() => ProjectZoneService.Delete(project, zoneB.Id));

            Equal(beforeVersion, project.ChangeVersion, "stored active rejection version");
            Equal(beforeCount, project.Zones.Count, "stored active rejection zone count");
            Equal(" zone-a ", project.ActiveZoneId, "stored active rejection state");
            Equal(zoneA, project.FindZone("zone-a"), "stored active original zone remains");
            Equal(zoneB, project.FindZone("zone-b"), "stored active peer zone remains");
        }
