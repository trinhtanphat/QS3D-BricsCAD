...
namespace QS3D.Core.SmokeTests
{
    internal static class QuantityReportGroupKeyCollisionSmoke
    {
        ...
        internal static void Initialize()
        {
            ...
            var collidingBeforeFix = new ElementInstance("E2", columnFamily, "F" + separator + "Beam") { LengthM = 2d };
            collidingBeforeFix.SourceHandles.Add("H2");

            var rows = QuantityReportBuilder.Group(new[] { first, sameGroup, collidingBeforeFix });
            if (rows.Count != 2)
                throw new Exception("Distinct accepted quantity-report grouping tuples must not alias through delimiter injection.");
            ...
            if (beamRow.Count != 2 || Math.Abs(beamRow.LengthM - 4d) > 1e-12d ||
                ...
                throw new Exception("Identical legacy quantity-report tuples must still group with independent aggregate/provenance state.");
