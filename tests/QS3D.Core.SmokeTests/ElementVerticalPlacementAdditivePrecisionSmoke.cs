...
namespace QS3D.Core.SmokeTests
{
    internal static class ElementVerticalPlacementAdditivePrecisionSmoke
    {
        internal static void Run()
        {
            ...
            BottomLevelOffsetRejectsSwallowedConfiguredTerm();
        ...
        private static void LegacyBottomOffsetRejectsSwallowedPositiveAndNegativeTerms()
        {
            ...
            var positive = NewElement(project, "legacy-positive");
            var version = project.ChangeVersion;

            Contains(
                ThrowsMessage<InvalidOperationException>(() =>
                    ElementVerticalPlacementService.Resolve(project, positive, 1e16d, 4d, 1d)),
                "legacy bottom elevation cannot preserve its non-zero additive term");
            Equal(version, project.ChangeVersion);

            var negative = NewElement(project, "legacy-negative");
            Contains(
                ThrowsMessage<InvalidOperationException>(() =>
                    ElementVerticalPlacementService.Resolve(project, negative, 1e16d, 4d, -1d)),
                "legacy bottom elevation cannot preserve its non-zero additive term");
            Equal(version, project.ChangeVersion);
        }

        private static void BottomLevelOffsetRejectsSwallowedConfiguredTerm()
        {
            var project = NewProject();
            project.Floors.Add(new FloorDefinition("HUGE", "Huge", 1e16d));
            var element = NewElement(project, "bottom-level-offset");
        ...
        private static void TopLevelOffsetRejectsSwallowedPositiveAndNegativeTerms()
        {
            foreach (var offset in new[] { "1", "-1" })
            {
                var project = NewProject();
                project.Floors.Add(new FloorDefinition("HUGE", "Huge", 1e16d));
                var element = NewElement(project, "top-level-offset-" + offset.Replace("-", "negative"));
