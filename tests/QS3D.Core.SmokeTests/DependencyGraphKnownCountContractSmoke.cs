...
namespace QS3D.Core.SmokeTests
{
    internal static class DependencyGraphKnownCountContractSmoke
    {
        ...
        private static void ConflictingKnownCountsFailBeforeEnumerationAndPreserveGraph()
        {
            ...
            ExpectInvalidOperation(() => graph.Rebuild(rebuildSource), "Rebuild must reject conflicting known counts before enumeration.");
            ...
            ExpectInvalidOperation(() => graph.TopologicalDirtyOrder(orderingSource), "Topological ordering must reject conflicting known counts before enumeration.");
        ...
        private static void NegativeKnownCountFailsBeforeEnumeration()
        {
            ...
            ExpectInvalidOperation(() => graph.Rebuild(source), "Negative known dependency count must fail closed before enumeration.");
        ...
        private static void NonGenericOversizedCountFailsBeforeEnumeration()
        {
            ...
            ExpectInvalidOperation(() => graph.Rebuild(rebuildSource), "Non-generic oversized rebuild count must fail before enumeration.");
            if (rebuildSource.EnumerationRequested)
            ...
            ExpectInvalidOperation(() => graph.TopologicalDirtyOrder(orderingSource), "Non-generic oversized ordering count must fail before enumeration.");
            if (orderingSource.EnumerationRequested)
        ...
        private static void ExactBoundRemainsAccepted()
        {
            ...
            graph.TopologicalDirtyOrder(elements);
        ...
        private static void DishonestKnownCountStillStopsAtStreamingBoundary()
        {
            ...
            ExpectInvalidOperation(() => new DependencyGraph().Rebuild(rebuildSource), "Dishonest rebuild count must still be bounded while streaming.");
            ...
            ExpectInvalidOperation(() => new DependencyGraph().TopologicalDirtyOrder(orderingSource), "Dishonest ordering count must still be bounded while streaming.");
