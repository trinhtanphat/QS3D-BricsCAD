...
namespace QS3D.Core.SmokeTests
{
    internal static class AutoRoomStaleSelectionBoundSmoke
    {
        ...
        internal static void Initialize()
        {
            ...
            SelectionNormalizationRemainsCompatible();
        }

        private static void KnownOversizeFailsBeforeEnumerationOrMutation()
        {
            var project = NewProject();
            var handles = new ProbeSet(MaximumSourceHandles + 1, MaximumSourceHandles + 1, true);
            ...
            Contains("cannot exceed 5000", error.Message, "Known oversize input must report the existing bound.");
            Equal(0, handles.ObservedEntries, "Known oversize input must fail from Count before enumeration.");
            Equal(beforeVersion, project.ChangeVersion, "Known oversize input must fail before project mutation.");
            Equal(beforeUpdatedUtc, project.UpdatedUtc, "Known oversize input must preserve project timestamps.");
        }

        private static void DishonestCountStopsAtFirstDisallowedEntry()
        {
            var project = NewProject();
            var handles = new ProbeSet(1, MaximumSourceHandles + 2, false);
            var beforeVersion = project.ChangeVersion;
            var beforeUpdatedUtc = project.UpdatedUtc;
            ...
            var error = Capture<InvalidOperationException>(() => AutoRoomLifecycle.MarkStaleForSelection(
                ...
                "f",
                "z",
                new DateTime(2026, 8, 17, 0, 1, 0, DateTimeKind.Utc)));

            Contains("cannot exceed 5000", error.Message, "Dishonest Count must not evade the streaming bound.");
            Equal(MaximumSourceHandles + 1, handles.ObservedEntries, "Streaming enforcement must stop on entry 5001 and not consume entry 5002.");
        ...
        {
            var project = NewProject();
            var handles = new ProbeSet(MaximumSourceHandles, MaximumSourceHandles, false);
            var beforeVersion = project.ChangeVersion;
            ...
            var stale = AutoRoomLifecycle.MarkStaleForSelection(
                ...
                "f",
                "z",
                new DateTime(2026, 8, 17, 0, 2, 0, DateTimeKind.Utc));

            Equal(0, stale.Count, "Exactly 5000 source handles must remain accepted.");
            Equal(MaximumSourceHandles, handles.ObservedEntries, "Exactly 5000 entries must be fully consumed.");
            Equal(beforeVersion, project.ChangeVersion, "Boundary-sized input with no matching rooms must not mutate the project.");
        }

        private static void SelectionNormalizationRemainsCompatible()
        {
            var project = NewProject();
            ...
            room.Properties[AutoRoomLifecycle.BoundarySourceHandlesKey] = "A;B";
            ...
            var stale = AutoRoomLifecycle.MarkStaleForSelection(
                project,
                new HashSet<string>(StringComparer.OrdinalIgnoreCase),
                new HashSet<string>(new[] { " a ", "B", " b ", "   " }, StringComparer.Ordinal),
                "f",
                "z",
                new DateTime(2026, 8, 17, 0, 3, 0, DateTimeKind.Utc));

            Equal(1, stale.Count, "Trim/case-insensitive deduplication semantics must remain compatible.");
            Equal(room, stale[0], "Expected the matching Auto Room to be marked stale.");
            Equal(beforeVersion + 1, project.ChangeVersion, "A real stale transition must still touch the project exactly once.");
            Equal(AutoRoomLifecycle.BoundaryStateStale, room.Properties[AutoRoomLifecycle.BoundaryStateKey], "Matching room must become stale.");
        }
        ...
        private sealed class ProbeSet : ISet<string>
        {
            ...
            {
                if (_failIfEnumerated) throw new Exception("Oversize set must not be enumerated.");
