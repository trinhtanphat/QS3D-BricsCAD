...
{
    internal static class XlsxHandleErrorCellSmoke
    {
        [ModuleInitializer]
        internal static void Initialize()
        {
            RejectsErrorTypedHandleValue();
            ErrorTypedLegacyTokenDoesNotActivateFallback();
            PreservesDefaultNumericHandleCell();
        }

        private static void RejectsErrorTypedHandleValue()
        {
            var path = CreateWorkbook(
                "<row r=\"1\"><c r=\"A1\" t=\"inlineStr\"><is><t>CAD Handle (hex)</t></is></c></row>" +
                "<row r=\"2\"><c r=\"A2\" t=\"e\"><v>1A</v></c></row>");
            try
            {
        ...
        private static void ErrorTypedLegacyTokenDoesNotActivateFallback()
        {
            var path = CreateWorkbook(
                "<row r=\"1\"><c r=\"A1\" t=\"inlineStr\"><is><t>Legacy Data</t></is></c></row>" +
                "<row r=\"2\"><c r=\"A2\" t=\"e\"><v>$123</v></c></row>");
            try
            {
                var result = XlsxHandleReader.ReadHandleLookup(path, 2);
                if (result.Handles.Count != 0)
                    throw new Exception("An XLSX error cell must not be synthesized into a legacy decimal Handle.");
                if (result.UsesLegacyDecimalHandles)
                    throw new Exception("An XLSX error cell containing a legacy-looking token must not activate legacy decimal mode.");
            }
            finally { Delete(path); }
        }
        ...
        {
            var path = Path.Combine(Path.GetTempPath(), "qs3d-xlsx-handle-error-cell-" + Guid.NewGuid().ToString("N") + ".xlsx");
