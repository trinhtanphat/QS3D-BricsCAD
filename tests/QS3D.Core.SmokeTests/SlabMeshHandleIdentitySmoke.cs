...
{
    internal static class SlabMeshHandleIdentitySmoke
    {
        [ModuleInitializer]
        ...
        private static void NumericAliasDuplicateCountsOnce()
        {
            ...
            Require(issues, setup.Element.Id, "DUPLICATE_SLAB_MESH_GENERATED_HANDLE", HealthSeverity.Error);
            Require(issues, setup.Element.Id, "SLAB_MESH_GENERATED_COUNT_MISMATCH", HealthSeverity.Warning);
        }
        ...
        private static void SourceAliasFailsVisible()
        {
            ...
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "SLAB_MESH_GENERATED_HANDLE_IN_SOURCE", HealthSeverity.Error);
        }
        ...
        private static void LiveAliasIsRecognized()
        {
            ...
            Forbid(issues, setup.Element.Id, "SLAB_MESH_GENERATED_SOLID_MISSING");
        }
        ...
        private static void CrossOwnerAliasConflicts()
        {
            ...
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "SLAB_MESH_GENERATED_OWNERSHIP_CONFLICT", HealthSeverity.Error);
        }
        ...
        private static void DistinctHandlesStayDistinct()
        {
            ...
            var issues = Inspect(setup);
            Forbid(issues, setup.Element.Id, "DUPLICATE_SLAB_MESH_GENERATED_HANDLE");
            Forbid(issues, setup.Element.Id, "SLAB_MESH_GENERATED_COUNT_MISMATCH");
        }
        ...
        private static void PrefixedHexRemainsInvalid()
        {
            ...
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "INVALID_SLAB_MESH_GENERATED_HANDLE", HealthSeverity.Error);
            Forbid(issues, setup.Element.Id, "SLAB_MESH_GENERATED_COUNT_MISMATCH");
        }

        private static Setup Create(string suffix, string handles, string count)
        {
            var project = new ProjectState("P-SLAB-MESH-HANDLE-" + suffix, "Slab Mesh handle identity");
            var element = new ProjectElement("E-SLAB-MESH-HANDLE-" + suffix, ElementCategory.Slab);
        ...
        private static IReadOnlyList<ModelHealthIssue> Inspect(Setup setup, ISet<string>? live = null) =>
            new GeneratedSlabMeshHealthService().Inspect(setup.Project, live);
        ...
        private static void Require(IReadOnlyList<ModelHealthIssue> issues, string elementId, string code, HealthSeverity severity)
        {
            if (issues.Any(x =>
                ...
                return;
            throw new InvalidOperationException("SlabMeshHandleIdentitySmoke expected issue was not reported: " + code + ".");
        }
        ...
        private static void Forbid(IReadOnlyList<ModelHealthIssue> issues, string elementId, string code)
        {
            if (!issues.Any(x => string.Equals(x.Code, code, StringComparison.Ordinal) && string.Equals(x.ElementId, elementId, StringComparison.Ordinal)))
                return;
            throw new InvalidOperationException("SlabMeshHandleIdentitySmoke reported unexpected issue: " + code + ".");
        }
