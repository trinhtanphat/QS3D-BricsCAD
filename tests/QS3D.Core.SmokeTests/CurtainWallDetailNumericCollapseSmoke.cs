...
internal static class CurtainWallDetailNumericCollapseSmoke
{
    ...
    private static void MixedZeroTransomPreservesMullionSolids()
    {
        var detail = CurtainWallDetailPlanner.Plan(Input(4d, 3d, 2d, 1.5d, .1d, .05d, 0d));
        Equal(4, detail.Panels.Count);
        Equal(3, detail.VerticalFrames.Count);
        Equal(2, detail.HorizontalFrames.Count);
        Equal(9, detail.DetailSolidCount);
        Equal(detail.Layout.ClearGlassAreaM2, SumPanelArea(detail), 1e-12d, "mixed zero-transom panel area");
        AllPositive(detail.VerticalFrames, "mixed zero-transom vertical solids");
        AllPositive(detail.HorizontalFrames, "mixed zero-transom horizontal solids");
    }

    private static void ZeroPerimeterPreservesInternalSolids()
    {
        var detail = CurtainWallDetailPlanner.Plan(Input(4d, 3d, 2d, 1.5d, 0d, .05d, .05d));
        Equal(4, detail.Panels.Count);
        Equal(1, detail.VerticalFrames.Count);
        Equal(1, detail.HorizontalFrames.Count);
        Equal(6, detail.DetailSolidCount);
        Equal(detail.Layout.ClearGlassAreaM2, SumPanelArea(detail), 1e-12d, "zero-perimeter panel area");
        AllPositive(detail.VerticalFrames, "zero-perimeter vertical solids");
        AllPositive(detail.HorizontalFrames, "zero-perimeter horizontal solids");
    }
