...
namespace QS3D.Core.SmokeTests
{
    internal static class ProjectInterchangeImportPreviewSmoke
    {
        ...
        private static void NewIdentitiesArePreviewedWithoutMutation()
        {
            ...
            Equal(0, target.Zones.Count); Equal(0, target.Floors.Count); Equal(0, target.Families.Count); Equal(0, target.Elements.Count);
        ...
        private static void ExistingSameCategoryRequiresPolicy()
        {
            ...
            Equal(4, preview.PolicyCollisionCount); Equal(0, preview.IncompatibleCollisionCount); True(preview.RequiresIdentityPolicy);
        ...
        private static void InvalidSnapshotStopsBeforeCollisionPlanning()
        {
            ...
            True(!preview.Validation.IsValid); Equal(0, preview.TotalIdentityCount); Equal(0, preview.Items.Count); Equal(0, preview.CollisionCount);
        }
        ...
        private static void AmbiguousTargetIdsFailClosed()
        {
            ...
            target.Zones.Add(new ZoneDefinition("DUP", "First")); target.Zones.Add(new ZoneDefinition("dup", "Second"));
        ...
        {
            var project = new ProjectState(projectId, "Project " + projectId) { DrawingFingerprint = fingerprint, UpdatedUtc = new DateTime(2026, 8, 10, 0, 0, 0, DateTimeKind.Utc) };
            project.Zones.Add(new ZoneDefinition(zoneId, "Zone")); project.Floors.Add(new FloorDefinition(floorId, "Floor", 0)); project.Families.Add(new ProjectFamily(familyId, "Family", category));
        ...
        private static void True(bool value) { if (!value) throw new Exception("Expected condition to be true."); }
