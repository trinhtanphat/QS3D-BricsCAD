...
{
    internal static class WallMeshHandleIdentitySmoke
    {
        [ModuleInitializer]
        ...
        private static void NumericAliasDuplicateCountsOnce()
        {
            ...
            Require(issues, setup.Element.Id, "DUPLICATE_WALL_MESH_GENERATED_HANDLE", HealthSeverity.Error);
            Require(issues, setup.Element.Id, "WALL_MESH_GENERATED_COUNT_MISMATCH", HealthSeverity.Warning);
        }
        ...
        private static void SourceAliasFailsVisible()
        {
            ...
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "WALL_MESH_GENERATED_HANDLE_IN_SOURCE", HealthSeverity.Error);
        }
        ...
        private static void LiveAliasIsRecognized()
        {
            ...
            Forbid(issues, setup.Element.Id, "WALL_MESH_GENERATED_SOLID_MISSING");
        }

        private static void CrossOwnerAliasConflicts()
        {
            var setup = Create("OWNER", "0A", "1");
            var other = new ProjectElement("E-WALL-MESH-HANDLE-OWNER-OTHER", ElementCategory.Slab);
            other.SourceHandles.Add("A");
            setup.Project.Elements.Add(other);
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "WALL_MESH_GENERATED_OWNERSHIP_CONFLICT", HealthSeverity.Error);
        }
        ...
        private static void DistinctHandlesStayDistinct()
        {
            ...
            var issues = Inspect(setup);
            Forbid(issues, setup.Element.Id, "DUPLICATE_WALL_MESH_GENERATED_HANDLE");
            Forbid(issues, setup.Element.Id, "WALL_MESH_GENERATED_COUNT_MISMATCH");
        }
        ...
        private static void PrefixedHexRemainsInvalid()
        {
            ...
            var issues = Inspect(setup);
            Require(issues, setup.Element.Id, "INVALID_WALL_MESH_GENERATED_HANDLE", HealthSeverity.Error);
            Forbid(issues, setup.Element.Id, "WALL_MESH_GENERATED_COUNT_MISMATCH");
        }

        private static Setup Create(string suffix, string handles, string count)
        {
            var project = new ProjectState("P-WALL-MESH-HANDLE-" + suffix, "Wall Mesh handle identity");
            var element = new ProjectElement("E-WALL-MESH-HANDLE-" + suffix, ElementCategory.StructuralWall);
        ...
        private static IReadOnlyList<ModelHealthIssue> Inspect(Setup setup, ISet<string>? live = null) =>
            new GeneratedWallMeshHealthService().Inspect(setup.Project, live);
        ...
        private static void Require(IReadOnlyList<ModelHealthIssue> issues, string elementId, string code, HealthSeverity severity)
        {
            if (issues.Any(x =>
                ...
                return;
            throw new InvalidOperationException("WallMeshHandleIdentitySmoke expected issue was not reported: " + code + ".");
        }
        ...
        private static void Forbid(IReadOnlyList<ModelHealthIssue> issues, string elementId, string code)
        {
            if (!issues.Any(x => string.Equals(x.Code, code, StringComparison.Ordinal) && string.Equals(x.ElementId, elementId, StringComparison.Ordinal)))
                return;
            throw new InvalidOperationException("WallMeshHandleIdentitySmoke reported unexpected issue: " + code + ".");
        }
