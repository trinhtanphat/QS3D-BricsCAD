    ...
    internal static class PaddedPersistedReferenceRegressionSmoke
    {
            ...
            string caseName)
        {
            ...
            AssertInvalidData(
                path,
                CanonicalityMessage,
                "Malformed persisted reference was silently accepted: " + caseName + "/" + attributeName + ".");
        }
        ...
        private static void AssertUnknownCanonicalReferencesStillFail(string directory)
        {
            foreach (var reference in References)
            {
                ...
                AssertInvalidData(
                    ...
                    "Canonical-but-unknown reference was silently accepted: " + reference.Attribute + ".");
            }
        }
            ...
            catch (InvalidDataException ex)
            {
                    ...
                    ex.Message.IndexOf(expectedMessageFragment, StringComparison.OrdinalIgnoreCase) < 0)
                {
                    throw new InvalidOperationException(
                        "QSDB persisted-reference rejection returned a non-deterministic diagnostic. Expected fragment '" +
