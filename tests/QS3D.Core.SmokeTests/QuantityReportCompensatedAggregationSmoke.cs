...
namespace QS3D.Core.SmokeTests
{
    internal static class QuantityReportCompensatedAggregationSmoke
    {
        ...
        private static void PreservesRepresentableSmallContributions()
        {
            ...
            Equal(10000000000000002d, rows[0].GrossConcreteM3, "Grouped gross concrete must retain the two representable small contributions.");
            Equal(10000000000000002d, rows[0].NetConcreteM3, "Grouped net concrete must retain the two representable small contributions.");
            Equal(10000000000000002d, rows[0].LengthM, "Grouped length must retain the two representable small contributions.");
        }
        ...
        private static void PreservesOrdinaryAggregation()
        {
            ...
            Equal(1, rows.Count, "Ordinary elements must remain in one report group.");
