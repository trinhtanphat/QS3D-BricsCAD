...
namespace QS3D.Core.SmokeTests
{
    internal static class ProjectPersistenceCheckpointCountContractSmoke
    {
        ...
        private static void HiddenOversizedSecondaryCountFailsBeforeEnumeration()
        {
            ...
            AssertAllCountsReadOnce(source, "Hidden-oversize validation");
        ...
        private static void ConflictingInBoundCountsFailBeforeEnumeration()
        {
            ...
            var error = Capture<InvalidOperationException>(() => ProjectPersistenceCheckpoint.Capture(project, source));
            AssertAllCountsReadOnce(source, "Conflicting-count validation");
        ...
        private static void ConsistentMultiContractCountRemainsAccepted()
        {
            ...
            AssertAllCountsReadOnce(source, "Consistent-count validation");
        ...
        private static void AllKnownCountContractsAreObservedBeforeEnumeration()
        {
            var project = BuildSingleElementProject("P-READ-ALL");
            var source = MultiCountCollection.NeverEnumerate(1, 1, MaximumElementCount + 1);
            Capture<InvalidOperationException>(() => ProjectPersistenceCheckpoint.Capture(project, source));
            AssertAllCountsReadOnce(source, "All-known-count validation");
            Equal(0, source.GetEnumeratorCalls, "Known-count validation must complete before enumeration begins.");
        }

        private static void AssertAllCountsReadOnce(MultiCountCollection source, string label)
        {
            Equal(1, source.GenericCountReads, label + " must inspect ICollection<string>.Count exactly once.");
            Equal(1, source.ReadOnlyCountReads, label + " must inspect IReadOnlyCollection<string>.Count exactly once.");
            Equal(1, source.NonGenericCountReads, label + " must inspect ICollection.Count exactly once.");
        }
        ...
        private sealed class MultiCountCollection : ICollection<string>, IReadOnlyCollection<string>, ICollection
        {
            ...
            int ICollection<string>.Count { get { GenericCountReads++; return _genericCount; } }
            int IReadOnlyCollection<string>.Count { get { ReadOnlyCountReads++; return _readOnlyCount; } }
