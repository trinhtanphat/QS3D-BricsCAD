...
Do not commit `BrxMgd.dll`, `TD_Mgd.dll`, BLT/BLT3D folders, private DWG/DOCX fixtures, signing certificates or private runtime assets. The BricsCAD plugin resolves V25 assemblies through `BRICSCAD_V25_DIR` with `Private=false`. Use `samples/generated` for public tests; the whitelist in `.gitignore` permits only the named repository-owned synthetic DXF/DWG/XLSX binaries.
