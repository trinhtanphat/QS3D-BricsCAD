...
- [ĐÃ XÁC NHẬN] `LoadExistingProjectOrThrow(...)` normalizes parse/load failures to `InvalidDataException`, so the lifecycle memo catches the corrupt-sidecar failure boundary rather than depending on individual serializer exception types.
- [ĐÃ XÁC NHẬN] `ProjectSidecarRevisionStamp` is a content-based SHA-256 authority stamp over the primary `.qsdb` and `.bak` pair, with bounded reads and pair-stability checks; memo invalidation therefore tracks the exact sidecar generation rather than only timestamps.
