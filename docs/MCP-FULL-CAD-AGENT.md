...
Status: SOURCE MERGED / `PENDING_LOCAL`  
Parent architecture issue: #4352  
OAuth integration issue: #4584  
Merged OAuth PR: #4597  
Merged main source: `d3e6e4d6e9f423efbd2d236d600110a6a2ced1f5`

QS3D embeds the MCP resource server, OAuth authorization server, Cloudflare onboarding, Agent Center, CAD runtime and audit surface directly in the BricsCAD plugin. End users do not need a second MCP repository, Node runtime, PowerShell setup, or command-shell workflow.

The active transport is `McpEmbeddedServerV2.cs`; the legacy monolithic `McpEmbeddedServer.cs` is historical source and is excluded from active V25/V26 compilation. `McpCadAgentRuntime.cs` owns CAD database/editor operations, bounded native command dispatch, BricsCAD-process-only UI input, emergency recovery and audit.
...
UI fallback uses `cad_ui_click`, `cad_ui_type`, and `cad_ui_key` only against windows owned by the current BricsCAD process. These are not desktop-wide remote-control tools.

Recovery/control includes `cad_agent_stop`, `cad_cancel_command`, and confirmed `cad_agent_resume`.

## Mutation and execution safety

Every ordinary mutation requires top-level `confirmMutation=true`. Nested or duplicate confirmation must not authorize a mutation. Emergency stop/cancel remain usable without that confirmation so an operator can recover an active action.
...
Emergency Stop advances/invalidate the mutation epoch. A queued mutation from an old epoch must remain stale even after Resume and must be resubmitted/reconfirmed.

UI input is also mutation-epoch-aware and aborts when BricsCAD process/window ownership or foreground conditions cease to be valid. `Alt+F4`, arbitrary desktop input, PowerShell, `cmd.exe`, shell/process launch and unrestricted command execution are outside the remote MCP boundary.

Mutations are locally audited with bounded/rotated sanitized records. Typed text/secrets must not be persisted into audit details.
...
-> BricsCAD-only UI fallback only when genuinely necessary
...
A successful tool call is not proof the drawing is correct; re-read state before final save/plot.
...
7. PKCE S256 exchange and tools discovery;
8. representative read-only tool calls including `cad_entity_inspect` where applicable;
9. authorization-code replay rejection;
10. no refresh token without `offline_access`;
11. refresh token with `offline_access`, rotation and replay rejection;
12. scope/resource mismatch rejection;
13. BricsCAD restart invalidating process-bound refresh and requiring reauthorization;
14. legacy static engineering-bearer compatibility;
15. Quick Tunnel URL change invalidating stale resource binding;
16. stable Named Tunnel restart/autostart/reconnect;
17. one confirmed disposable-DWG mutation, audit, `cad_agent_stop`/cancel/recovery;
18. save/reopen and clean process shutdown.

Do not commit access/refresh tokens, bearer secrets, Cloudflare credentials, private paths, private/customer DWGs, proprietary BricsCAD binaries or unsanitized screenshots.
