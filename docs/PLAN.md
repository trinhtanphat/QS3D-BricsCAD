...
Updated 2026-08-10 for the current `main` source baseline plus the full-repository ownership/capture hardening audit.
...
- generated output ownership has one Core contract: `PhysicalOpeningCutSolidHandle` and every `Generated*Handle(s)` owner slot are normalized/enumerated centrally; selection, ownership health, BOM release validation, semantic capture and Release Check consume the shared policy, while provenance/reference handles remain non-owner metadata;
- semantic capture rejects QS3D-generated output before mutation and uses full `ProjectStateSnapshot` rollback for single recognition/manual capture and multi-selection batches; HT_Phòng generation/synchronization uses the same rollback pattern to avoid partial project state after a failed regenerator;
...
- Quick Takeoff, bounded Current-Space B4D scan, BQ stable-ID grouping/filtering/Locate/XLSX, ED2/Excel Locate and deterministic recognition/review/auto-apply. B4D excludes every generated owner-slot handle through the shared ownership classification contract rather than a generated-family list;
...
- `QS3DREBARHEALTHALL` aggregates seven generated-rebar families and cross-family ownership; `QS3DHEALTHALL` adds model/source/generated-solid/stale/mode and curtain-frame health with issue-specific Locate;
- `QS3DRELEASECHECK` now includes Foundation Mesh health and generated rebar mode/category health in addition to provenance-safe ownership, live CAD, stale-state and BOM release guards;
- Ribbon, Workspace, Full Domain Hub, Curtain Hub and Geometry Extensions expose the main product workflows consistently, including Curtain 3D, Giao tường/snap, Auto/Manual Host, curved cuts, slab/wall/foundation mesh and Full Health;
...
- feature-specific static preflights cover Room curves/lifecycle, wall junction/snap, Auto Host, straight/curved opening, WallPier profile, slab/wall/foundation mesh, curtain native frame/config fingerprint, unified/release health, generated ownership, transactional semantic capture/finish rollback, command uniqueness, XAML well-formedness and Family/Instance inspector contracts;
...
2. Core Release build + deterministic smoke suite on the newest curtain/wall/opening/rebar/ownership/capture/UI head; older green runs do not validate later commits automatically.
...
4. NETLOAD/DemandLoad regression for workspace, Ribbon, Domain Hub, Curtain Hub, Family/Instance/Floor-Level editing, Focus/Isolate, Giao tường, wall snap, Auto Host, Room Auto, straight/curved opening cuts, recognition/template/revision/BQ/BBS, generated-source rejection and all generated-rebar families.
5. Private-DWG regression: Tường KT LINE/open-POLYLINE/curves; WallPier profiles; L/T/X networks and snap cleanup; Door/Opening ambiguous/elevation/curved-cut cases; Curtain host/frame overlay and stale fingerprint; Room Auto mixed LINE/POLYLINE/ARC/SPLINE; structure/takeoff/BQ/BBS; slab/wall/foundation mesh; shape/stirrup/tie/longitudinal rebar; failed-capture/finish rollback; save/reopen and multi-DWG lifecycle.
...
7. Performance tests for large room-boundary networks, wall-junction graphs, Auto Host candidate sets, curtain grids, BQ tables, SPLINE sampling, ownership registries and rebar/mesh batches.
...
Items depending on BricsCAD V25 runtime, private drawings, signing infrastructure or external services must not be marked complete from repository inspection alone. See `docs/FULL-REPO-AUDIT-2026-08-10.md` for the latest P0/P1/P2 audit rationale and validation boundary.
