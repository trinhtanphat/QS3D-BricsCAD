...
- `QS3DROOMAUTO` accepts planar LINE/POLYLINE/ARC networks. Direct ARC and polyline bulges are tessellated in metric geometry with configurable maximum sagitta; the adapter enforces plan-view/+Z and boundary elevation tolerance before face discovery;
...
- generic/full-domain/room-lifecycle/geometry-completion static preflights plus deterministic Core smoke coverage, including end-to-end guards for all three Tường KT native 3D variants and direct planar ARC Room Auto wiring;
...
- V25/private-DWG proof and performance tuning of automatic room-boundary discovery; direct planar ARC is implemented in source, while SPLINE/native non-planar curve support remains future work;
