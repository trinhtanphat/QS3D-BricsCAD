...
The preview reports source/target project IDs, whether project IDs match, and drawing fingerprint relation `Match`, `Different` or `Unknown`. These are review signals only. Matching IDs/fingerprints do not authorize overwrite, and a mismatch does not itself define the future copy/merge policy.
...
`ProjectInterchangeImportPreview.MaxDetailedItems = 10000`; complete counts remain available if detail rows are truncated.
