...
`WallJunctionOwnershipPlanner`, V25 native source materialization, strict XData ownership, whole-group invalidation/replacement, read-only health integration, command/UI wiring and static preflight are source-implemented on the LOCAL-007 P03 task branch. This is `SOURCE_IMPLEMENTED / PENDING_LOCAL`, not `LOCAL_PASS`: the licensed runtime evidence portion remains `LOCAL_ONLY`, and exact-SHA BricsCAD V25.2.10 geometry, failure, save/reopen, Undo/Redo and multi-DWG evidence remain required under `docs/LOCAL-AGENT-INBOX.md` item `LOCAL-007`.

The remaining gate is to qualify this exact implementation against real BricsCAD V25 native behavior without weakening the multi-owner boundary above.
