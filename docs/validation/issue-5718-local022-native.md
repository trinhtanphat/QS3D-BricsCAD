...
The final receipt and restoration record independently verify all private/protected/profile cleanup, nonce removal, zero hosts and original autostart restoration; cleanup_failure is null and MCP execution is false. Exact7134270dd push33968563514 and PR33968566293 passed preflight/core. Unchanged observed allocations are paused after repeated inter-turn operator scheduling failures; changing the runner to an explicit pause/resume contract requires resolving that design with the owner. Existing native/product assertions and consumed receipts remain unchanged. No further licensed host is left running while this is unresolved.
