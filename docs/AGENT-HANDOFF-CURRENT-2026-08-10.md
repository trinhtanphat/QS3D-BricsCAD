...
- Canonical GlassWall host replacements and individual LINE/path Curtain frame replacements remain cross-layer atomic in their own nested transaction families. `QS3DCURTAIN3D` now adds one outer native transaction around all host/frame phases plus a command-level `ProjectStateSnapshot`; aborting the outer transaction rolls back every nested CAD commit before the semantic snapshot is restored.
- The obsolete `PARTIAL COMMIT` path has been removed. Source/build/static proof is present, but exact-SHA V25 failure injection after LINE host, path host, LINE frame and path frame remains required before runtime qualification.
...
- Curtain phase-failure regression proving the outer native transaction and semantic snapshot restore the previous complete state after failures injected in each host/frame phase;
