# V25 structural document-generation affinity
...
This carrier hardens both the geometry handoff and post-commit UI phase of `QS3DBEAMREBAR3D` and `QS3DREBARSTIRRUP3D`. Native CAD/project mutation remains owned by the existing builders and mutation context. The change does not alter MCP, installer/release, or Quantity behavior.
...
- Immediately before handing selected `ObjectId`/semantic targets to `BuildSelected`, require that exact managed document + native database generation still owns the command. A reload/replacement must fail closed instead of handing stale selection geometry to a new generation.
...
- A stale document/generation suppresses mutation-path status/error output rather than writing through a stale editor wrapper.
...
- MDI A -> B and A -> B -> A switching between selection acquisition and geometry handoff;
- document reload/replacement where selected `ObjectId` values originate from an earlier native database generation;
- generation invalidation after native commit but before palette refresh/Regen/status output;
