...
`CoordinationManagerCommands` must retain one atomic published owner containing both the manager window and its `Document`.

1. Same active document + live published manager: activate/reuse it and return; do not close/recreate it.
2. Different active document + live published manager: request close, but retain static ownership while the request is in flight.
...
Capture exact SHA, BricsCAD version, package identity, active document identity, window count, and sanitized PASS/FAIL observations. Do not report `LOCAL_PASS` from source/static CI alone.
