...
- when host quiescence aborts, a failed initial registration that subscribed to the abort event immediately retries cleanup before any normal modeless quit-abort recovery path runs;
...
3. Race the initial attach failure against host quit/quiescence; verify the failed registration remains owned while cleanup is unsafe, retry fails closed during quiescence, and quit-abort eagerly completes cleanup before a later retry creates a fresh registration.
