...
- a later retry also asks that same failed registration to complete any remaining deferred cleanup when the host is safe; only after it is detached and its per-window native lifecycle subscription has been released may the table evict it and construct a fresh registration from the retry `Document`;
