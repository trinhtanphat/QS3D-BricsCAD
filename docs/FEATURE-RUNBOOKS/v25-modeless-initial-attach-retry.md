...
- a failed initial registration records cleanup ownership before modeless/WPF event subscription and before rollback starts;
- if host quiescence, a document close boundary, native subscription cleanup, or managed event unsubscription prevents rollback from completing, the failed registration remains canonical and retry fails closed instead of orphaning handlers or creating a duplicate lifecycle owner;
- managed unsubscribe exceptions do not clear the cleanup obligation: all managed/modeless event detach attempts must complete successfully before the failed registration is restartable;
- when host quiescence aborts, a failed initial registration that subscribed to the abort event schedules cleanup onto the window dispatcher; the dispatcher callback rechecks quiescence before cleanup, preserving the rule that WPF/document cleanup stays outside the native quit reactor stack;
- a later retry also asks that same failed registration to complete any remaining deferred cleanup when the host is safe; only after it is detached, its per-window native lifecycle subscription has been released, and managed handler cleanup is complete may the table evict it and construct a fresh registration from the retry `Document`;
...
3. Race the initial attach failure against host quit/quiescence; verify the failed registration remains owned while cleanup is unsafe, retry fails closed during quiescence, and quit-abort dispatches cleanup outside the native callback before a later retry creates a fresh registration.
...
5. Inject a managed/modeless event-unsubscribe failure during failed-initial-attach rollback; verify retry remains blocked, the exact failed registration stays canonical, a later successful cleanup clears the obligation, and no duplicate handler fires after restart.
6. Retry after an initial failure using a different project/drawing affinity and verify the normal fail-closed project/document checks still reject it.
7. Trigger two concurrent attach requests for the same `Window`; verify only one initial registration proceeds and no duplicate managed/native handlers are installed.
8. Trigger same-thread reentrant attach during an injected attach callback; verify the nested call fails without evicting/replacing the outer registration.
9. Force a repeated attach/rebind to fail after a window was already successfully attached; verify the original registration remains the sole lifecycle owner and a later retry cannot create duplicate handlers.
10. Close/abort/quit after a successful retry and verify subscription disposal, quiescence and window-close behavior remain compatible with the #6872 wrapper-rebind lifecycle contract.
