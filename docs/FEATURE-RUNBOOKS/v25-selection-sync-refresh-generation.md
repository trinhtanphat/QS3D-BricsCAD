...
A second ABA boundary existed in `Attach`: subscription/initial refresh happened before the catch rollback, while rollback blindly unsubscribed and removed document state. If native/modeless work detached T1 and reattached T2 before T1 threw, T1's catch could tear down T2. The fixed contract publishes the exact attachment token before entering native event subscription and rolls back only when that token still owns the wrapper.
...
- attachment generation identity to be published before the native subscription boundary;
- in-flight refresh ownership to map the exact `Document` to the captured attachment token;
- stale attach rollback to preserve a newer generation;
