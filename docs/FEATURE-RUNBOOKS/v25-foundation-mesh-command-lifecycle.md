...
- Exceptions observed after successful native commit return a committed result with `PostCommitCleanupWarning=true`, preventing a false mutation failure and unsafe user retry.
...
4. Throw from transaction/document-lock cleanup after commit: command must report committed success with a bounded cleanup warning and must not invite retry.
