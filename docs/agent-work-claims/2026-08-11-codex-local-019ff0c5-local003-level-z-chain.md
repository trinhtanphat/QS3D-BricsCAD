...
- `QS3DLEVELZPROBE` and `scripts/test-bricscad-v25-level-z.ps1` now exist as an automation-only boundary probe for a disposable synthetic drawing. It proves legacy native Beam Z and verifies a configured-Level rebuild remains blocked before ownership replacement; it deliberately records `production_level_qualified=false`. The probe is not a full Level geometry test and has not been run on this candidate because an unrelated user BricsCAD process is active. Do not claim exact-V25 Level proof until the broader local matrix is implemented and run against the exact clean candidate.
