...
### 2026-08-14 Material Usage collision fixture delegation

Parent task `/root` explicitly delegates only `tests/QS3D.Core.SmokeTests/MaterialUsageScheduleGroupKeyCollisionSmoke.cs` to `/root/fix_level_curtain_frame_z` under `2026-08-14-codex-material-usage-valid-collision-fixture.md`: replace unreachable U+001F Floor identity data with valid printable-delimiter values and add an explicit test-local legacy delimiter-collision assertion over the existing grouping tuple. The parent retains LOCAL-003 probe/runner ownership and will not edit this smoke while the child claim is `ACTIVE`.
...
Completed through PR `#1179` at `e23c7473a3783407a4ebc4d2db38a4848e1f644e`. The registered smoke now advances past the Browser null-metadata fixture and reaches the separate stale `ProjectQuantityReportGroupKeySmoke` delimiter fixture; no Level or production surface changed.

### 2026-08-14 Project Quantity collision fixture delegation
...
Across three consecutive goal turns, issue `#990` remained open without an assignee/fix and the operator-owned responsive BricsCAD session continued to hold a private drawing. The runner correctly refuses any pre-existing BricsCAD process, and the local worker must neither close that process nor bypass the mandatory full-smoke gate. The claim is therefore `BLOCKED` while preserving its reservation. Resume only after a non-local fix makes the full Core smoke pass and the operator saves/closes every BricsCAD session; then reactivate/push the claim, fetch the newest `main`, rebuild and rerun all exact-SHA gates plus the fresh disposable Level probe. No GitHub Actions were dispatched.
...
## 2026-08-14 product PASS; runner restore-order false negative

The clean exact `origin/main@13e83755cf34a5fde11ffd2ba05509db185143e4` licensed run emitted a complete sanitized product `PASS`: legacy wall `1.2 m .. 3.7 m`, bounded wall `3.1 m .. 6.8 m`, Bottom-Level Beam `3.25 m .. 3.85 m`, physical-opening reduction, 16 Curtain frames, 14 Curtain panels, four longitudinal bars, six stirrups, zero pre-edit Level health issues, seven stale outputs after the Level edit, Level-edit invalidation and Top-only fail-closed behavior. The host exited and no process, sidecar, backup or lock file remained.

The harness nevertheless rejected that PASS because it compared the disposable DWG hash before its `finally` block restored `level-z-original.dwg`; the hash was verified equal immediately after `finally`. Within this already-published runner/evidence reservation, cleanup and drawing restoration are moved into one idempotent helper that runs after host shutdown and before PASS hash/private-state verification, then runs again from `finally` for every exceptional path. Metadata records only cleanup/restore booleans. Product/Core source and the marker contract remain unchanged. LOCAL-003 remains `OPEN / PENDING_LOCAL` until this harness-only correction is merged and a fresh clean exact-SHA licensed run passes all runner assertions. No GitHub Actions are authorized.

## 2026-08-14 exact representative runtime PASS

PR `#1199` merged the harness-only restore-order correction at `2a1967d66f005cbdef20bb024a7b92a7f44077cc`. On that exact clean SHA, full Core smoke returned `ALL PASS`, every focused Level gate passed, the installed-reference BricsCAD V25 `Release|x64` build completed with zero warnings/errors, and plugin/Core ProductVersion ended in the exact SHA. The rebuilt adapter SHA-256 was `86D3938C89BB42BBE4F4854F7C8C027736426B5B20648363798ED634D811516F`.

The fresh licensed BricsCAD V25.2.10 x64 run passed every representative marker assertion: legacy wall `1.2..3.7 m`; Bottom+Top wall `3.1..6.8 m`; Bottom-only Beam `3.25..3.85 m`; physical-opening reduction; Curtain counts `16/14`; Beam longitudinal/stirrup counts `4/6`; zero pre-edit Level health issues; seven stale outputs after the Level edit; Level-edit invalidation and Top-only pre-mutation refusal. Graceful exit, zero process, script cleanup, private-state cleanup and drawing restoration all verified true. Before/after/current disposable-DWG SHA-256 remained `CEC1350FB2207542AEECD96A790A198A6C9CC9E99A9F875871F367554B3D967E`; retained files are only the synthetic copy and sanitized result/metadata outside the repository.

This completes the exact representative automated runtime probe and closes source issue `#1125`, but it does not satisfy the claim's complete local matrix. The mm/m complete-family, Undo, save/reopen, multi-DWG and representative private-DWG scenarios remain `PENDING_LOCAL`; this claim therefore remains `ACTIVE`, LOCAL-003 remains `IN_PROGRESS`, and customer-release qualification remains false. No GitHub Actions were run.

## 2026-08-15 exact representative native-unit parity PASS

Draft PR `#1432` combines the native-unit marker/runner contract with the handed-off read-only scripted-open correction. Exact clean implementation SHA `f00a948521d911aef314e7d8dd5b4bf1f7613bb3` passed the focused Level runtime preflight, full Core smoke and installed-reference BricsCAD V25 `Release|x64` build with zero warnings/errors. Plugin/Core ProductVersion ended in the exact SHA; the adapter SHA-256 was `F6D3113F6336FC3E945898F72B239646361F7524A3CEE0080E0D3246D08EB6AD`.

Licensed BricsCAD V25.2.10 x64 then passed the same guarded representative probe on separate synthetic copies in both native Millimeter (`INSUNITS=4`) and native Meter (`INSUNITS=6`) modes. Each marker identified its actual native unit and verified legacy wall `1.2..3.7 m`, bounded wall `3.1..6.8 m`, Bottom-Level Beam `3.25..3.85 m`, physical-opening reduction, Curtain `16/14`, Beam bars/stirrups `4/6`, zero pre-edit Level health issues, seven stale outputs, Level-edit invalidation and Top-only fail-closed behavior. Both test-owned hosts exited gracefully under the default 15-second bound after scripted no-save drawing close plus host quit.

For each run, zero-process cleanup, private script/sidecar/backup/lock cleanup, read-only-before-launch and through host exit, unwritten drawing verification, backup restoration and original attribute restoration all passed. Before/after/current synthetic-copy SHA-256 stayed `CEC1350FB2207542AEECD96A790A198A6C9CC9E99A9F875871F367554B3D967E`. Earlier branch attempts that either timed out before a marker or produced a PASS marker but failed graceful-exit acceptance remain diagnostic `NO_RESULT`/runner failures and are superseded by this exact dual-unit PASS.

This evidence closes only representative mm/m unit parity. Complete applicable-family coverage in both units, Undo/Redo, save/reopen/cold-cache rebind, multi-DWG isolation and representative authorized private-DWG evidence remain `PENDING_LOCAL`; the claim stays `ACTIVE`, LOCAL-003 stays `IN_PROGRESS`, customer-release qualification remains false, and no GitHub Actions were run.

## 2026-08-15 integrated-main native-unit parity confirmation

PR `#1432` was reported merged at `205e5d2c9ae2a4e6db1528745ee75bbfa0a3541c`, but a concurrent later GitHub merge was created from a stale base and current `main` temporarily lost that ancestry. PR `#1466` restored exactly the already-claimed/validated unit-parity diff without force-push and preserved the simultaneous P11 closeout and Source Reconcile claim records.

Exact merged SHA `44195df7acf5db6ea06605090384410367cf95f2` then passed the focused Level-Z runtime gate, full Core smoke and installed-reference V25 `Release|x64` build with zero warnings/errors. Plugin/Core ProductVersion ended in the exact SHA; adapter SHA-256 was `52669B6120A99422C357E25AF1D6343C324B598853C76D0D088FC3D6A30DE747`.

Fresh guarded BricsCAD V25.2.10 x64 runs passed separately for native Millimeter (`INSUNITS=4`) and Meter (`INSUNITS=6`). Each retained the complete representative contract: legacy wall `1.2..3.7 m`, bounded wall `3.1..6.8 m`, Bottom-Level Beam `3.25..3.85 m`, physical-opening reduction, Curtain `16/14`, Beam bars/stirrups `4/6`, zero pre-edit Level health issues, seven stale outputs, Level-edit invalidation and Top-only refusal. Both hosts exited gracefully; process/script/private-state cleanup, read-only-before-launch and through host exit, unwritten drawing, backup restoration and original attribute restoration passed. The synthetic drawing remained SHA-256 `CEC1350FB2207542AEECD96A790A198A6C9CC9E99A9F875871F367554B3D967E`.

This confirms representative mm/m parity on an integrated merged-main SHA. The claim deliberately remains `ACTIVE`; complete-family dual-unit coverage, Undo/Redo, save/reopen/cold-cache rebind, multi-DWG and authorized private-DWG evidence remain `PENDING_LOCAL`. No GitHub Actions or private/customer data was used.

## 2026-08-15 Level Undo/save-reopen lifecycle expansion

Issue `#1521` reserves the next bounded LOCAL_ONLY slice from synchronized baseline `origin/main@992ef63f1049bb6b49f2cf5b598e908757137eb1`: native Undo/Redo coherence around Level-bound generated output, followed by explicit save/close, cold reopen into a fresh plugin/project context and Level-bound semantic/native Z rebind. This scope is intentionally separate from the already-qualified representative unit-parity probe and does not claim complete-family, multi-DWG or private-DWG coverage.

The additive reservation is limited to `src/QS3D.BricsCAD.V25/LevelZLifecycleRuntimeProbeCommands.cs`, `scripts/test-bricscad-v25-level-z-lifecycle.ps1`, `scripts/preflight-level-z-lifecycle-runtime-probe.py`, and exact evidence/status updates in this claim, `docs/LOCAL-AGENT-INBOX.md` and the focused Level runbook. The existing representative probe/runner, production Level/builders, Core, shared lifecycle/Undo/persistence services, V26, installer, release and GitHub Actions remain read-only and excluded.

Qualification requires a clean exact SHA with matching plugin/Core ProductVersion, a zero-process preflight, repository-generated disposable DWGs only, outside-repository private scripts/sidecars/artifacts, allowlisted sanitized markers, and complete process/script/sidecar/DWG cleanup. Any ordinary production or runner-source defect must be reduced to sanitized evidence and handed to a non-local agent; this local worker will not absorb the source fix. The already-main-visible parent claim plus issue `#1521` and this pushed task branch/PR provide the reservation boundary; none authorizes a merge to `main`.

Even a complete bounded lifecycle PASS must publish `production_local003_qualified=false`; complete-family dual-unit, multi-DWG and authorized private-DWG rows remain outside this slice and keep overall LOCAL-003 `PENDING_LOCAL`.

## 2026-08-15 exact bounded Level lifecycle PASS

Clean exact implementation SHA `189bd34eb2e5795cf90acbc275b0d43b642cd1be` was committed and pushed before build or qualification. The official portable .NET SDK 8.0.423 used the existing user NuGet cache; full Core smoke returned `ALL PASS`, all focused lifecycle/P11/Level/static guards passed, and the installed-reference V25 `Release|x64` build completed with zero warnings/errors. Plugin/Core ProductVersion ended in the exact SHA. Adapter SHA-256 was `C947C814DD124744FBF140361BFB67C44C1B49583BD2824FC77E34BBD44818FA`; Core SHA-256 was `B3FCF2BFE2D2380E936BE2A7D41D7F4E0F967F8290F8484AD537BCD5C6F4F352`.

Licensed BricsCAD V25.2.10 x64 then passed the nonce-bound Millimeter lifecycle run. One Direct Draw GlassWall retained Bottom Level `3 m + 0.1 m` and Top Level `7 m - 0.2 m`; Undo preserved that configuration, restored the pre-build host and removed the generated set; Redo restored coherent `1/10/15` host/frame/panel output at `3.1..6.8 m`. Explicit save/close and a fresh BricsCAD process cold-reopened coherent Level config and output. Rebuild removed the old generated set, produced a disjoint new set with stable `1/10/15` counts, preserved the same Level span and returned zero Level/P11 Health issues. The reused production P11 marker independently reported coherent native/semantic Undo, Redo, reopen and rebuild.

Both test-owned hosts exited. Independent cleanup checks found zero BricsCAD processes, no transient `.scr` or private restore copy, no `.qsdb`/lock/backup/DWG lock residue, no runner environment residue, and exact restoration of the disposable drawing to SHA-256 `CEC1350FB2207542AEECD96A790A198A6C9CC9E99A9F875871F367554B3D967E`. Sanitized evidence remains outside Git; no private/customer drawing was used.

This closes issue `#1521`'s bounded representative lifecycle qualification only after PR `#1522` review/integration. It does not close the parent claim: complete-family dual-unit lifecycle rows, multi-DWG isolation and authorized private-DWG evidence remain `PENDING_LOCAL`; `production_local003_qualified=false`, customer-release qualification remains false, and no GitHub Actions were run.
