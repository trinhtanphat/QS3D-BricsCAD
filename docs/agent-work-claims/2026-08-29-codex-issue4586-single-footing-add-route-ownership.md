...
Licensed successor source `0db6e659510809a6781221204a32409605c851ba` now reaches the correct dimensions dialog and passes Cancel invariance, but fresh allocations 15/16 stopped at Family verification. Allocation 16 recorded canonical L1/W1=`2`, L2/W2/H1=`true`, H2=`false` after actual OK. The generic Workspace view initializes editable rows before the specialized footing view; NormalizePropertyValue classified canonical and legacy footing keys containing 0/1 as booleans and wrote them back during initial display. `WorkspaceViewModel.cs` now explicitly classifies the six canonical and six legacy dimension keys as numeric, without changing ordinary boolean semantics. The new `test-workspace-footing-property-normalization.ps1` executes actual production normalization, reproduces RED (`SINGLE_FOOTING_L1 changed 0 to false`), and verifies numeric preservation/boolean rejection plus ordinary boolean and metre/mm controls after correction. This is bounded acceptance work on the same Add workflow; matching new exact-source binaries remain required for the next licensed allocation.
