...
Compare this branch against moving `main`, confirm `StructuralRegenerator.cs` was not concurrently modified, inspect the exact PR patch, merge with expected head SHA only if clean, then post-merge read back source/test and record immutable merge evidence on `main`.
