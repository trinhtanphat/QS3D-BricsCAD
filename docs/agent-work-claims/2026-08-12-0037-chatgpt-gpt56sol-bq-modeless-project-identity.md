...
- The `Commands.cs` edit is restricted to capturing/passing BQ `ProjectId`; do not change unit-resolution ordering or unrelated command behavior in this lane.
...
- `QS3DBQ` captures the reviewed existing `ProjectId` from the read-only project used for initial row preparation and passes it explicitly to `QuantitySummaryWindow`.
- The window retains only that stable string identity, not a stale mutable `ProjectState`.
