...
6. `phase=reopen` — includes `cold_reopen_verified=true`, `prior_sequence_reasserted=false`, and `qualification_requires_prior_markers=true`.

`QS3DSRGRIPP05REOPEN` runs after a real process restart, so in-memory `_state` from the earlier phases no longer exists. Its PASS marker proves persisted final-state continuity only. It MUST NOT reassert manual cancel/commit/reconcile/rebuild evidence and MUST NOT be treated as a standalone aggregate qualification marker.
