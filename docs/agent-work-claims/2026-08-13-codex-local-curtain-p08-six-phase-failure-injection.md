...
- Follow-up diagnostics resolved that broad result to `LINE_HOST_METADATA_REJECTED`, and source inspection confirmed the mixed-selection contradiction described in the expansion above. The canonical selection guard/orchestrator correction is implemented locally: each specialized builder now receives only its validated LINE/path partition, while the complete validated selection is restored for stamps/UI/failure paths. Builders and their fail-closed single-type contracts are unchanged. V25 `Release|x64` builds at zero warnings/errors and all focused P01-P08/native/orchestration gates pass; exact merged-SHA licensed proof remains required.
