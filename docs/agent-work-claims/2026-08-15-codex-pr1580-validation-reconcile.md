...
6. Once those blockers are removed, the aggregate reaches two additional static gates that still require pre-XML-hardening `SetPersistedScalar(...)` call literals. The source now passes the same raw/normalized values through `PersistedTextXml.Verify(...)` immediately before `SetPersistedScalar`, preserving and strengthening the guarded mutation order; the gates must bind that current validation-to-mutation contract.
...
The Floor and Interchange/QSDB smokes will assert the supported public mutation rejection, construct valid state, inject only the exact malformed legacy/raw backing field through test-local reflection, assert the injection, and retain the downstream planner/exporter/persistence rejection. The structural gates will change only their directly stale generalized helper/source-elevation tokens. The two persisted-scalar gates will require the current `PersistedTextXml.Verify(...)`-wrapped values immediately before `SetPersistedScalar`, retaining their revision/mutation-order assertions. The runtime probe will capture nullable measurement properties in stable locals before `HasValue`/`Value`, without changing marker, geometry, runner, or privacy behavior.
