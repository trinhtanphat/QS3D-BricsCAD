...
- Record exact interactive V25 selection/cancel/Undo/save-reopen checks as LOCAL_ONLY; do not claim them executed remotely.

## Coordination

The only currently published `ACTIVE` claim is the repository-wide registration-protocol bootstrap, whose explicit exclusion is QS3D product source changes. This lane therefore does not overlap it. If a new overlapping Direct Draw or selection-ownership claim appears after this reservation is pushed, stop and re-scope before implementation.

## Completion condition

Create Similar Quick/Advanced commands are merged to current `main` with static contract coverage and handoff documentation, this claim is marked `COMPLETED` with the implementation SHA(s), and all live BricsCAD V25-only evidence remains explicitly unclaimed/LOCAL_ONLY.
