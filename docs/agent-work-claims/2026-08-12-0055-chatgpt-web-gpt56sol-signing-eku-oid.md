...
Current `scripts/sign-v25.ps1` requires an Enhanced Key Usage extension, locates it by OID, then calls `Format($false)` and accepts/rejects based on rendered text containing either the code-signing OID or the English words `Code Signing`. X509 extension display text is presentation data and can vary by platform/locale; a valid code-signing certificate may therefore be falsely rejected on a non-English/alternate Windows renderer.

## Reserved scope

Validate the required EKU extension from its structured `X509EnhancedKeyUsageExtension.EnhancedKeyUsages` OID collection and require OID `1.3.6.1.5.5.7.3.3`. Preserve current policy that a certificate **without** an EKU extension is rejected, and preserve CurrentUser store, private-key, validity, timestamp HTTPS, SHA-256 signing and post-sign signer checks. Add an auto-discovered static/model regression and document the certificate gate.
...
- Require EKU extension; copy/cast it to `X509EnhancedKeyUsageExtension`, enumerate `EnhancedKeyUsages` OID values and require exact Code Signing OID.
- Regression model: missing EKU FAIL; Code Signing OID PASS; unrelated Client Auth/Server Auth OIDs FAIL; labels/localization are irrelevant.
...
Signing EKU authorization is required, OID-structural and locale-independent before signing starts, with regression/docs on `main` and this claim marked `COMPLETED`.
