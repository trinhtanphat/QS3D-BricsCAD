...
Parent task `/root` delegated the four stale aggregate-preflight consumers to `/root/fix_curtain_method_gates` under claim `2026-08-14-codex-issue1106-curtain-method-gates.md`. That lane exclusively owns `preflight-curtain-empty-partition-orchestration.py`, `preflight-curtain-frame-atomicity.py`, `preflight-curtain-frame-transaction-boundary.py`, and `preflight-curtain-orchestration-boundary.py`; it will align their exact method/call tokens with the already-merged optional `allowInteractiveSelection` signatures without changing production source or runtime automation. This claim retains issue `#1106`, LOCAL-002 P10 and the exact licensed rerun.
