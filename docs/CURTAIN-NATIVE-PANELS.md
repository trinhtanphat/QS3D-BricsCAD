...
Run `scripts/preflight-curtain-panel-ownership-failure-runtime-probe.py` first. The guarded runner requires an exact clean repository x64 Release V25 DLL, initialized profile, fresh `*.curtain-ownership-probe-copy.dwg`, empty artifact directory outside the repository, no existing BricsCAD process and no sidecar/backup. It launches one hidden PID and accepts only `QS3D_CURTAIN_PANEL_OWNERSHIP_RUNTIME_V1`; PASS/FAIL markers contain aggregate booleans/counts or allowlisted phase/code only, never raw Handles, semantic IDs, paths, profiles, drawing content or exception detail. PASS is deferred until process exit, private-script deletion, unchanged DWG hash and sidecar/backup absence are verified. This source handoff is not runtime evidence: P06, P07-P12 and overall LOCAL-002 remain `PENDING_LOCAL` until a fresh licensed exact-SHA run returns the complete PASS contract.
