...
Run `scripts/preflight-curtain-panel-budget-provenance-runtime-probe.py` first. The runner requires a clean exact repository x64 Release V25 DLL, initialized profile, fresh `*.curtain-budget-provenance-probe-copy.dwg`, empty artifact directory outside the repository, no BricsCAD process and no pre-existing sidecar/backup/drawing lock. PASS/FAIL markers are aggregate-only and allowlist phase/code diagnostics; PASS is deferred until process exit, private-script and `.dwl`/`.dwl2` deletion, unchanged DWG hash and sidecar/backup absence are verified. This is a source handoff, not runtime evidence. P07-P12 and overall LOCAL-002 remain `PENDING_LOCAL` until a fresh licensed exact-SHA run returns the complete contract.
