...
## Scope
Issue #6970 removes four independent compensated-sum implementations from 2D quantity aggregation, 2D takeoff workflow, live workbook dependency aggregation, and Cubicost reviewed inventory aggregation. All four now delegate numeric authority to `QuantityReportMath.FiniteAccumulator` while keeping their surface-specific validation and error behavior.
