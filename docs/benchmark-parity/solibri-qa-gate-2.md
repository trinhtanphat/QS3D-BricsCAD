...
Profile construction validates the complete severity domain. Both the blocking threshold and every configured per-rule severity must be one of `Info`, `Warning`, `Error`, or `Critical`; undefined numeric enum values are rejected before evaluation. This keeps malformed external configuration from turning the hard gate into a late `Rank(...)` exception path.
...
The severity-domain hardening keeps the public `QsQaRuleProfile` constructor and severity enum unchanged for valid callers. Configuration adapters and deserializers that previously cast arbitrary integers into `QsQaSeverity` must now map only the supported `Info`, `Warning`, `Error`, and `Critical` values and reject unknown numeric values before profile construction. Existing valid serialized profiles and severity overrides require no migration.
...
The registered `BenchmarkParitySuiteSmoke` additionally verifies that `NaN`, positive infinity and negative infinity dimensions produce `QA2.INVALID_DIMENSIONS`, leave QA2 blocked, and deny Takeoff/BOQ/Estimate. It also verifies that undefined blocking-threshold and per-rule severity enum values are rejected when a profile is constructed while valid severity overrides remain accepted.
