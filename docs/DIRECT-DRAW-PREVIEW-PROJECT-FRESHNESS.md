...
- the command started projectless and a project appears before commit;
...
`scripts/preflight-direct-draw-project-preview-freshness.py` statically guards the shared resolver and covered command families.

Exact BricsCAD V25 interaction proof remains in `LOCAL-008` of `docs/LOCAL-AGENT-INBOX.md`: hold each `*ADV` command at a numeric prompt, replace/remove/reload the project or make a project appear, then confirm the command refuses before source CAD, semantic or native mutation. Source review or static preflight does not constitute `LOCAL_PASS`.
