...
| LOCAL-004 | P0 / SOURCE_FIX_REQUIRED | P01-P05 remain bounded PASS. P06 on exact `3c5367e9408c5f93b58163e493ae95c0bbaf206c` passed Wall/Door cut, native STRETCH, reconcile invalidation, rebuild and recut, then reproduced `NATIVE_UNDO_SEMANTIC_DIVERGENCE`: native Solid3d returned to uncut while complete `PhysicalOpeningCut*` semantic state remained. Remote/source issue #3787 owns the correction. | Do not rerun unchanged source. After #3787 publishes a distinct exact pushed fix SHA, rerun marked recut -> native Undo -> native Redo first; only then resume save/cold-reopen/multi-DWG and broader #80 coverage. |
