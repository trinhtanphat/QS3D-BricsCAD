...
- Scenario: Run `scripts/run-local-v25-qualification.ps1` from a clean exact SHA with the real V25 install directory; prove Core Release build, Core smoke, adapter exact-V25 build, NETLOAD, DemandLoad, command registration, save/reopen, and multi-DWG isolation. Also cold-start/reopen a drawing with an existing `.qsdb`, invoke an existing-project mutation before another command has warmed the cache, verify the mutation binds the canonical project, then save/reopen. Include one regeneration-based CSV/XLSX export and one modeless Door/Room refresh/export in that cold-cache state. Repeat with the sidecar absent and verify ownership-dependent mutation/export/refresh refuses without leaving a new project in the live cache.
- Evidence required: Exact QS3D SHA, Windows build, BricsCAD V25 build, .NET/MSBuild version, command/load results, cold-cache existing-sidecar ProjectId continuity, regenerated/exported data continuity, absent-sidecar refusal/no-new-project result, sanitized failure log if any.
