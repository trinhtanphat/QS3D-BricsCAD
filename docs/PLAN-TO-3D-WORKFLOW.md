...
Exact BricsCAD V25 proof cho Plan-to-3D command contract nằm trong **LOCAL-014** của `docs/LOCAL-AGENT-INBOX.md`. Automation-only `QS3DPLAN2DPROBE` khóa P01; P01/P02 quick creation, P03 physical-ESC cancellation, P04/P05 preview freshness và P06 mixed-batch/ownership-scoped exception compensation đã có bounded licensed evidence. P07 đã PASS native QSAVE/sidecar/fresh-process reopen nhưng phát hiện native Undo không bỏ grouped `QS3DPLAN2WALLS` output; source defect `#3966` đang chặn promotion và exact-SHA P07 rerun vẫn `PENDING_LOCAL` sau fix. Vì vậy LOCAL-014 là `PARTIAL_LOCAL_PASS / BLOCKED_SOURCE_FIX`, không phải full `LOCAL_PASS`; `production_local014_qualified=false` vẫn authoritative. Runtime proof cho `QS3DDRAWWINDOW`, các quick-workflow Ribbon entry, Auto Host và explicit `QS3DCUTSELECTEDOPENINGS` handoff vẫn nằm riêng trong **LOCAL-008**, nơi source review không được coi là `LOCAL_PASS`.

P02 cho LOCAL-014 dùng automation-only `QS3DPLAN2DP02PREPARE`, production quick alias `QS3DCONVERT2D`, `QS3DPLAN2DP02SELECTALIAS`, production quick alias `QS3DPLAN2WALLS`, rồi `QS3DPLAN2DP02VERIFY` qua runner `scripts/test-bricscad-v25-plan-to-3d-p02.ps1`. Probe tạo đúng một LINE và một open straight POLYLINE, áp preferred active `ArchitecturalWall` Family `0.31 m / 4.2 m / 0.45 m`, và yêu cầu unrelated dirty semantic state được giữ nguyên.

P02 đã `LOCAL_PASS` tại exact clean SHA `7f57130470d4440f25dd27ea0bc3207cbb777a07` trên BricsCAD V25.2.10 x64. Hai production quick aliases đã tạo hai semantic `ArchitecturalWall` và hai owned Solid3d từ một LINE + một open straight POLYLINE với preferred Family `0.31 m / 4.2 m / 0.45 m`; unrelated dirty state, source geometry, native bounds, ownership disjointness và wall-scoped Core/runtime Health đều đạt. Disposable DWG giữ nguyên SHA-256, process/script/sidecar/backup/lock được dọn sạch.

P03/P04 lần lượt PASS physical ESC tại cả ba advanced numeric prompt ở exact SHA `1736ae8db0086041f0b1e8ce4b79839469b10061`, và fail-closed cho same-ObjectId geometry edit, planar UCS, `INSUNITS`, project appearance/same-project version drift ở exact SHA `f70c694d94f26bb1cc1be8025931d9a1a6559bb0`. Trên exact SHA `dffb7e334f997a981a9d918c23b67592e232d61f`, P05 tiếp tục PASS Model/Paper-space context drift, non-planar UCS, source delete/retype và project replacement trước commit. Active-DWG switch ngay trong numeric prompt bị chính BricsCAD từ chối bằng `BRX_E334_EDOCUMENTSWITCHDISABLED`; cell này kết thúc bằng physical ESC, không mutation và được ghi là `HOST_BLOCKED`, không suy diễn product verdict.

P06 trên cùng exact SHA đã PASS mixed LINE/open-POLYLINE với cả `QS3DPLAN2WALLS` và `QS3DCONVERT2DADV`. Failure injection tại lần append Solid3d thứ hai của `QS3DCONVERT2D` trả current-batch output về zero, restore semantic snapshot/audit/change/dirty/pending state chính xác, chỉ xóa owned objects của batch lỗi, đồng thời giữ nguyên source, unrelated dirty semantics, pre-existing owned CAD, foreign CAD và DWG thứ hai.

P07 tạo state `2 semantic walls / 2 owned solids`, trong đó wall thứ hai nằm trong native Undo group. Native `U` đáng lẽ trả về `1 / 1` nhưng state vẫn `2 / 2` (`UNDO_AFTER_GENERATED_STILL_PRESENT`); `REDO` không tạo divergence, rồi native `QSAVE`, sidecar persistence và cold reopen ở process mới đều giữ coherent `2 / 2`. Kết quả này PASS persistence boundary nhưng FAIL Undo/Redo pair. Issue `#3966` phải được sửa ở source; sau merge chỉ rerun bounded P07 trên exact clean SHA mới trước khi xem xét promotion LOCAL-014.
