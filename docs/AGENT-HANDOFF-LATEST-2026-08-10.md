...
**Branch:** `main`  
**Source reconciliation cutoff for this edition:** `c2edfc5f6ce069dbe78196c66b456f5b06a173db` (`fix(geometry): clear stale metadata after dependent invalidation`) plus the rebased B4D/ED2, DWG-identity and generated-geometry hardening branch documented below.
...
- branch baseline: `origin/main` `c2edfc5` plus rebased B4D/ED2 and hardening changes;
...
- all seventeen local `preflight*.py` scripts: PASS using BricsCAD's bundled Python 3.9;
