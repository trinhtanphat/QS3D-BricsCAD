...
Parent program: #3142  
Original matrix lane: #3308 / `issue-3308`  
Current reconciliation: #3313 / `issue-3313`  
Baseline audited: `main@c80405e4cd1e0530b16acf1e98d580ef4e76cd0c`  
...
- **ACTIVE_DEPENDENCY** — another current canonical carrier owns remaining source-safe work for the stated row; this matrix must not duplicate it.
...
| 8 | Export a readable workbook/report with stable source provenance and support reverse trace requested by the customer | Existing ED2/XLSX evidence semantics and Element ID / CAD Handle / drawing-fingerprint provenance are merged through #3145. PR #3299 / Issue #3296 landed the requested `DGKL`, `COP_PHA`, `CHI_TIET`, `TRACE_MODEL` customer workbook, evidence-aware blank-vs-zero semantics, canonical TRACE_KEY/provenance validation, aggregate/detail Excel -> CAD live-handle resolution, and `QS3DEXCEL` / `QS3DEXCELTRACE` routing. | SOURCE_PROVEN at source/CI boundary | Interactive Excel -> CAD selection/highlight/focus in a licensed V25 host remains LOCAL_ONLY under #72. |
...
| 10 | Reject stale/missing/unsupported conditions instead of guessing | #3144 rejects malformed hosted-opening identity before publishing replacement quantities; #3145 preserves missing-vs-zero evidence; PR #3155 fails closed on unresolved units/materials; PR #3240 pins missing-element selection refusal. PR #3299 additionally fails closed on wrong-DWG fingerprint, malformed/ambiguous trace identity, provenance drift and incomplete live-handle resolution before PICKFIRST replacement. | SOURCE_PROVEN for the landed source-safe contracts | Runtime-only host failures and interactive stale-object behavior remain LOCAL_ONLY. |
...
## Customer workbook / reverse-trace evidence

Issue #3296 is closed/completed and PR #3299 merged to `main` as `99dc024faafa4becc1a89fa61a894f69fba8aa49`.

The landed source-safe contract includes:

- exactly four customer worksheets: `DGKL`, `COP_PHA`, `CHI_TIET`, `TRACE_MODEL`;
- one semantic element per `CHI_TIET` row and complete grouped Element-ID provenance for aggregate rows;
- canonical, recomputed `TRACE_KEY` identity instead of trusting workbook text alone;
- unsigned 64-bit CAD Handle support, including high-bit handles;
- wrong-DWG/provenance/malformed/partial-live-resolution refusal before selection replacement;
- Excel 32,767-character text-cell limit enforcement;
- deterministic Core smoke and protected `preflight` + `core` validation.

This closes the prior source-safe `ACTIVE_DEPENDENCY` classification for acceptance row 8. It does **not** convert the licensed V25 interaction boundary into a runtime PASS.

## Current metadata reconciliation notes

Some child Issue bodies can be stale relative to merged PR truth and therefore must not be used alone as current acceptance evidence:
...
- #3296 is now closed/completed; PR #3299 is the implementation truth for the customer workbook/reverse-trace slice.

This reconciliation does **not** mutate historical child carriers merely to normalize metadata. PR/main ancestry remains the implementation truth; ownership of unrelated active lanes remains untouched.

## Remaining P0 blocker / evidence boundary
...
No currently identified source-safe dependency remains for the customer workbook + aggregate/detail Excel -> CAD trace slice itself. New source defects still require their own leaf Issue/Lane-Key rather than being folded into this reconciliation.

## C0 closure rule

The source-safe matrix is complete for the currently identified P0 rows when every row has merged evidence or an explicit LOCAL_ONLY boundary. Program #3142 must **not** be called fully customer-ready merely because this document is source-safe complete. Native customer acceptance still depends on #72 and the separate product-gap issues explicitly retained by the program.
