...
This preparation is deliberately dormant for Level-configured elements. `LevelReferenceNativeIntegrationPolicy.EnsureQualified(...)` blocks native mutation while the policy qualifies no category. Legacy elements without Level metadata continue through the existing source-relative path. This fail-closed boundary prevents imported or hand-edited Level metadata from moving only part of a host/dependent chain.

The Floor/Level UI intentionally does **not** expose Bottom/Top Level assignment yet. Physical opening cutters and Curtain LINE/path frames/panels already consume the shared resolver. Beam/Column longitudinal bars, stirrups/ties, Slab/Foundation/StructuralWall meshes, Level-configured BBS shape origins and Railing LINE height now share the host bottom/effective-height result. Stair rise/thickness semantics and the complete V25 runtime matrix remain open before any category can be enabled.
