...
The Floor/Level UI intentionally does **not** expose Bottom/Top Level assignment yet. Physical opening cutters and Curtain LINE/path frames/panels already consume the shared resolver. Beam/Column longitudinal bars, stirrups/ties, Slab/Foundation/StructuralWall meshes and Level-configured BBS shape origins now share the host bottom/effective-height result. Stair/Railing placement and the complete V25 runtime matrix remain open before any category can be enabled.
