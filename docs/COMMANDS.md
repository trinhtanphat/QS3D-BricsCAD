...
- `QS3DBEAMREBAR3D` — guarded top/bottom Beam longitudinal-bar generation for supported horizontal Beam `LINE` source. Explicit top/bottom counts are supported; a single even-count notation can be split symmetrically. Compound notation with ambiguous placement is rejected rather than guessed.
- `QS3DREBARHEALTH` — verify generated Column/Beam longitudinal ownership, handle/count, mode/category and stale state.
...
- `QS3DREBARMAT3D` — generate deterministic orthogonal X/Y rebar mats for selected Slab/Foundation rectangle footprints. Direction notation is `Dxx@spacing`; `RebarMatFaces=Bottom|Top|Both`. The current native adapter deliberately rejects non-rectangular/bulged footprints rather than extending bars outside the host.
- `QS3DREBARMATHEALTH` — verify generated Slab/Foundation mat handles, ownership, live solids, category/faces, count and stale state.
- `QS3DREBARHEALTHALL` — aggregate longitudinal, BBS-shape, Column tie, Beam stirrup and Slab/Foundation mat health in one review/Locate window.
...
See [`ADVANCED-GEOMETRY.md`](ADVANCED-GEOMETRY.md) for shape/stirrup/tie metadata and current geometric limits, and [`REBAR-MAT3D.md`](REBAR-MAT3D.md) for the Slab/Foundation mat contract.
...
The main palette, Ribbon and Full Domain Hub expose the major product flows consistently: Room Auto, Tường KT, Giao tường + review-gated snap cleanup, Auto/Manual Door-Opening host linking, physical cuts, Build 3D, Focus/Isolate/Section Box, BQ/BBS, Column/Beam longitudinal rebar, BBS-shape rebar, Beam stirrups, Column ties, Slab/Foundation rebar mats, unified health checks and revision tools. The goal is to minimize command-line memorization while preserving explicit commands for power users and test harnesses.
