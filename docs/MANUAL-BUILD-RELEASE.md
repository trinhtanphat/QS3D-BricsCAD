...
Every workflow in `.github/workflows/` must remain `workflow_dispatch` only. Commits, pushes, pull requests, merges, documentation updates, source fixes, reviews and `continue all` requests must leave GitHub Actions idle.
...
The workflow deliberately has no `push`, `pull_request`, `schedule`, `workflow_run`, `workflow_call`, `repository_dispatch`, release-event or deployment-event trigger.
...
9. runs `scripts/package-v25.ps1`;
