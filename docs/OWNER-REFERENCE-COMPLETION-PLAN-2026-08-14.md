...
| Screenshot right-side drawing/layer manager lacks a dense selected-target drilldown | Current `RightPanel` already exposes Xref/layer actions and list columns, but the selected drawing/Xref path/kind/scale/lock/instance data and selected layer visibility/lock/color data are scattered across columns/status | Add compact presentation-only selected drawing/Xref and selected layer detail cards bound to existing `DrawingList.SelectedItem` / `LayerList.SelectedItem`; introduce no new CAD/semantic mutation path | `preflight-owner-reference-right-detail.py` pins registration, bindings, labels, null safety and forbids native/service mutation ownership | Remote source implemented; exact docking/DPI/dark-theme visual acceptance remains LOCAL_ONLY |
...
6. **Family/detail/drawing re-audit** — change only proven residual gaps after checking current claims. The screenshot RightPanel selected-target drilldown is handled as a presentation-only sub-lane and must not absorb RightPanel startup or mutation services.
