...
- Deterministic recognition/review/auto-apply with mandatory entity-type compatibility for fallback rules and exact project layer mappings; DBText/MText cannot be auto-captured as structural geometry merely because its layer/text matches. Metricless `ProxyEntity`/BRC candidates are review-only and capture itself fails closed, preventing Family defaults from manufacturing quantities before a supported measurement adapter exists.
- `QS3DUNITS` removes the silent millimeter fallback for undefined/unsupported `INSUNITS`: an explicit override is saved atomically in `.qsdb`, quantities are bound to the effective capture unit, and B4D/BQ/ED2/source reconcile reject unresolved or mismatched unit state.
