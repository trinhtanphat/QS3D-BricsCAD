...
- `QS3DROOMAUTO` accepts selected planar LINE/POLYLINE/ARC networks. Direct ARC inputs require plan-view normal +Z, LINE/ARC/POLYLINE elevations are checked against `RoomBoundaryToleranceM`, and ARC/polyline bulges are tessellated before Core topology discovery. Boundary provenance is stored without taking duplicate `SourceHandles` ownership; the operation writes audit events, regenerates Room quantities, is exposed in Ribbon + Full Domain Hub, and rolls semantic/audit mutations back if the update/regeneration pipeline fails.
...
- expanded generic/full-domain/geometry-completion preflight guards cover schema/persistence, command uniqueness, generated geometry, structural quantities, BBS CSV safety, all three Tường KT line/polyline 3D variants, wall footprint/opening boolean/rebar geometry source paths, planar LINE/POLYLINE/ARC Room Auto discovery/planarity/lifecycle/rollback/UI wiring, DemandLoad wiring and PowerShell syntax.
...
- Those runs predate the newest Room Auto ARC/planarity, wall footprint/polyline, physical opening, rebar-geometry and BLT UI/Tường KT completion batches. These newest heads must not be called CI-verified until a later explicitly approved run covers them.
...
- V25/private-DWG proof and large-network performance tuning for automatic room-boundary discovery; direct planar ARC is implemented in source, while SPLINE/native non-planar curve support is not claimed;
