...
- clean-room dark WPF design system, three-pane main workspace, separate Drawing/Xref + Layer manager, Full Domain Hub, Curtain Wall Hub, Geometry Extensions and native Ribbon bootstrapper.
...
- the primary Workspace exposes **Giao tường**, **Snap xem**, **Snap áp** and **Auto Host** beside the main Family/modeling actions. Ribbon and Full Domain Hub additionally expose Curtain 3D, slab/wall mesh and unified health entry points.
...
- `GeneratedCurtainFrameConfigFingerprint` deterministically snapshots current length/height/bottom offset/panel limits/frame widths/frame depth. `GeneratedCurtainFrameHealthService` detects missing/live/ownership/count issues and stale configuration after Family/Instance/grid changes.
...
- current limitations are explicit: no curved/open-POLYLINE frame overlay, no panel-by-panel backing glass solids, and Door/Opening booleans do not yet interrupt frame overlay solids.
...
- `scripts/preflight-all.py` auto-discovers feature `preflight-*.py` scripts; both GitHub workflows remain `workflow_dispatch` only. No GitHub Action was dispatched as part of these source changes.
...
That proof **predates** the newest curtain native frames/config fingerprint, dedicated slab/wall mesh ownership/health, curved-opening idempotence hardening and several concurrent UI/health batches. It must not be used as proof that the current `main` head has compiled or run inside BricsCAD V25.
...
- These runs predate the newest geometry/rebar/curtain/UI batches. The current head must not be described as CI-verified until a later explicitly approved run covers it.
...
- compile the **current final SHA** against the exact target BricsCAD V25 managed assemblies and run DemandLoad/NETLOAD command/Ribbon/palette regression;
...
- real V25 validation of Family/Instance/Floor-Level scope, typed controls, Focus/Isolate/Section and Unicode/HiDPI behavior;
...
- curtain opening-aware frame interruption, panel-by-panel glass solids and curved/open-POLYLINE frame overlay remain product work;
