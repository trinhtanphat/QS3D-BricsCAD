...
15. `docs/REMAINING-ISSUE-INDEX-2026-08-10.md` — **compact GitHub issue map for the remaining runtime, BLT-authoring, geometry, engineering, documentation, performance and release gaps**;
16. `docs/AGENT-HANDOFF-SESSION-HISTORY-2026-08-10.md` only when deeper session chronology, old branch/gate history, screenshot requirements or early implementation evidence is needed.
...
Start local qualification from `docs/LOCAL-V25-QUALIFICATION.md`. Run `scripts/run-local-v25-qualification.ps1` against a **clean exact SHA** before manual scenario testing. For feature work that is intentionally deferred to a real V25/engineering/signing environment, also read `docs/LOCAL-AGENT-REMAINING-GATES-2026-08-10.md` and `docs/REMAINING-ISSUE-INDEX-2026-08-10.md`, then close only the gates/issues for which the local agent can produce the required evidence. Keep generated runtime evidence under `artifacts/` or another explicitly local folder; `artifacts/` is intentionally gitignored. Do not claim a customer-release qualification when the runner used `-SkipRuntime`.
