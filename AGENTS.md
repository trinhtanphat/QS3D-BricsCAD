...
- A local worker may run **local** builds, preflights, deterministic smoke, BricsCAD runtime probes, or other local scripts only when they are required evidence for the already-assigned LOCAL_ONLY item. If such a local validation exposes an unrelated failure, record the minimum sanitized observation and stop at that boundary; do not turn it into a new coding lane unless the owner explicitly marks that defect LOCAL_ONLY or assigns it to that local worker.
- If no compatible `OPEN` / `IN_PROGRESS` LOCAL_ONLY item exists, or the remaining items require capability the worker does not have, the local worker must stop/idle rather than broadening scope into CI or general code work.
- This scope lock overrides older or more general wording that merely says local agents should “prioritize” local work. For these two local workers, LOCAL_ONLY is a **hard allow-list**, not a preference.
