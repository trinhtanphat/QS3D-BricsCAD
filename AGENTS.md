...
14. `docs/LOCAL-AND-REMAINING-TODO-2026-08-10.md` — **remaining product/runtime gaps mapped to local/remote agents and GitHub issues**;
...
When a remote agent reaches a task that requires local-only access, leave the repository in a runnable/testable state and document the exact local validation needed. The canonical local execution handoff is `docs/LOCAL-V25-QUALIFICATION.md`; extend it when a new runtime-only feature needs a new scenario. Use `docs/LOCAL-AND-REMAINING-TODO-2026-08-10.md` to keep unresolved product/runtime gaps explicit rather than silently calling them complete. When a local agent finishes that validation, commit only reusable source/scripts/docs and a sanitized text summary if useful; never commit proprietary BricsCAD DLLs, private fixtures, screenshots containing private drawings, signing secrets or raw machine evidence.
