...
Scope: add bounded, deduplicated self-healing repair metadata to existing MCP tools/call failures without adding a new MCP tool or bypassing the process-global CAD writer. Repeated identical repairable failures must open a circuit and require human review; caller/policy/schema mistakes must never become automatic source-repair candidates.
