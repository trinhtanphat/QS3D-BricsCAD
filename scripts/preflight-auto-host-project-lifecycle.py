...
if not PATH.is_file():
    ...
    if "ExistingProjectMutationContext.TryGet(document, out var project)" not in text:
        errors.append("QS3DAUTOLINKHOSTS must resolve an existing canonical mutation project")
    ...
    if selected_index < 0 or empty_selection_index < 0 or project_guard_index < 0:
        ...
        errors.append("Auto Host must reject empty selection before resolving existing canonical project state")
...
print("PASS: Auto Host is side-effect free without selection/project state, mutates canonical existing state, and preserves rollback coverage.")
