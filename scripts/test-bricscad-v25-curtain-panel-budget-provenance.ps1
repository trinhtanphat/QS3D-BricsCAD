...
if (-not (Test-Path -LiteralPath $windowInteropPath -PathType Leaf)) { throw "Curtain P07 runner window interop helper is missing." }
. $windowInteropPath
...
function Read-Qs3dMarker {
    ...
    foreach ($line in Get-Content -LiteralPath $Path) {
        ...
        if ($separator -le 0) { throw "Malformed Curtain P07 marker line." }
        $key = $line.Substring(0, $separator).Trim()
        $value = $line.Substring($separator + 1).Trim()
        if ($marker.ContainsKey($key)) { throw "Duplicate Curtain P07 marker key." }
        $marker[$key] = $value
    }
    return $marker
}
...
function Require-Qs3dValue {
    ...
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P07 marker is missing a required key." }
    if (-not [string]::Equals([string]$Marker[$Key], $Expected, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Curtain P07 marker contains an unexpected required value."
    }
}
...
function Read-Qs3dAllowedValue {
    ...
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P07 marker is missing a diagnostic key." }
    $value = [string]$Marker[$Key]
    foreach ($candidate in $Allowed) {
        if ([string]::Equals($value, $candidate, [StringComparison]::Ordinal)) { return $value }
    }
    throw "Curtain P07 marker diagnostic token is not allowlisted."
}

function Read-PositiveMarkerInt {
    param([Parameter(Mandatory = $true)]$Marker, [Parameter(Mandatory = $true)][string]$Key)
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P07 marker is missing a positive-count key." }
    [int]$value = 0
    if (-not [int]::TryParse([string]$Marker[$Key], [Globalization.NumberStyles]::None, [Globalization.CultureInfo]::InvariantCulture, [ref]$value) -or $value -le 0) {
        throw "Curtain P07 marker count is not a positive invariant integer."
    }
    return $value
}
...
function Stop-Qs3dLaunchedProcess {
    ...
    if (-not $Process.HasExited) { throw "Launched BricsCAD Curtain P07 process did not exit." }
}
...
function Remove-Qs3dDrawingLocks {
    ...
    foreach ($path in $Paths) {
        ...
        if (Test-Path -LiteralPath $path) { throw "Curtain P07 drawing-lock cleanup failed." }
    }
}

if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) { throw "Curtain P07 runtime qualification requires Windows." }
if (-not [Environment]::UserInteractive) { throw "Curtain P07 runtime qualification requires an interactive Windows session." }
if (-not $ConfirmDisposableCopy) { throw "Pass -ConfirmDisposableCopy only for a disposable synthetic drawing copy." }
if ([string]::IsNullOrWhiteSpace($Profile)) { throw "Curtain P07 runtime qualification requires an initialized BricsCAD profile." }
...
if (-not [IO.Path]::GetFileName($DrawingCopy).EndsWith(".curtain-budget-provenance-probe-copy.dwg", [StringComparison]::OrdinalIgnoreCase)) {
    throw "DrawingCopy must use the guarded '*.curtain-budget-provenance-probe-copy.dwg' suffix."
}
...
foreach ($required in @($bricscadExe, $PluginDll, $coreDll, $DrawingCopy)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "A required Curtain P07 runtime input is missing." }
}
...
if ($gitStatus.Count -ne 0) { throw "Curtain P07 runtime qualification requires a clean exact-SHA worktree." }
...
if (@(Get-Qs3dExactBricsCadProcesses -ExpectedExecutable $bricscadExe).Count -gt 0) {
    throw "Close existing BricsCAD V25 processes before starting the isolated Curtain P07 runtime probe."
}
...
foreach ($forbiddenInput in @($projectSidecar, ($projectSidecar + ".bak"), $drawingBackup) + $drawingLocks) {
    if (Test-Path -LiteralPath $forbiddenInput) { throw "The disposable Curtain P07 copy has a pre-existing sidecar or backup." }
}
...
$resultPath = Join-Path $ArtifactDir "curtain-panel-budget-provenance-runtime-result.txt"
$scriptPath = Join-Path $ArtifactDir "curtain-panel-budget-provenance-runtime.scr"
$metadataPath = Join-Path $ArtifactDir "curtain-panel-budget-provenance-runtime-metadata.json"
foreach ($output in @($resultPath, $scriptPath, $metadataPath)) {
    if (Test-Path -LiteralPath $output) { throw "Curtain P07 runtime output must not already exist." }
}
...
$oldResult = [Environment]::GetEnvironmentVariable("QS3D_CURTAIN_PANEL_P07_RESULT", "Process")
$oldNonce = [Environment]::GetEnvironmentVariable("QS3D_CURTAIN_PANEL_P07_NONCE", "Process")
$process = $null
$proxyInformationDialogsDismissed = 0
$startedAt = Get-Date

try {
    $env:QS3D_CURTAIN_PANEL_P07_RESULT = $resultPath
    $env:QS3D_CURTAIN_PANEL_P07_NONCE = $nonce
    $script = @(
        "FILEDIA", "0",
        "CMDECHO", "1",
        "TILEMODE", "1",
        ...
        "UCS", "W",
        "NETLOAD", ('"' + $PluginDll + '"'),
        "QS3DCURTAINP07SEEDHOSTS",
        "QS3DGLASSWALL",
        "QS3DCURTAINP07SEEDOPENINGS",
        "QS3DDOOR",
        "QS3DCURTAINP07PREPARE",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP07BASELINE",
        "QS3DCURTAINP07BUDGET",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP07CHECKBUDGET",
        "QS3DCURTAINP07MISSING",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP07CHECKMISSING",
        "QS3DCURTAINP07OFFHOST",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP07CHECKOFFHOST",
        "QS3DCURTAINP07VALID",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP07PROBE"
    )
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        $process.Refresh()
        if ($process.HasExited) { throw "BricsCAD exited before the Curtain P07 marker." }
        Start-Sleep -Milliseconds 500
    }
    if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) { throw "Timed out waiting for the synthetic Curtain P07 ownership matrix." }
    ...
    if ($marker.ContainsKey("status") -and [string]::Equals([string]$marker["status"], "FAIL", [StringComparison]::Ordinal)) {
        $failurePhases = @(
            ...
            "PREPARE_OFF_HOST", "VERIFY_OFF_HOST_ROLLBACK", "PREPARE_VALID_REPLACEMENT", "VERIFY_VALID_REPLACEMENT", "RESULT_PUBLISH"
        )
        ...
        foreach ($key in $marker.Keys) {
            if (-not $failureKeys.Contains([string]$key)) { throw "Curtain P07 FAIL marker contains a non-contract field." }
        }
        Require-Qs3dValue -Marker $marker -Key "command" -Expected "QS3DCURTAINP07PROBE"
        Require-Qs3dValue -Marker $marker -Key "nonce" -Expected $nonce
        Require-Qs3dValue -Marker $marker -Key "schema" -Expected "QS3D_CURTAIN_PANEL_BUDGET_PROVENANCE_RUNTIME_V1"
        Require-Qs3dValue -Marker $marker -Key "qualification_boundary" -Expected "LOCAL_002_P07_ONLY"
        Require-Qs3dValue -Marker $marker -Key "production_local002_qualified" -Expected "false"
        Require-Qs3dValue -Marker $marker -Key "error_code" -Expected "CURTAIN_PANEL_BUDGET_PROVENANCE_RUNTIME_FAILED"
        ...
        Require-Qs3dValue -Marker $marker -Key "opening_count" -Expected "2"
        Require-Qs3dValue -Marker $marker -Key "refusal_count" -Expected "3"
        foreach ($key in @(
            "panel_budget_refused", "missing_source_refused", "off_host_refused",
            "later_element_failure_verified", "whole_batch_native_preserved", "whole_batch_semantic_preserved",
        ...
        if ($baselinePanelCount -le 2 -or $finalPanelCount -le 2) { throw "Curtain P07 panel counts are unexpectedly small." }
    }

    Stop-Qs3dLaunchedProcess -Process $process
    Remove-Qs3dDrawingLocks -Paths $drawingLocks
    if (-not (Wait-Qs3dNoExactBricsCadProcesses -ExpectedExecutable $bricscadExe -TimeoutSeconds 30)) { throw "Curtain P07 V25 process cleanup is incomplete." }
    if (Test-Path -LiteralPath $scriptPath) { Remove-Item -LiteralPath $scriptPath -Force -ErrorAction Stop }
    if (Test-Path -LiteralPath $scriptPath) { throw "Curtain P07 runtime script cleanup failed." }
    $drawingHashAfter = (Get-FileHash -LiteralPath $DrawingCopy -Algorithm SHA256).Hash.ToUpperInvariant()
    if (-not [string]::Equals($drawingHashBefore, $drawingHashAfter, [StringComparison]::Ordinal)) { throw "The disposable Curtain P07 drawing was written unexpectedly." }
    foreach ($forbiddenOutput in @($projectSidecar, ($projectSidecar + ".bak"), $drawingBackup)) {
        if (Test-Path -LiteralPath $forbiddenOutput) { throw "Curtain P07 runtime probe persisted an unexpected sidecar or backup." }
    }
    if ($diagnosticFailure) { throw "Curtain P07 probe failed at sanitized phase '$failurePhase' with code '$failureCode'; cleanup was verified." }
    ...
    Write-Host "QS3D BricsCAD V25 Curtain-panel P07 budget/provenance rollback runtime PASS"
    ...
    try {
        ...
        Restore-EnvironmentValue -Name "QS3D_CURTAIN_PANEL_P07_RESULT" -Value $oldResult
        Restore-EnvironmentValue -Name "QS3D_CURTAIN_PANEL_P07_NONCE" -Value $oldNonce
    }
}
