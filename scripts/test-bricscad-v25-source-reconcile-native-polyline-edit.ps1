...
if (-not (Test-Path -LiteralPath $windowInteropPath -PathType Leaf)) { throw "LOCAL-004 P02 runner window interop helper is missing." }
. $windowInteropPath
...
function Read-Qs3dMarker {
    ...
    foreach ($line in Get-Content -LiteralPath $Path) {
        ...
        if ($separator -le 0 -or $separator -eq $line.Length - 1) { throw "Malformed LOCAL-004 P02 marker line." }
        $key = $line.Substring(0, $separator).Trim()
        $value = $line.Substring($separator + 1).Trim()
        if ($marker.ContainsKey($key)) { throw "Duplicate LOCAL-004 P02 marker key." }
        $marker[$key] = $value
    }
    return $marker
}
...
function Require-Qs3dValue {
    ...
    if (-not $Marker.ContainsKey($Key)) { throw "LOCAL-004 P02 marker is missing a required field." }
    if (-not [string]::Equals([string]$Marker[$Key], $Expected, [StringComparison]::OrdinalIgnoreCase)) {
        throw "LOCAL-004 P02 marker field did not match its required value."
    }
}
...
function Require-Qs3dMarkerSchema {
    ...
    $expectedKeys = @(
        "status", "command", "nonce", "schema", "qualification_boundary", "production_local004_p02_qualified",
        "native_vertex_stretch_verified", "pre_sync_isolation_verified", "area_perimeter_reconcile_verified",
        "quantity_recalculation_verified", "generated_invalidation_verified", "generated_rebuild_verified",
        "native_solid_bounds_verified", "scoped_health_verified",
        "cold_reopen_verified", "source_type", "edit_command", "final_geometry_class", "error_code"
    )
    if (@($Marker.Keys).Count -ne $expectedKeys.Count) { throw "LOCAL-004 P02 marker contains an unexpected field." }
    foreach ($key in $expectedKeys) {
        if (-not $Marker.ContainsKey($key)) { throw "LOCAL-004 P02 marker is missing a required field." }
    }
    Require-Qs3dValue $Marker "status" "PASS"
    Require-Qs3dValue $Marker "nonce" $Nonce
    Require-Qs3dValue $Marker "schema" "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_RUNTIME_V1"
    Require-Qs3dValue $Marker "qualification_boundary" "LOCAL_004_P02_CLOSED_POLYLINE_VERTEX"
    Require-Qs3dValue $Marker "production_local004_p02_qualified" $(if ($Qualified) { "true" } else { "false" })
    foreach ($key in @(
        "native_vertex_stretch_verified", "pre_sync_isolation_verified", "area_perimeter_reconcile_verified",
        "quantity_recalculation_verified", "generated_invalidation_verified", "generated_rebuild_verified",
        "native_solid_bounds_verified", "scoped_health_verified"
    ...
    Require-Qs3dValue $Marker "source_type" "POLYLINE"
    Require-Qs3dValue $Marker "edit_command" "STRETCH"
    Require-Qs3dValue $Marker "final_geometry_class" "QUADRILATERAL_13_5_M2"
...
function Require-Qs3dFailureMarker {
    ...
    $expectedKeys = @(
        ...
        "production_local004_p02_qualified", "error_code", "failure_phase", "failure_code"
    )
    if (@($Marker.Keys).Count -ne $expectedKeys.Count) { throw "LOCAL-004 P02 failure marker contains an unexpected field." }
    foreach ($key in $expectedKeys) {
        if (-not $Marker.ContainsKey($key)) { throw "LOCAL-004 P02 failure marker is missing a required field." }
    }
    Require-Qs3dValue $Marker "status" "FAIL"
    Require-Qs3dValue $Marker "nonce" $Nonce
    Require-Qs3dValue $Marker "schema" "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_RUNTIME_V1"
    Require-Qs3dValue $Marker "qualification_boundary" "LOCAL_004_P02_CLOSED_POLYLINE_VERTEX"
    Require-Qs3dValue $Marker "production_local004_p02_qualified" "false"
    Require-Qs3dValue $Marker "error_code" "SOURCE_RECONCILE_NATIVE_POLYLINE_RUNTIME_FAILED"
    $allowedPhases = @(
        "prepare", "native_stretch", "select_source", "check_reconcile", "final_rebuild", "cold_reopen"
    )
    if (-not ($allowedPhases -contains [string]$Marker["failure_phase"])) { throw "LOCAL-004 P02 failure phase is not allowlisted." }
    $allowedCodes = @(
        ...
        "SELECTION_REJECTED", "SOURCE_GEOMETRY_REJECTED", "EXPECTED_GEOMETRY_REJECTED",
        "SEMANTIC_METRICS_REJECTED", "QUANTITIES_REJECTED", "GENERATED_HANDLE_REJECTED",
        "GENERATED_OWNERSHIP_REJECTED", "GENERATED_MUTATED_BY_NATIVE_STRETCH",
        "NATIVE_STRETCH_GEOMETRY_REJECTED", "NATIVE_STRETCH_SEMANTIC_REJECTED",
        "NATIVE_STRETCH_QUANTITY_REJECTED",
    ...
    if (-not ($allowedCodes -contains [string]$Marker["failure_code"])) { throw "LOCAL-004 P02 failure code is not allowlisted." }
}
...
function Stop-Qs3dLaunchedProcess {
    ...
    if (-not $Process.HasExited) { throw "Launched LOCAL-004 P02 BricsCAD process did not exit." }
}
...
function Find-Qs3dHandoffProcess {
    ...
    if ($matches.Count -gt 1) { throw "LOCAL-004 P02 launcher produced an ambiguous BricsCAD process handoff." }
...
function Wait-Qs3dHandoffProcess {
    ...
    throw "BricsCAD launcher exited without an exact child-process handoff or LOCAL-004 P02 marker."
}
...
function Stop-Qs3dLateHandoffProcesses {
    ...
    foreach ($launcherId in $LauncherIds) {
        ...
        if ($null -ne (Find-Qs3dHandoffProcess -LauncherId $launcherId -ExpectedExecutable $ExpectedExecutable)) {
            throw "LOCAL-004 P02 launcher retained a late BricsCAD child process."
        }
    }
}
...
function Wait-Qs3dMarker {
    ...
    while ((Get-Date) -lt $Deadline) {
        ...
        if ($current.HasExited) {
            if ($handoffAdopted) { throw "BricsCAD exited before the LOCAL-004 P02 marker was published." }
    ...
    throw "Timed out waiting for the LOCAL-004 P02 marker."
}
...
function Wait-Qs3dExit {
    ...
    throw "Timed out waiting for the LOCAL-004 P02 BricsCAD process to exit."
}

function Remove-ExactFile {
    param([Parameter(Mandatory = $true)][string]$Path)
    if (Test-Path -LiteralPath $Path) { Remove-Item -LiteralPath $Path -Force -ErrorAction Stop }
    if (Test-Path -LiteralPath $Path) { throw "LOCAL-004 P02 exact private-file cleanup failed." }
}

if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) { throw "LOCAL-004 P02 qualification requires Windows." }
if (-not [Environment]::UserInteractive) { throw "LOCAL-004 P02 qualification requires an interactive Windows session." }
if (-not $ConfirmDisposableCopies) { throw "Pass -ConfirmDisposableCopies only for repository-sample disposable copies." }
if ([string]::IsNullOrWhiteSpace($Profile)) { throw "LOCAL-004 P02 qualification requires an initialized BricsCAD profile." }
...
if ($LASTEXITCODE -ne 0 -or $gitOutput.Count -ne 1) { throw "Cannot resolve the exact LOCAL-004 P02 Git candidate SHA." }
$gitHead = ([string]$gitOutput[0]).Trim().ToLowerInvariant()
if ($gitHead -notmatch '^[0-9a-f]{40}$') { throw "LOCAL-004 P02 Git candidate SHA is invalid." }
...
$drawing = Join-Path $fixtureRoot "source-native-polyline-edit-probe-copy.dwg"
...
if (-not [string]::Equals((Get-FileHash -LiteralPath $drawing -Algorithm SHA256).Hash, $fixtureHash, [StringComparison]::OrdinalIgnoreCase)) {
    throw "LOCAL-004 P02 disposable copy hash mismatch."
}
...
foreach ($path in $privateFiles) {
    if (-not $path.StartsWith($fixtureRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
        throw "LOCAL-004 P02 private-state path escaped the fixture root."
    }
    if (Test-Path -LiteralPath $path) { throw "LOCAL-004 P02 disposable copy has pre-existing private state." }
}

$resultPath = Join-Path $ArtifactDir "source-reconcile-native-polyline-result.txt"
$phasePath = Join-Path $ArtifactDir "source-reconcile-native-polyline-session1.txt"
$scriptOnePath = Join-Path $ArtifactDir "source-reconcile-native-polyline-session1.private.scr"
$scriptTwoPath = Join-Path $ArtifactDir "source-reconcile-native-polyline-session2.private.scr"
$metadataPath = Join-Path $ArtifactDir "source-reconcile-native-polyline-metadata.json"
$nonce = [Guid]::NewGuid().ToString("N")
$pluginHash = (Get-FileHash -LiteralPath $PluginDll -Algorithm SHA256).Hash.ToUpperInvariant()
$environmentNames = @(
    "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_RESULT", "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_PHASE_RESULT",
    "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_NONCE", "QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_DWG"
)
$oldEnvironment = @{}
...
try {
    $env:QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_RESULT = $resultPath
    $env:QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_PHASE_RESULT = $phasePath
    $env:QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_NONCE = $nonce
    $env:QS3D_SOURCE_RECONCILE_NATIVE_POLYLINE_DWG = $drawing
    ...
    if (Test-Path -LiteralPath $resultPath -PathType Leaf) {
        ...
        throw "LOCAL-004 P02 session one published a sanitized failure marker."
    }
    ...
    if (-not $drawingPersistedChanged) { throw "LOCAL-004 P02 session one did not persist its disposable drawing changes." }
    $sidecarPersisted = Test-Path -LiteralPath $sidecar -PathType Leaf
    if (-not $sidecarPersisted) { throw "LOCAL-004 P02 session one did not persist its disposable semantic sidecar." }
    Remove-ExactFile -Path $scriptOnePath

    $scriptTwo = @(
        "FILEDIA", "0", "CMDECHO", "1", "TILEMODE", "1", "INSUNITS", "4", "UCS", "W",
        "NETLOAD", ('"' + $PluginDll + '"'), "QS3DSRPOLYREOPEN", "_.QUIT", "_Y"
    )
    ...
    if ([string]::Equals([string]$finalMarker["status"], "PASS", [StringComparison]::OrdinalIgnoreCase)) {
        ...
        Require-Qs3dFailureMarker -Marker $finalMarker -Nonce $nonce
        throw "LOCAL-004 P02 cold reopen published a sanitized failure marker."
    }
}
    ...
    try {
        ...
        if (@(Get-Process -Name "bricscad" -ErrorAction SilentlyContinue).Count -ne 0) { throw "LOCAL-004 P02 process cleanup is incomplete." }
        ...
        if (-not $drawingRestoreVerified) { throw "LOCAL-004 P02 drawing restore verification failed." }
        Remove-ExactFile -Path $drawing
        if (@(Get-ChildItem -LiteralPath $fixtureRoot -Force).Count -ne 0) { throw "LOCAL-004 P02 fixture root retained private files." }
        Remove-Item -LiteralPath $fixtureRoot -Force -ErrorAction Stop
        if (Test-Path -LiteralPath $fixtureRoot) { throw "LOCAL-004 P02 fixture-root cleanup failed." }
    }
...
$metadata = [ordered]@{
    status = $status
    qualification_boundary = "LOCAL_004_P02_CLOSED_POLYLINE_VERTEX"
    git_sha = $gitHead
