...
def main():
    if not SOURCE.exists() or not SMOKE.exists() or not KIND_SMOKE.exists():
    ...
    if not ok:
        return 1
    print("PASS: preview review snapshots are versioned, fingerprinted, canonical-kind guarded, handle-safe, atomic, and Core-only.")
    return 0
