...
def main():
    ...
    require(commands, '"\\nQS3DSTART could not open the Start Center."', "stable command diagnostic message")
    forbid(commands, "ex.Message", "Start Center command diagnostic")
    ...
    require(host, "if (_documentActivatedMayBeSubscribed) return;", "idempotent activation subscription")
    require(host, "if (!_documentActivatedMayBeSubscribed || _documentActivatedDetachInProgress) return;", "idempotent activation unsubscription")
    require(host, "private static void OnDocumentActivated(object sender, DocumentCollectionEventArgs e)", "activation handler")
    require(host, "document = e.Document ?? Application.DocumentManager.MdiActiveDocument;", "event-document capture")
    require(host, "nativeDatabaseIdentity = DocumentGenerationGuard.CaptureCurrent(document);", "native generation capture")
    require(host, "panel.RefreshFromDocument(document, nativeDatabaseIdentity);", "event-document generation refresh callback")
    forbid(host, "_panel.RefreshFromActiveDocument();", "process-global active-DWG refresh callback")
    require(host, "TryWriteRefreshDiagnostic(document, nativeDatabaseIdentity);", "generation-fenced activation refresh diagnostic")
    require(host, '"\\nQS3DSTART refresh could not update the Start Center."', "stable activation refresh diagnostic text")
    forbid(host, "ex.Message", "activation refresh diagnostic")
    ...
    require(panel, "ProjectFileUiService.OpenProject(normalized)", "recent DWG shared open")
    ...
    forbid(panel, "Application.DocumentManager.Open(normalized, false)", "recent DWG direct-open bypass")
