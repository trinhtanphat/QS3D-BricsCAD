...
def main():
    ...
    require(host, "panel.RefreshFromDocument(e.Document ?? Application.DocumentManager.MdiActiveDocument);", "event-document refresh callback")
    forbid(host, "_panel.RefreshFromActiveDocument();", "process-global active-DWG refresh callback")
    require(host, 'e.Document?.Editor.WriteMessage("\\nQS3DSTART refresh could not update the Start Center.");', "stable activation refresh diagnostic")
