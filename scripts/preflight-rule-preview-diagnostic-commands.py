...
for path in (SOURCE, RULES, DIAG):
    if not path.is_file():
        errors.append("missing rule-preview/diagnostic command contract file: " + str(path.relative_to(ROOT)))
...
if SOURCE.is_file():
    ...
    for forbidden in (
        ...
        if forbidden in text:
            errors.append("QS3DRULEPREVIEW/DIAGSUMMARY must remain read-only with respect to live semantic/native project state: " + forbidden)
...
if errors:
    print("QS3D rule preview + diagnostic command preflight")
    for error in errors:
        print("ERROR:", error)
    print("FAILED with", len(errors), "error(s).")
    sys.exit(1)

print("PASS: QS3DRULEPREVIEW is a detached/read-only rule dry-run and QS3DDIAGSUMMARY atomically exports the privacy-safe aggregate diagnostic contract. V25 runtime qualification remains local.")
