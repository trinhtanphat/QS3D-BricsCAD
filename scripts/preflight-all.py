...
def discover():
    ...
    for path in candidates:
        if path.is_symlink():
            raise RuntimeError("feature preflight must not be a symlink: " + str(_relative_to_root(path)))
        if not path.is_file():
            raise RuntimeError("feature preflight must be a regular file: " + str(_relative_to_root(path)))
...
def run_gate(path, child_env):
    ...
    try:
        ...
        return str(rel), "timeout"
    except OSError as exc:
        print("ERROR: failed to start", rel, "-", exc)
        return str(rel), "launch"

    if completed.returncode != 0:
        return str(rel), "exit=" + str(completed.returncode)
    return None
