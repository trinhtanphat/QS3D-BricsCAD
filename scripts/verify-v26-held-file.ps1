...
function Assert-NoReparseAncestor {
    ...
    while ($null -ne $cursor) {
        if (($cursor.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "Held V26 release input traverses a reparse-point ancestor: $($cursor.FullName)"
        }
...
function Open-HeldGeneration {
    ...
    if ($admitted.PSIsContainer) { throw "Held V26 release input must be a file: $canonical" }
    if (($admitted.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
        throw "Held V26 release input must not be a reparse point: $canonical"
    }
    ...
    if (-not [string]::Equals($canonical, $admittedPath, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Held V26 release input canonical identity drifted before open: $canonical"
    }
    ...
    try {
        ...
        if ($rebound.PSIsContainer -or (($rebound.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) {
            throw "Held V26 release input changed to a non-ordinary file after open: $canonical"
        }
        if (-not [string]::Equals($admittedPath, $reboundPath, [StringComparison]::OrdinalIgnoreCase) -or
            ...
            throw "Held V26 release input generation changed across admission/open: $canonical"
        }
    ...
    switch ($Operation) {
        ...
        'Copy' {
            ...
            if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
                throw "Held V26 release copy destination parent does not exist: $parent"
            }
            if (Test-Path -LiteralPath $destinationFull) {
                throw "Held V26 release copy destination already exists: $destinationFull"
            }
            ...
            if ([int64]$copied.Length -ne [int64]$held.Length) {
                ...
                throw "Held V26 release copy length mismatch: $destinationFull"
            }
