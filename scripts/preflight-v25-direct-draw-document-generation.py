...
# Error cleanup mutates CAD and semantic state. A stale generation must not
# restore its snapshot into a successor context, and Forget(document) must use
# a fresh generation check after cleanup because cleanup itself is a native /
# re-entrant boundary.
require(
    r"if\s*\(generationIsCurrent\)\s*\{\s*try\s*\{\s*EraseDirectDrawCad\(.*?\}\s*catch.*?try\s*\{\s*rollback\.Restore\(project\);\s*\}\s*catch.*?\}",
    "Direct Draw rollback restore must stay inside the captured-generation cleanup fence",
)
require(
