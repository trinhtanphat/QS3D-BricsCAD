...
function Require-Value {
    ...
    if ([string]::IsNullOrWhiteSpace($Value)) {
        throw "$Name is required for this lane."
    }
}
...
function Get-CanonicalLocalQueue {
    ...
    foreach ($match in $matches) {
        ...
        $rows.Add([pscustomobject]@{
            ...
            heading = $heading
            noRerun = ($body -match '(?im)\bNO_RERUN\b')
            body = $body
        })
    }
    if ($rows.Count -eq 0) {
        throw 'Canonical local queue contains no parseable items.'
    }
    return $rows.ToArray()
}
...
$contracts = [ordered]@{
    ...
    'LOCAL-008' = [ordered]@{ host = 'V25'; mode = 'MANUAL_OR_EXTERNAL'; runners = @(); note = 'Quick/ADV prompt drift, Auto Host/reference and UI breadth require interactive editor input.' }
...
if ($Action -eq 'Plan') {
    ...
    if ($selected.Count -eq 0) {
        throw "Lane '$Lane' is not present in the canonical inbox."
    }
...
if ($contract.Contains('minimumMergedCarrier')) {
    $minimumMergedCarrier = [string]$contract.minimumMergedCarrier
    & git -C $repoRoot merge-base --is-ancestor $minimumMergedCarrier HEAD *> $null
    if ($LASTEXITCODE -ne 0) {
        throw "NO_RESULT: HEAD does not contain required merged carrier $minimumMergedCarrier for lane $Lane. Pull/sync the newest intended exact SHA before running."
    }
}
...
switch ($Lane) {
    ...
    'LOCAL-011' {
        $args = @{ ArtifactDir = (Join-Path $ArtifactDir 'local-011') }
        if (-not [string]::IsNullOrWhiteSpace($BricsCadDir)) {
            $args.BricsCadDir = $BricsCadDir
        }
        if (-not [string]::IsNullOrWhiteSpace($Profile)) {
            $args.Profile = $Profile
        }
        & (Join-Path $PSScriptRoot 'run-local-v25-local-011.ps1') @args
    }
    'LOCAL-013' {
        Require-Value $Profile '-Profile'
