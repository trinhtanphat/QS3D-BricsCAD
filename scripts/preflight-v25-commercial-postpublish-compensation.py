#!/usr/bin/env python3
"""Fail closed unless manual V25 post-publish compensation preserves release classification."""
...
def contract_errors(source: str) -> list[str]:
    ...
    pre_prerelease_proof = "[bool]$authoritativeCommercialReleaseBeforeCompensation.prerelease -ne ($env:RELEASE_PRERELEASE -eq 'true')"
    ...
    compensation_body = "$commercialCompensationBody = @{ draft = $true } | ConvertTo-Json"
    ...
    post_prerelease_proof = "[bool]$authoritativeCommercialReleaseAfterCompensation.prerelease -ne ($env:RELEASE_PRERELEASE -eq 'true')"
    ...
    required = (
        ...
        (pre_prerelease_proof, "pre-compensation prerelease preservation proof"),
        ...
        (compensation_body, "narrow safe-state compensation payload"),
        ...
        (post_prerelease_proof, "post-compensation prerelease preservation proof"),
    ...
    ordered = (
        pre_get, pre_release_id_proof, pre_url_proof, pre_name_proof, pre_body_proof,
        pre_prerelease_proof, pre_asset_id_proof, pre_asset_size_proof, unambiguous_state,
        already_draft, compensation_body, compensation_patch, post_get, post_release_id_proof,
        post_url_proof, post_prerelease_proof, post_name_proof, post_body_proof,
        post_asset_id_proof, post_asset_size_proof, terminal,
    )
    indexes = [region.find(token) for token in ordered]
    if min(indexes) >= 0 and indexes != sorted(indexes):
        errors.append("commercial compensation ordering must prove exact identity/classification before narrow PATCH and re-prove it afterwards")

    conditional_patch = re.compile(
        r"if\s*\(\s*\$authoritativeCommercialReleaseBeforeCompensation\.draft\s*-eq\s*\$true\s*\)\s*\{"
        r"(?:(?!\n\s*\}\s*else\s*\{).)*\n\s*\}\s*else\s*\{\s*"
        r"\$commercialCompensationBody\s*=\s*@\{\s*draft\s*=\s*\$true\s*\}\s*\|\s*ConvertTo-Json.*?"
    ...
    if not conditional_patch.search(region):
        errors.append("commercial compensation PATCH must be conditional and mutate only draft=true")

    if re.search(r"\$commercialCompensationBody\s*=\s*@\{[^}]*\bprerelease\s*=", region, re.DOTALL):
        errors.append("commercial safety compensation must not mutate prerelease classification")
    ...
    if pre >= 0 and patch >= 0:
        ...
        for token, label in (
            ...
            (pre_prerelease_proof, "prerelease classification"),
    ...
    if post >= 0 and terminal_index >= 0:
        ...
        for token, label in (
            ...
            (post_prerelease_proof, "prerelease classification"),
...
def self_test() -> list[str]:
    exact_uri = '$releaseUri = "https://api.github.com/repos/$env:GITHUB_REPOSITORY/releases/$releaseId"'
    pre_id = "[long]$authoritativeCommercialReleaseBeforeCompensation.id -ne $releaseId"
    pre_url = "[string]$authoritativeCommercialReleaseBeforeCompensation.url, $releaseUri"
    pre_name = "[string]$authoritativeCommercialReleaseBeforeCompensation.name, $expectedReleaseName"
    pre_body = "([string]$authoritativeCommercialReleaseBeforeCompensation.body).IndexOf($draftTransactionMarker"
    pre_pre = "[bool]$authoritativeCommercialReleaseBeforeCompensation.prerelease -ne ($env:RELEASE_PRERELEASE -eq 'true')"
    pre_aid = "[long]$preMatches[0].id -ne $expectedAssetId"
    pre_asize = "[int64]$preMatches[0].size -ne [int64]$localAssets[$expectedAsset].Length"
    post_id = "[long]$authoritativeCommercialReleaseAfterCompensation.id -ne $releaseId"
    post_url = "[string]$authoritativeCommercialReleaseAfterCompensation.url, $releaseUri"
    post_name = "[string]$authoritativeCommercialReleaseAfterCompensation.name, $expectedReleaseName"
    post_body = "([string]$authoritativeCommercialReleaseAfterCompensation.body).IndexOf($draftTransactionMarker"
    post_pre = "[bool]$authoritativeCommercialReleaseAfterCompensation.prerelease -ne ($env:RELEASE_PRERELEASE -eq 'true')"
    post_aid = "[long]$postMatches[0].id -ne $expectedAssetId"
    post_asize = "[int64]$postMatches[0].size -ne [int64]$localAssets[$expectedAsset].Length"
    ambiguous = "$authoritativeCommercialReleaseBeforeCompensation.draft -ne $true -and $authoritativeCommercialReleaseBeforeCompensation.draft -ne $false"
    safe = "\n".join([
        exact_uri,
        ...
        f"  if ({pre_id}) {{ throw 'release-id' }}",
        f"  if (-not [string]::Equals({pre_url}, [StringComparison]::Ordinal)) {{ throw 'url' }}",
        ...
        f"  if (-not [string]::Equals({pre_name}, [StringComparison]::Ordinal)) {{ throw 'name' }}",
        f"  if ({pre_body}, [StringComparison]::Ordinal) -lt 0) {{ throw 'body' }}",
        f"  if ({pre_pre}) {{ throw 'prerelease' }}",
        ...
        f"  if ({pre_aid} -or {pre_asize}) {{ throw 'asset' }}",
        f"  if ({ambiguous}) {{ throw 'state' }}",
        "  if ($authoritativeCommercialReleaseBeforeCompensation.draft -eq $true) {",
        "    Write-Host 'already safe'",
        "  } else {",
        "    $commercialCompensationBody = @{ draft = $true } | ConvertTo-Json",
        ...
        f"  if ({post_id}) {{ throw 'release-id2' }}",
        f"  if (-not [string]::Equals({post_url}, [StringComparison]::Ordinal)) {{ throw 'url2' }}",
        "  if ($authoritativeCommercialReleaseAfterCompensation.draft -ne $true) { throw 'public' }",
        f"  if ({post_pre}) {{ throw 'prerelease2' }}",
        ...
        f"  if (-not [string]::Equals({post_name}, [StringComparison]::Ordinal)) {{ throw 'name2' }}",
        f"  if ({post_body}, [StringComparison]::Ordinal) -lt 0) {{ throw 'body2' }}",
        ...
        f"  if ({post_aid} -or {post_asize}) {{ throw 'asset2' }}",
    ...
    if contract_errors(safe):
        errors.append("guard rejected intended exact prerelease-preserving compensation contract")
    mutations = {
        "force prerelease true": safe.replace("@{ draft = $true }", "@{ draft = $true; prerelease = $true }", 1),
        "drop pre prerelease proof": safe.replace(f"  if ({pre_pre}) {{ throw 'prerelease' }}\n", "", 1),
        "drop post prerelease proof": safe.replace(f"  if ({post_pre}) {{ throw 'prerelease2' }}\n", "", 1),
        ...
        "drop pre release ID proof": safe.replace(f"  if ({pre_id}) {{ throw 'release-id' }}\n", "", 1),
        "drop pre release URL proof": safe.replace(f"  if (-not [string]::Equals({pre_url}, [StringComparison]::Ordinal)) {{ throw 'url' }}\n", "", 1),
        "drop pre release name proof": safe.replace(f"  if (-not [string]::Equals({pre_name}, [StringComparison]::Ordinal)) {{ throw 'name' }}\n", "", 1),
        "drop pre transaction proof": safe.replace(f"  if ({pre_body}, [StringComparison]::Ordinal) -lt 0) {{ throw 'body' }}\n", "", 1),
        "drop pre asset ID proof": safe.replace(pre_aid, "$false", 1),
        "drop pre asset size proof": safe.replace(pre_asize, "$false", 1),
        ...
        "drop post release ID proof": safe.replace(f"  if ({post_id}) {{ throw 'release-id2' }}\n", "", 1),
        "drop post release URL proof": safe.replace(f"  if (-not [string]::Equals({post_url}, [StringComparison]::Ordinal)) {{ throw 'url2' }}\n", "", 1),
        "drop post release name proof": safe.replace(f"  if (-not [string]::Equals({post_name}, [StringComparison]::Ordinal)) {{ throw 'name2' }}\n", "", 1),
        "drop post transaction proof": safe.replace(f"  if ({post_body}, [StringComparison]::Ordinal) -lt 0) {{ throw 'body2' }}\n", "", 1),
        "drop post asset ID proof": safe.replace(post_aid, "$false", 1),
        "drop post asset size proof": safe.replace(post_asize, "$false", 1),
        "drop ambiguous state rejection": safe.replace(f"  if ({ambiguous}) {{ throw 'state' }}\n", "", 1),
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("PASS: manual V25 post-publish compensation is exact-release/transaction/asset bound, conditionally re-drafted, prerelease-preserving, and authoritatively proved non-public before terminal failure.")
    return 0
