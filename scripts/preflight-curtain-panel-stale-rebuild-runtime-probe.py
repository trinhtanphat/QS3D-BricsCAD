...
if COMMAND.is_file():
    ...
    if text.count('VerifySemanticStale(document, project, state, expectedLiveGeometryStale: false);') != 3:
        errors.append("Curtain-panel P05 legacy grid/depth/height stages must preserve the CAD-only live fingerprint")
    if text.count('VerifySemanticStale(document, project, state, expectedLiveGeometryStale: true);') != 1:
        errors.append("Curtain-panel P05 linked-opening stage must change the live geometry fingerprint")
    for forbidden in (
