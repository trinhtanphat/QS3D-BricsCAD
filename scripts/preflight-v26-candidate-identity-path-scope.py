...
# direct JSON here; insert the JSON unicode escape after the raw template is built
# so Python/payload escaping cannot accidentally turn one JSON backslash into two.
...
# Convert Python-source quote escapes into literal PowerShell JSON quotes, then
# insert exactly one JSON backslash without relying on nested string escaping.
probe = probe.replace('\\"', '"')
