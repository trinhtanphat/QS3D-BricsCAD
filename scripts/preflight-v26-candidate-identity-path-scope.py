...
# PowerShell single-quoted strings preserve JSON double quotes literally. Keep
# ordinary JSON quotes literal in this raw Python template and insert only the
# JSON unicode escape after construction, avoiding every nested quote-escape layer.
probe = helper_block + r'''
$cases = @(
    @{ Json='{"product":"QS3D"}'; Name='product'; Expected=1 },
    @{ Json='{"product":"QS3D","PRODUCT":"evil"}'; Name='product'; Expected=2 },
    @{ Json='{"product":"QS3D","pro__JSON_UNICODE_ESCAPE__uct":"evil"}'; Name='product'; Expected=2 },
...
# The source template above uses a single Python escaping layer for quotes.
# Normalize that layer deterministically, then insert exactly one JSON backslash.
probe = probe.replace(chr(92) + '"', '"')
probe = probe.replace('__JSON_UNICODE_ESCAPE__', chr(92) + 'u0064')
