...
if not path.is_file():
    ...
    for needle in required:
        if needle not in text:
            errors.append("Opening boolean atomicity guard missing: " + needle)
    ...
    if min(semantic, touch, commit, committed) < 0 or not (semantic < touch < commit < committed):
        errors.append("Opening boolean semantic/touch/CAD commit ordering is not rollback-safe")
    ...
    if "document.Editor.Regen();" in post_commit and "TryRegen(document);" not in post_commit:
        errors.append("Opening boolean still performs fatal Editor.Regen after native commit")
...
print("PASS: physical opening boolean metadata/audit/touch are applied before native commit with ProjectState rollback, and post-commit viewport regen is best-effort.")
