...
if "TryReadOpenAiRuntimeApiKey(out verified)" not in save_block:
    fail("Runtime API key save must read the Credential Manager value back before reporting success")
if "string.Equals(verified, secret, StringComparison.Ordinal)" not in save_block:
    fail("Runtime API key save must verify the exact normalized value after Credential Manager write")
if save_block.index("TryReadOpenAiRuntimeApiKey(out verified)") > save_block.index('Environment.SetEnvironmentVariable("CONTROL_PLANE_API_KEY"'):
