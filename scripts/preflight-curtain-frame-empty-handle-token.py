...
if not SOURCE.is_file():
    ...
    for token in required:
        if token not in text:
            errors.append("missing curtain-frame empty-token contract token: " + token)
...
print("PASS: curtain-frame health preserves delimiter-empty tokens so malformed generated handle lists fail visible.")
