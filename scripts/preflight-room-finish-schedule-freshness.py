...
if not WINDOW.is_file():
    ...
    for token in required:
        if token not in text:
            errors.append("HT_PHÒNG schedule missing detached freshness token: " + token)

    build_start = text.find("private IReadOnlyList<RoomFinishScheduleRow> BuildCurrentRows")
    ...
    for forbidden in (
        ...
        if forbidden in body:
            errors.append("HT_PHÒNG read-only refresh must not mutate/bind live project state: " + forbidden)
    ...
    build = body.find("RoomFinishScheduleBuilder.Build(snapshot)")
    if min(lookup, snapshot, regen, build) < 0 or not lookup < snapshot < regen < build:
        errors.append("HT_PHÒNG refresh order must be read-only lookup -> detached copy -> regenerate -> schedule build")
    ...
    exporter = export_body.find("RoomFinishXlsxExporter.Export(dialog.FileName, current)")
    if min(dialog, current, exporter) < 0 or not dialog < current < exporter:
        errors.append("HT_PHÒNG export must confirm Save before detached fresh-row build and export")
    if "RoomFinishXlsxExporter.Export(dialog.FileName, _rows)" in export_body:
        errors.append("HT_PHÒNG export must not export stale cached _rows")
...
print("[PASS] HT_PHÒNG modeless refresh/export regenerates only a detached read-only snapshot and never mutates/binds live project state")
