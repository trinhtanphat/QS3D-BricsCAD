...
errors = []
for path, start_token, end_token, build_token, label, requires_regen in CASES:
    if not path.is_file():
    ...
    if all(pos >= 0 for pos in positions.values()):
        ...
        if ordered != sorted(ordered):
            expected = "read-only probe -> ProjectId snapshot -> point prompt -> cancel guard -> canonical bind -> freshness check"
            expected += " -> regeneration" if requires_regen else ""
            expected += " -> native build"
            errors.append(label + ": expected " + expected + " ordering")
    ...
    if cancel >= 0 and bind >= 0 and bind < cancel:
        errors.append(label + ": canonical project bind occurs before cancelled placement can return")
    if bind >= 0 and build >= 0 and bind > build:
        errors.append(label + ": native build occurs before canonical project binding")
...
print("PASS: all six native Table placement commands probe existing state read-only, return on cancelled placement before canonical binding, verify the same ProjectId after binding, and only then regenerate/build as required by each workflow.")
