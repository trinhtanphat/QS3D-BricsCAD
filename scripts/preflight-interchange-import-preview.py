...
if PREVIEW.is_file():
    ...
    if text.find("var validation = ProjectInterchangeJsonValidator.Validate(json);") > text.find("var manifest = ParseValidatedManifest(json);"):
        errors.append("import preview must validate the snapshot before parsing its collision manifest")
    ...
    forbidden = (
        ...
        "targetProject.ActiveZoneId =",
        "targetProject.ActiveFloorId =",
        "targetProject.Touch(",
...
if VALIDATOR.is_file():
    ...
    for token in (
        ...
        if token not in text:
            errors.append("interchange validator lost a preview prerequisite: " + token)
...
if DOC.is_file():
    ...
    for token in (
        ...
        "validator PASS is still **not import permission**",
