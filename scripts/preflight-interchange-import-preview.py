...
for path in (PREVIEW, VALIDATOR, SMOKE, REG, DOC):
    if not path.is_file(): errors.append("missing interchange import-preview contract file: " + str(path.relative_to(ROOT)))
...
if PREVIEW.is_file():
    ...
    for token in required:
        if token not in text: errors.append("ProjectInterchangeImportPreview.cs missing preview/fail-closed token: " + token)
    ...
    for token in (
        ...
        if token in text: errors.append("read-only import preview contains forbidden mutation/ownership authority token: " + token)

if VALIDATOR.is_file():
    text = VALIDATOR.read_text(encoding="utf-8")
    for token in ("ProjectInterchangeJsonExporter.FormatName", "ProjectInterchangeJsonExporter.FormatVersion", "sourceRefScope must be exactly 'drawing-local'"):
        if token not in text: errors.append("interchange validator lost a preview prerequisite: " + token)

if SMOKE.is_file():
    text = SMOKE.read_text(encoding="utf-8")
    for token in ("NewIdentitiesArePreviewedWithoutMutation", "ExistingSameCategoryRequiresPolicy", "CategoryMismatchIsIncompatible", "InvalidSnapshotStopsBeforeCollisionPlanning", "FingerprintRelationIsDescriptive", "AmbiguousTargetIdsFailClosed"):
        if token not in text: errors.append("ProjectInterchangeImportPreviewSmoke.cs missing regression scenario: " + token)
...
if DOC.is_file():
    ...
    for token in ("read-only collision/provenance preview", "not import permission", "sourceRefScope = drawing-local", "does not reconstruct any of them", "Do **not** add `QS3DINTERCHANGEIMPORT`", "REMOTE_DONE as read-only import planning only"):
        if token not in text: errors.append("INTERCHANGE-IMPORT-PREVIEW.md missing safety/import boundary: " + token)
...
if errors:
    ...
    sys.exit(1)
print("PASS: Semantic Snapshot v1 collision preview is validation-first, bounded and target-read-only; merge/import, source-handle rebinding and native ownership reconstruction remain explicitly unimplemented.")
