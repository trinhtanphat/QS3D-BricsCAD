...
if not SOURCE.is_file():
    ...
    if start < 0 or end < 0:
        ...
        project = body.find("ProjectContextCoordinator.GetOrCreate(_document)")
        ...
        if min(confirm, project, regenerate, build, bind, export) < 0:
            errors.append("BBS review export missing save/project/regenerate/build/rebind/export contract token")
        elif not confirm < project < regenerate < build < bind < export:
            errors.append("BBS review XLSX must confirm Save before current-project regeneration, fresh build, UI rebind, and export")
...
print("PASS: BBS review export confirms Save, rebuilds from the current project, refreshes visible totals, and exports fresh rows.")
