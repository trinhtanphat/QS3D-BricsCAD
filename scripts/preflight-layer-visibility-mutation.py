...
def validate(text: str) -> list[str]:
    ...
    if text.count("count++;") != 2:
        errors.append("matched-layer return count must remain independent from mutation count")
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print("ERROR:", error)
        return 1
    expect_mutation_failure(text, "if (layer.IsOff == targetIsOff && !requiresThaw) continue;", "if (false) continue;", "visibility no-op detection")
    expect_mutation_failure(text, "if (layer.IsLocked == locked) continue;", "if (false) continue;", "lock no-op detection")
    expect_mutation_failure(text, "if (requiresThaw) layer.IsFrozen = false;", "// thaw removed", "show-thaws-frozen-layer behavior")
    expect_mutation_failure(text, "if (changed > 0) document.Editor.Regen();", "document.Editor.Regen();", "mutation-gated regeneration")
    print("PASS: LayerVisibilityService preserves matched-count semantics while skipping redundant mutations and regen.")
    return 0
