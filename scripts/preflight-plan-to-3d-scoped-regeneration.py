...
if not errors:
    ...
    if not convert:
        ...
        for token in required:
            if token not in convert:
                errors.append("PlanTo3D scoped regeneration contract missing: " + token)
        ...
        build_at = min(
            x for x in (
                convert.find("WallSolidBuilder.BuildSelectedLineWalls"),
                convert.find("PolylineWallSolidBuilder.BuildSelected"),
            ) if x >= 0
        ) if ("WallSolidBuilder.BuildSelectedLineWalls" in convert and "PolylineWallSolidBuilder.BuildSelected" in convert) else -1
    ...
    if "public int RegenerateDirtySubset(ProjectState project, IEnumerable<string> elementIds)" not in engine:
        errors.append("Core RegenerationEngine no longer exposes the targeted regeneration API required by PlanTo3D")
...
print("PASS: QS3DCONVERT2D/QS3DPLAN2WALLS regenerate only each newly captured wall before native build; unrelated dirty semantic elements remain outside the conversion side effect.")
