...
try {
    # Pin the already-existing parent before creating the private extraction root. The handle denies
    # write/delete sharing, so the parent cannot be renamed, deleted, or converted to a junction while
    # the destination root is created and admitted.
    ...
    foreach ($record in $records) {
        if ($record.IsDirectory) {
            Ensure-HeldSafeDirectory -Path $record.Target -BoundaryRoot $destinationFull -Holds $directoryHolds -HoldOrder $holdOrder
            continue
        }
        ...
        if (-not $directoryHolds.ContainsKey($parent)) {
            throw "Commercial archive extraction parent generation was not held before file creation: $parent"
        }
        ...
        try {
            # The parent directory generation is held with FILE_SHARE_READ only for the entire write.
            # CreateNew therefore cannot be redirected by replacing/renaming the admitted parent.
    ...
    for ($i = $holdOrder.Count - 1; $i -ge 0; $i--) {
        try { $holdOrder[$i].Handle.Dispose() } catch { }
    }
