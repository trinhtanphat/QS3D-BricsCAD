...
if RUNNER.is_file():
    ...
    scripted_open = text.find('"_.OPEN", (\'"\' + $DrawingCopy + \'"\')', guard_before_launch)
    ...
    if min(original_attributes, hash_before, backup, guard_set, guard_before_launch, scripted_open, netload, automation, profile, batch, launch, validation, finalizer, stop, guard_through_exit, drawing_hash, restore, metadata) < 0 or not (
        original_attributes < hash_before < backup < guard_set < guard_before_launch < scripted_open < netload < automation < profile < batch < launch < validation < finalizer < stop < guard_through_exit < drawing_hash < restore < metadata
    ):
