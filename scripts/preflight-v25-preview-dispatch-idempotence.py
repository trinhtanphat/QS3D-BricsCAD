...
def main() -> int:
    ...
    required = (
        ...
        'prior_dispatch_status="$(jq -er \' .status | strings\' <<< "${prior_dispatch_run_json}")"'.replace("\\' ", "\\'"),
        'prior_dispatch_conclusion="$(jq -er \' .conclusion // "" | strings\' <<< "${prior_dispatch_run_json}")"'.replace("\\' ", "\\'"),
