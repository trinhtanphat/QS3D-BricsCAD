...
checks = {
    "src/QS3D.Core/Domain/ProjectFamilyService.cs": [
        "MaxPropertyValueLength = 1000", "SetProperty(ProjectState project", "InheritedInstancesUpdated", "OverridesPreserved",
    ],
    "src/QS3D.Core/Reporting/CurtainWallSchedule.cs": [
    ...
    "src/QS3D.BricsCAD.V25/UI/CurtainWallWindow.xaml.cs": [
        ...
        '"CurtainFrameDepthM"', "FrameDepthBox.Text", "ApplyFamilyValue",
        "ProjectFamilyService.SetProperty(project, family.Id, key, next)",
        "update.InheritedInstancesUpdated", "update.OverridesPreserved",
        "RegenerationEngine", "CurtainPanelCount", "CurtainNetGlassAreaM2",
...
if curtain_ui.is_file():
    curtain_text = curtain_ui.read_text(encoding="utf-8")
    if ".Sum(" in curtain_text:
        errors.append("Curtain Hub summary must not use unchecked LINQ Sum")
    if "family.Properties[key] = next" in curtain_text:
        errors.append("Curtain Hub must not bypass ProjectFamilyService.SetProperty with direct Family property writes")
    save_start = curtain_text.find("private void OnSaveClick")
    save_end = curtain_text.find("private void OnRecalculateClick", save_start)
    if save_start < 0 or save_end < 0:
        errors.append("Curtain Hub Save block could not be located")
    else:
        save_block = curtain_text[save_start:save_end]
        if "project.Touch();" in save_block:
            errors.append("Curtain Hub Save must not unconditionally Touch the project; canonical Family service owns change tracking")
        if "ProjectFamilyService.SetProperty" not in curtain_text:
            errors.append("Curtain Hub Save must route Family values through ProjectFamilyService.SetProperty")
...
print("PASS: Curtain Hub uses canonical bounded Family property mutation without no-op Touch, while source-DWG binding, summary arithmetic, exact-set native frame ownership, frame health, schedule grouping and XLSX export remain guarded.")
