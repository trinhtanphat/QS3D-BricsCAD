...
def test_run_gate_failure_modes(module, root):
    ...
    try:
        ...
        require(module.run_gate(gate, {}) == ("scripts/preflight-child.py", "exit=7"), "nonzero exit was not propagated with a canonical path")

        def timeout(*args, **kwargs):
            raise subprocess.TimeoutExpired(args[0], module.CHILD_TIMEOUT_SECONDS)

        module.subprocess.run = timeout
        require(module.run_gate(gate, {}) == ("scripts/preflight-child.py", "timeout"), "timeout was not propagated with a canonical path")

        def launch(*args, **kwargs):
            raise OSError("fixture launch failure")

        module.subprocess.run = launch
        require(module.run_gate(gate, {}) == ("scripts/preflight-child.py", "launch"), "launch failure was not propagated with a canonical path")
    finally:
