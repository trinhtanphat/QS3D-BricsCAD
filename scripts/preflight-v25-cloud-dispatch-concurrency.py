...
def validate(text: str) -> list[str]:
    errors: list[str] = []
    concurrency_start = text.find("\nconcurrency:\n")
    jobs_start = text.find("\njobs:\n", concurrency_start + 1)
    if concurrency_start < 0 or jobs_start < 0 or jobs_start <= concurrency_start:
        errors.append("dispatcher requires one top-level concurrency block before jobs")
        return errors

    concurrency = text[concurrency_start:jobs_start]
    group = concurrency.count("group: qs3d-v25-cloud-main-integration")
    cancel_false = concurrency.count("cancel-in-progress: false")
    queue_max = concurrency.count("queue: max")
    ...
    if "cancel-in-progress: true" in concurrency:
        errors.append("dispatcher top-level concurrency must not preempt an active transaction owner")
