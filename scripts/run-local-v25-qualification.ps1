...
try {
    ...
    try {
        ...
        Invoke-QualificationStep "Manual-only CI policy" {
            Invoke-ExternalChecked $pythonExe @("scripts/preflight-ci-manual-only.py")
        }
        Invoke-QualificationStep "Generic source preflight" {
            Invoke-ExternalChecked $pythonExe @("scripts/preflight.py")
        }
        Invoke-QualificationStep "Aggregate feature preflights" {
            Invoke-ExternalChecked $pythonExe @("scripts/preflight-all.py")
        }
