...
class PreviewDownloadGuardMutationTests(unittest.TestCase):
    ...
    def test_transport_integrity_and_staging_mutations_are_rejected(self):
        cases = (
            ...
            (DOWNLOADER, "await DownloadBoundedAsync(release.PackageUri, partialPath, MaxPackageBytes, progress)", "await DownloadBoundedAsync(release.PackageUri, partialPath, long.MaxValue, progress)"),
            ...
            (DOWNLOADER, "await CopyBoundedAsync(source, buffer, MaxChecksumBytes, null, 0)", "await source.CopyToAsync(buffer)"),
            (DOWNLOADER, "await CopyBoundedAsync(source, target, maxBytes, progress, totalBytes)", "await source.CopyToAsync(target)"),
