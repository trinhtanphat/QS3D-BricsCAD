...
FIXTURE_FILES = (PREFLIGHT, CLIENT, DOWNLOADER, WINDOW, START_CENTER)
...
class PreviewDownloadGuardMutationTests(unittest.TestCase):
    ...
    def test_streamed_byte_bound_cannot_be_removed(self):
        self.assert_rejected(
            DOWNLOADER,
            "if (total > maxBytes)",
            "if (false)",
            "missing required preview-download contract: if (total > maxBytes)",
        )

    def test_content_length_bound_cannot_be_removed(self):
        self.assert_rejected(
            DOWNLOADER,
            "if (response.ContentLength > maxBytes)",
            "if (false)",
            "missing required preview-download contract: if (response.ContentLength > maxBytes)",
        )
    ...
    def test_final_response_uri_must_remain_validated(self):
        self.assert_rejected(
            DOWNLOADER,
            "EnsureAllowedUri(response.ResponseUri);",
            "// final response URI validation removed",
            "missing required preview-download contract: EnsureAllowedUri(response.ResponseUri);",
        )
    ...
    def test_uri_user_info_rejection_cannot_be_removed(self):
        self.assert_rejected(
            DOWNLOADER,
            "if (!string.IsNullOrEmpty(uri.UserInfo))",
            "if (false)",
            "missing required preview-download contract: if (!string.IsNullOrEmpty(uri.UserInfo))",
        )
    ...
    def test_conin_reserved_name_guard_cannot_be_removed(self):
        self.assert_rejected(
            DOWNLOADER,
            'string.Equals(stem, "CONIN$", StringComparison.OrdinalIgnoreCase)',
            "false",
            'missing required preview-download contract: string.Equals(stem, "CONIN$", StringComparison.OrdinalIgnoreCase)',
        )

    def test_conout_reserved_name_guard_cannot_be_removed(self):
        self.assert_rejected(
            DOWNLOADER,
            'string.Equals(stem, "CONOUT$", StringComparison.OrdinalIgnoreCase)',
            "false",
            'missing required preview-download contract: string.Equals(stem, "CONOUT$", StringComparison.OrdinalIgnoreCase)',
        )
