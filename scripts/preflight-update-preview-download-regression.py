...
class PreviewDownloadGuardMutationTests(unittest.TestCase):
    ...
    def test_cached_size_check_must_precede_hashing(self):
        temporary, fixture = self.make_fixture()
        self.addCleanup(temporary.cleanup)
        path = fixture / DOWNLOADER
        text = path.read_text(encoding="utf-8")
        size_gate = "if (existingLength <= MaxPackageBytes)"
        hash_line = "var existingSha256 = ComputeSha256(packagePath);"
        self.assertLess(text.index(size_gate), text.index(hash_line))
        mutated = text.replace(size_gate, "if (true)", 1)
        path.write_text(mutated, encoding="utf-8")
        result = self.run_guard(fixture)
        self.assertNotEqual(0, result.returncode, result.stdout)
        self.assertIn(size_gate, result.stdout)
