...
for filename in FILES:
    ...
    if not path.is_file():
        errors.append("missing health command file: " + filename)
        continue
...
for filename in LOCATE_FILES:
    ...
    if "currentProject.FindElement(issue.ElementId)" not in callback:
        errors.append(filename + ": locate callback must re-resolve ElementId from current project")
...
print("PASS: targeted health commands are read-only, and modeless Locate callbacks re-resolve current project/ElementId instead of captured semantic state.")
