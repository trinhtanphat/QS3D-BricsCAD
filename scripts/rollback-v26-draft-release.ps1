...
function Assert-DraftDeleteCommittedAfterError {
    ...
    if ($null -eq $remainingRelease -or [long]$remainingRelease.id -ne $ReleaseId) {
        throw "V26 draft DELETE reconciliation returned a mismatched release identity; refusing to assume deletion."
    }
    if (-not [string]::Equals([string]$remainingRelease.url, $ReleaseUri, [StringComparison]::Ordinal)) {
        throw "V26 draft DELETE reconciliation returned a mismatched repository identity; refusing to assume deletion."
    }
    if ($remainingRelease.draft -ne $true) {
        throw "V26 draft DELETE reconciliation found release $ReleaseId but it is no longer an owned draft; refusing to assume deletion."
    }
    if (-not [string]::Equals([string]$remainingRelease.tag_name, $ReleaseTag, [StringComparison]::Ordinal)) {
        throw "V26 draft DELETE reconciliation found a mismatched release tag; refusing to assume deletion."
    }
...
function Assert-TagDeleteCommittedAfterError {
    ...
    if ($null -eq $remainingTag -or -not [string]::Equals([string]$remainingTag.ref, $expectedRef, [StringComparison]::Ordinal)) {
        throw "V26 tag DELETE reconciliation returned a mismatched ref identity; refusing to assume deletion."
    }
    if ($null -eq $remainingTag.object -or -not [string]::Equals([string]$remainingTag.object.sha, $WorkflowSha, [StringComparison]::OrdinalIgnoreCase)) {
        throw "V26 tag DELETE reconciliation found a moved or ambiguous tag identity; refusing to assume deletion."
    }
