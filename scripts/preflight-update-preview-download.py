...
def main():
    ...
    print(
        ...
        "validates every bounded HTTPS GitHub redirect hop and final response URI with bounded network timeouts, rejects user-info, "
        "verifies SHA-256 before retaining the ZIP, escapes Windows reserved release-tag cache segments, bounds the readable cache "
        "prefix, appends a SHA-256 identity of the exact release tag to prevent case/sanitization cache collisions, stages under "
        "LocalApplicationData, exposes the Update Center directly from Start Center without command dispatch, and only reveals unsigned "
