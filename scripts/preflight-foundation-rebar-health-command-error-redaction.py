...
if not SOURCE.is_file():
    ...
    for token in ('catch (System.Exception ex)', 'ex.Message', 'QS3DFOUNDATIONREBARHEALTH lỗi: " +'):
        if token in text:
            errors.append("Foundation Rebar Health command must not reflect exception detail: " + token)
...
print("PASS: QS3DFOUNDATIONREBARHEALTH preserves live-handle/modeless locate behavior while top-level exception details remain redacted from Palette and Editor output.")
