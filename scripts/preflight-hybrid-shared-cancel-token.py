...
def validate(text: str) -> list[str]:
    ...
    if not re.search(r"(?m)^\s{2}actions:\s*write\s*$", permissions_body):
        errors.append("workflow must grant top-level actions: write for same-repo cancellation")

    refresh = re.search(
        r"(?ms)^  refresh-branches:\s*\n(?P<body>.*)\Z", text
    )
...
def self_test(good: str) -> list[str]:
    ...
    mutants = {
        "actions-read": good.replace("  actions: write", "  actions: read", 1),
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f" - {error}")
        return 1
    print("PASS: Hybrid stale-Shared cancellation is bound to github.token with actions: write and remains fail-closed.")
    return 0
