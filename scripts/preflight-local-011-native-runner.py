...
if not INBOX.is_file(): fail("missing canonical local-agent inbox")
r=RUNNER.read_text(encoding="utf-8"); d=RUNBOOK.read_text(encoding="utf-8"); m=MATRIX_DOC.read_text(encoding="utf-8"); i=INBOX.read_text(encoding="utf-8")
for x in SHAS:
...
inbox_match = re.search(r"(?ms)^## LOCAL-011\b(?P<section>.*?)(?=^## LOCAL-012\b)", i)
if not inbox_match: fail("canonical inbox LOCAL-011 section is missing")
inbox_section = inbox_match.group("section").lower()
for token in ("scripts/run-local-v25-local-011.ps1", "remote_done", "pending_local", "do_not_retry_remote", "pull/sync", "one command"):
    if token not in inbox_section: fail(f"canonical inbox LOCAL-011 handoff missing token: {token}")
