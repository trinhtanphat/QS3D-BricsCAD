...
expect("exact reservation is reusable", reservation_state([f"{RESERVATION_PREFIX} ordinal=10303 source_sha={source} run_id=1"], 10303, source), (True, None, False))
expect("single earlier owner is preserved rather than overwritten", reservation_state([f"{RESERVATION_PREFIX} ordinal=10303 source_sha={other} run_id=2"], 10303, source), (False, other, False))
expect("multiple earlier owners are ambiguous", reservation_state([f"{RESERVATION_PREFIX} ordinal=10303 source_sha={other} run_id=2", f"{RESERVATION_PREFIX} ordinal=10303 source_sha={third} run_id=3"], 10303, source), (False, other, True))
expect("exact plus earlier owner remains visible for fail-closed reconciliation", reservation_state([f"{RESERVATION_PREFIX} ordinal=10303 source_sha={source} run_id=1", f"{RESERVATION_PREFIX} ordinal=10303 source_sha={other} run_id=2"], 10303, source), (True, other, False))
expect("other ordinals and human notes are ignored", reservation_state(["human note", f"{RESERVATION_PREFIX} ordinal=10302 source_sha={'d' * 40} run_id=3"], 10303, source), (False, None, False))
...
if not WORKFLOW.is_file():
    ...
    for forbidden in (
        ...
        if forbidden in text:
            errors.append("dispatcher must not use legacy/fail-open reservation allocator token: " + forbidden)

    capture_index = text.find('reservation_rows="$(', text.find("committed_preview_ordinal="))
    status_index = text.find("reservation_query_status=$?", capture_index)
    failure_index = text.find("if (( reservation_query_status != 0 )); then", status_index)
    loop_index = text.find("while IFS= read -r reservation; do", failure_index)
    loop_end = text.find('done <<< "${reservation_rows}"', loop_index)
    multi_owner_index = text.find("if (( reservation_owner_conflict != 0 || dispatch_fence_owner_conflict != 0 )); then", loop_end)
    prior_owner_index = text.find('if [[ -n "${reservation_owner_source}" || -n "${dispatch_fence_owner_source}" ]]; then', multi_owner_index)
    ...
    indexes = (capture_index, status_index, failure_index, loop_index, loop_end, multi_owner_index, prior_owner_index, reserve_index, dispatch_index)
    if min(indexes) < 0 or list(indexes) != sorted(indexes):
        errors.append("dispatcher must materialize the complete ledger fail-closed, reject ambiguous ownership, reconcile a legitimate prior owner before side effects, then reserve before downstream dispatch")
...
print("PASS: automatic V25 dispatch fail-closes ledger enumeration, binds reservation identity to committed ProductVersion, preserves one immutable legitimate prior owner neutrally, rejects ambiguous ownership, records a new exact reservation before dispatch, and keeps main contents read-only.")
