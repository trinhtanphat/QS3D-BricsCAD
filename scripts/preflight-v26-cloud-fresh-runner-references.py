...
def main() -> int:
    ...
    mutations = (
        text.replace("needs.v26-reference-primary.outputs.ready != 'true'", "false", 1),
        text.replace("qs3d-v26-hostrefs-v1-${{ github.run_id }}-fallback", "qs3d-v26-hostrefs-v1-static-fallback", 1),
        text.replace("  v26-reference-fallback:\n", "  v26-reference-fallback:\n    continue-on-error: true\n", 1),
        text.replace("-ExtractReferences | Select-Object -Last 1", "| Select-Object -Last 1", 1),
    )
    for index, mutated in enumerate(mutations, start=1):
        if not validate(mutated):
            raise SystemExit(f"mutation probe {index} was not rejected")

    print("PASS: V26 cloud reference extraction recovers once on a distinct fresh runner and qualify consumes only a run-bound validated handoff.")
    return 0
