...
def main() -> int:
    ...
    identity_cases = (
        (
            ...
            {"assemblyVersion": "0.1.0.0", "productVersion": "0.1.0-preview.2"},
            True,
            "canonical plugin/Core identity",
        ),
        (
            "0.1.0.0",
            "0.1.0-preview.2",
        ...
        (
            ...
            "0.1.0-preview.2",
            {"assemblyVersion": "0.1.0.0", "productVersion": "0.1.0-preview.2"},
            {"assemblyVersion": "0.1.0.0", "productVersion": "0.1.0-preview.3"},
            False,
            "Core productVersion mismatch",
        ),
        (
            "0.1.0.0",
            "0.1.0-preview.2",
            ...
            {"assemblyVersion": "0.1.0.0", "productVersion": "0.1.0-preview.2"},
            False,
            "plugin productVersion case mismatch",
        ),
    )
    ...
    require(
        extension_guard < staging_guard < zip_alias_guard < signer_check < managed_loop < product_compare < zip_binding < zip_hash < manifest_create < should_process < output_write,
        "manifest output isolation and both managed identities must precede ZIP/hash verification and manifest mutation",
    )

    print(
        "PASS: update manifest generation requires external JSON output and exact metadata identity across both signed managed DLLs before ZIP/staging verification, hash derivation and manifest write."
    )
    return 0
