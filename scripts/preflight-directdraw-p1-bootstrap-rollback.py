...
def main():
    ...
    execute = method_slice(
        ...
        "private static IReadOnlyList<Point3d>? AcquireFixedPath(")

    require_order(
        execute,
        "P1 Direct Draw bootstrap ownership",
    ...
    require_order(
        execute,
        "P1 Direct Draw failed bootstrap cleanup",
    ...
    require(execute, "new Build3DCommands().Build3D();", "nested Build3D preservation")
    require(execute, "if (ownershipDiscoveryError != null || cleanupError != null || restoreError != null)", "rollback error aggregation")
    require(execute, "new AggregateException(errors)", "rollback aggregate preservation")

    print("PASS: Direct Draw P1 forgets only failed projectless bootstraps after CAD/semantic rollback.")
    return 0
