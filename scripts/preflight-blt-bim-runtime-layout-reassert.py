...
for token in (
    ...
    if token not in palette:
        errors.append("PaletteCoordinator four-palette BIM/success contract missing: " + token)
...
print("PASS: BIM activation reasserts Workspace + dedicated QS3D Properties + Management + Quantity through a bounded BricsCAD docking settle window, consumes retries only after successful reassert, ordinary ShowWorkspace restores the same real editor in-place, manual palette close is not selection-driven, and native modelspace stays host-owned.")
