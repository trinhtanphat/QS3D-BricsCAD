...
# Owner-facing QS3D activation restores the BIM arrangement while preserving the BricsCAD host.
# The latest owner reference keeps Properties embedded below Family and leaves Quantity Insight
# on-demand; the dedicated Properties PaletteSet remains an optional capability rather than default.
...
require(palette, "_workspacePanel?.SetDedicatedPropertiesPaletteActive(false);", "embedded properties")
...
require(palette, "SetVisibility(workspace: true, right: true, quantityInsight: false);", "owner-reference BIM visibility")
require(palette, "private static readonly Guid PropertiesGuid", "optional dedicated properties palette")
...
forbid(palette, "SetVisibility(workspace: true, right: true, quantityInsight: true);", "default BIM quantity takeover")
...
print("PASS: owner-reference embedded BIM activation, exact simple closed-boundary Móng Bè, and selected-object quantity explanation source contracts")