...
if bq.is_file():
    ...
    for needle in ('EnsureActive("tính lại BQ")', 'EnsureActive("định vị BQ")'):
        if needle not in text:
...
bbs = windows["BBS"]
if bbs.is_file() and "EnsureActive();\n                _locate(row);" not in bbs.read_text(encoding="utf-8"):
    errors.append("BBS Locate must verify its source DWG before invoking the CAD callback")
...
print("PASS: Recognition/BQ/BBS/Revision/Health modeless CAD/project actions are bound to their source DWG.")
