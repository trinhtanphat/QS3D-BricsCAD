...
def validate(validator: str, packager: str, workflow: str) -> list[str]:
    ...
    for token in (
        ...
        "$metadataSource.ToLowerInvariant()",
...
mutations = {
    ...
    "validator ignores metadata source": (VALIDATOR.replace("$metadataSource = [string]$metadata.gitCommit", "$metadataSource = [string]$ExpectedSourceCommit", 1), PACKAGER, WORKFLOW),
    "validator normalizes metadata source": (VALIDATOR.replace("-not [string]::Equals($metadataSource, $metadataSource.Trim(), [StringComparison]::Ordinal)", "$false", 1), PACKAGER, WORKFLOW),
