...
if not SOURCE.is_file():
    ...
    auto_host = "hostId = AutoHostLinkCommands.LinkSingleOpening(document, project, createdElementId);"
    canonical_lookup = "string.Equals(x.Id, createdElementId, StringComparison.OrdinalIgnoreCase)"
    host_read = 'createdElement.Properties.TryGetValue("HostWallId", out var recordedHostId)'
    host_match = 'string.Equals(recordedHostId.Trim(), hostId, StringComparison.OrdinalIgnoreCase)'
    erase = "EraseSource(document, sourceId)"
    restore = "rollback.Restore(project)"

    required = (capture_id, auto_host, canonical_lookup, host_read, host_match, erase, restore)
    ...
    if min(id_pos, auto_pos, lookup_pos, host_pos) >= 0 and not (id_pos < auto_pos < lookup_pos < host_pos):
        errors.append("created element Id must be captured before exact AutoHost and canonical state must be re-resolved before HostWallId is read")

    if "new AutoHostLinkCommands().AutoLinkHosts()" in text:
        errors.append("Direct Draw must not re-enter broad AutoLinkHosts after exact single-opening ownership is known")
...
print("PASS: Door/Opening Direct Draw captures stable semantic identity, uses exact single-opening AutoHost on the authorized project, re-resolves canonical state, verifies HostWallId against the resolved host, and preserves outer CAD/project rollback ordering.")