...
def scan_tree(root: Path) -> list[str]:
    ...
    for path in root.rglob("*"):
        ...
        lowered_relative = relative.lower()
        if any(marker in lowered_relative for marker in FORBIDDEN_PATH_MARKERS):
            failures.append(f"external scheduler/orchestration alias must stay outside the QS3D source tree: {relative}")
            continue
        ...
        if is_external_orchestration(relative, text):
            failures.append(f"external controller/worker scheduler topology leaked into repository content: {relative}")
    return failures
