...
function Require-Qs3dFailureMarker {
    ...
    $allowedCodes = @(
        ...
        "OUTPUT_HOST_CONTAINMENT_REJECTED", "GENERATED_MUTATED_BY_NATIVE_MOVE",
...
foreach ($assemblyPath in @($PluginDll, $coreDll)) {
    ...
    if (-not $productVersion.EndsWith($expectedAssemblyRevision, [StringComparison]::OrdinalIgnoreCase)) {
        throw "LOCAL-004 P03 assembly was not built from the exact Git candidate SHA."
    }
}
...
Write-Host "QS3D BricsCAD V25 LOCAL-004 P03 native Beam dependent-output MOVE runtime PASS"
