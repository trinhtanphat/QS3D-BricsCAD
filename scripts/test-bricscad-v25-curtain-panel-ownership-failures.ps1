...
if (-not (Test-Path -LiteralPath $windowInteropPath -PathType Leaf)) { throw "Curtain P06 runner window interop helper is missing." }
. $windowInteropPath
...
function Read-Qs3dMarker {
    ...
    foreach ($line in Get-Content -LiteralPath $Path) {
        ...
        if ($separator -le 0) { throw "Malformed Curtain P06 marker line." }
        $key = $line.Substring(0, $separator).Trim()
        $value = $line.Substring($separator + 1).Trim()
        if ($marker.ContainsKey($key)) { throw "Duplicate Curtain P06 marker key." }
        $marker[$key] = $value
    }
    return $marker
}
...
function Require-Qs3dValue {
    ...
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P06 marker is missing a required key." }
    if (-not [string]::Equals([string]$Marker[$Key], $Expected, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Curtain P06 marker contains an unexpected required value."
    }
}
...
function Read-Qs3dAllowedValue {
    ...
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P06 marker is missing a diagnostic key." }
    $value = [string]$Marker[$Key]
    foreach ($candidate in $Allowed) {
        if ([string]::Equals($value, $candidate, [StringComparison]::Ordinal)) { return $value }
    }
    throw "Curtain P06 marker diagnostic token is not allowlisted."
}

function Read-PositiveMarkerInt {
    param([Parameter(Mandatory = $true)]$Marker, [Parameter(Mandatory = $true)][string]$Key)
    if (-not $Marker.ContainsKey($Key)) { throw "Curtain P06 marker is missing a positive-count key." }
    [int]$value = 0
    if (-not [int]::TryParse([string]$Marker[$Key], [Globalization.NumberStyles]::None, [Globalization.CultureInfo]::InvariantCulture, [ref]$value) -or $value -le 0) {
        throw "Curtain P06 marker count is not a positive invariant integer."
    }
    return $value
}
...
function Stop-Qs3dLaunchedProcess {
    ...
    if (-not $Process.HasExited) { throw "Launched BricsCAD Curtain P06 process did not exit." }
}
...
function Remove-Qs3dDrawingLocks {
    ...
    foreach ($path in $Paths) {
        ...
        if (Test-Path -LiteralPath $path) { throw "Curtain P06 drawing-lock cleanup failed." }
    }
}

if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) { throw "Curtain P06 runtime qualification requires Windows." }
if (-not [Environment]::UserInteractive) { throw "Curtain P06 runtime qualification requires an interactive Windows session." }
if (-not $ConfirmDisposableCopy) { throw "Pass -ConfirmDisposableCopy only for a disposable synthetic drawing copy." }
if ([string]::IsNullOrWhiteSpace($Profile)) { throw "Curtain P06 runtime qualification requires an initialized BricsCAD profile." }
...
if (-not [IO.Path]::GetFileName($DrawingCopy).EndsWith(".curtain-ownership-probe-copy.dwg", [StringComparison]::OrdinalIgnoreCase)) {
    throw "DrawingCopy must use the guarded '*.curtain-ownership-probe-copy.dwg' suffix."
}
...
foreach ($required in @($bricscadExe, $PluginDll, $coreDll, $DrawingCopy)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "A required Curtain P06 runtime input is missing." }
}
...
if ($gitStatus.Count -ne 0) { throw "Curtain P06 runtime qualification requires a clean exact-SHA worktree." }
...
if (@(Get-Qs3dExactBricsCadProcesses -ExpectedExecutable $bricscadExe).Count -gt 0) {
    throw "Close existing BricsCAD V25 processes before starting the isolated Curtain P06 runtime probe."
}
...
foreach ($forbiddenInput in @($projectSidecar, ($projectSidecar + ".bak"), $drawingBackup) + $drawingLocks) {
    if (Test-Path -LiteralPath $forbiddenInput) { throw "The disposable Curtain P06 copy has a pre-existing sidecar or backup." }
}
...
$resultPath = Join-Path $ArtifactDir "curtain-panel-ownership-failure-runtime-result.txt"
$scriptPath = Join-Path $ArtifactDir "curtain-panel-ownership-failure-runtime.scr"
$metadataPath = Join-Path $ArtifactDir "curtain-panel-ownership-failure-runtime-metadata.json"
foreach ($output in @($resultPath, $scriptPath, $metadataPath)) {
    if (Test-Path -LiteralPath $output) { throw "Curtain P06 runtime output must not already exist." }
}
...
$oldResult = [Environment]::GetEnvironmentVariable("QS3D_CURTAIN_PANEL_OWNERSHIP_RESULT", "Process")
$oldNonce = [Environment]::GetEnvironmentVariable("QS3D_CURTAIN_PANEL_OWNERSHIP_NONCE", "Process")
$process = $null
$proxyInformationDialogsDismissed = 0
$startedAt = Get-Date

try {
    $env:QS3D_CURTAIN_PANEL_OWNERSHIP_RESULT = $resultPath
    $env:QS3D_CURTAIN_PANEL_OWNERSHIP_NONCE = $nonce
    $script = @(
        "FILEDIA", "0",
        "CMDECHO", "1",
        "TILEMODE", "1",
        ...
        "QS3DCURTAINP06SEED",
        "QS3DGLASSWALL",
        "QS3DCURTAINP06PREPARE",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06BASELINE",
        "QS3DCURTAINP06MISSING",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06CHECKMISSING",
        "QS3DCURTAINP06DUPLICATE",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06CHECKDUPLICATE",
        "QS3DCURTAINP06FOREIGN",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06CHECKFOREIGN",
        "QS3DCURTAINP06CROSS",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06CHECKCROSS",
        "QS3DCURTAINP06CLEARCROSS",
        "QS3DCURTAINP06VALID",
        "QS3DCURTAIN3D",
        "QS3DCURTAINP06PROBE"
    )
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        $process.Refresh()
        if ($process.HasExited) { throw "BricsCAD exited before the Curtain P06 marker." }
        Start-Sleep -Milliseconds 500
    }
    if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) { throw "Timed out waiting for the synthetic Curtain P06 ownership matrix." }
    ...
    if ($marker.ContainsKey("status") -and [string]::Equals([string]$marker["status"], "FAIL", [StringComparison]::Ordinal)) {
        $failurePhases = @(
            "PROBE_AUTH", "SEED_HOSTS", "PREPARE_BASELINE", "VERIFY_BASELINE",
            "CORRUPT_MISSING", "VERIFY_MISSING_REFUSAL", "CORRUPT_DUPLICATE", "VERIFY_DUPLICATE_REFUSAL",
            "CORRUPT_FOREIGN", "VERIFY_FOREIGN_REFUSAL", "CORRUPT_CROSS_OWNER", "VERIFY_CROSS_OWNER_REFUSAL",
            "CLEAR_CROSS_OWNER", "PREPARE_VALID_REPLACEMENT", "VERIFY_VALID_REPLACEMENT", "RESULT_PUBLISH"
        )
        ...
        foreach ($key in $marker.Keys) {
            if (-not $failureKeys.Contains([string]$key)) { throw "Curtain P06 FAIL marker contains a non-contract field." }
        }
        Require-Qs3dValue -Marker $marker -Key "command" -Expected "QS3DCURTAINP06PROBE"
        Require-Qs3dValue -Marker $marker -Key "nonce" -Expected $nonce
        Require-Qs3dValue -Marker $marker -Key "schema" -Expected "QS3D_CURTAIN_PANEL_OWNERSHIP_RUNTIME_V1"
        Require-Qs3dValue -Marker $marker -Key "qualification_boundary" -Expected "LOCAL_002_P06_ONLY"
        Require-Qs3dValue -Marker $marker -Key "production_local002_qualified" -Expected "false"
        Require-Qs3dValue -Marker $marker -Key "error_code" -Expected "CURTAIN_PANEL_OWNERSHIP_RUNTIME_FAILED"
        ...
        Require-Qs3dValue -Marker $marker -Key "glass_wall_count" -Expected "6"
        Require-Qs3dValue -Marker $marker -Key "refusal_count" -Expected "4"
        foreach ($key in @(
            "missing_handle_refused", "duplicate_canonical_refused", "foreign_unmarked_refused", "cross_owner_refused",
            "no_erase_append_verified", "surviving_old_sets_preserved", "semantic_metadata_preserved",
            "unrelated_owners_preserved", "foreign_object_preserved", "valid_replacement_succeeded",
            "valid_old_set_removed", "valid_new_set_complete"
        )) { Require-Qs3dValue -Marker $marker -Key $key -Expected "true" }
        Require-Qs3dValue -Marker $marker -Key "health_issue_count" -Expected "0"
        $baselinePanelCount = Read-PositiveMarkerInt -Marker $marker -Key "baseline_panel_count"
        $finalPanelCount = Read-PositiveMarkerInt -Marker $marker -Key "valid_replacement_panel_count"
        if ($baselinePanelCount -le $finalPanelCount) { throw "Curtain P06 six-owner baseline is unexpectedly small." }
    }

    Stop-Qs3dLaunchedProcess -Process $process
    Remove-Qs3dDrawingLocks -Paths $drawingLocks
    if (-not (Wait-Qs3dNoExactBricsCadProcesses -ExpectedExecutable $bricscadExe -TimeoutSeconds 30)) { throw "Curtain P06 V25 process cleanup is incomplete." }
    if (Test-Path -LiteralPath $scriptPath) { Remove-Item -LiteralPath $scriptPath -Force -ErrorAction Stop }
    if (Test-Path -LiteralPath $scriptPath) { throw "Curtain P06 runtime script cleanup failed." }
    $drawingHashAfter = (Get-FileHash -LiteralPath $DrawingCopy -Algorithm SHA256).Hash.ToUpperInvariant()
    if (-not [string]::Equals($drawingHashBefore, $drawingHashAfter, [StringComparison]::Ordinal)) { throw "The disposable Curtain P06 drawing was written unexpectedly." }
    foreach ($forbiddenOutput in @($projectSidecar, ($projectSidecar + ".bak"), $drawingBackup)) {
        if (Test-Path -LiteralPath $forbiddenOutput) { throw "Curtain P06 runtime probe persisted an unexpected sidecar or backup." }
    }
    if ($diagnosticFailure) { throw "Curtain P06 probe failed at sanitized phase '$failurePhase' with code '$failureCode'; cleanup was verified." }
    ...
    Write-Host "QS3D BricsCAD V25 Curtain-panel P06 ownership-failure runtime PASS"
    Write-Host "Final valid replacement panels: $finalPanelCount"
    ...
    try {
        ...
        Restore-EnvironmentValue -Name "QS3D_CURTAIN_PANEL_OWNERSHIP_RESULT" -Value $oldResult
        Restore-EnvironmentValue -Name "QS3D_CURTAIN_PANEL_OWNERSHIP_NONCE" -Value $oldNonce
    }
}
