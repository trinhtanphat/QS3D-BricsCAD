...
>>>>>>> origin/ci/full-domain-integration-final-20260810
        ...
        private static void TryPopulateRecognitionMetadata(Entity entity, EntitySnapshot snapshot)
        {
            foreach (var name in new[] { "TextString", "Contents", "Text", "Name", "BlockName", "Tag" })
            {
                try
                {
                    var property = entity.GetType().GetProperty(name, BindingFlags.Instance | BindingFlags.Public);
                    if (property == null || property.GetIndexParameters().Length != 0 || !property.CanRead) continue;
                    var value = property.GetValue(entity, null)?.ToString();
                    if (!string.IsNullOrWhiteSpace(value)) snapshot.Metadata[name] = value.Trim();
                }
                catch { }
            }
        }
