...
=======
            ...
            var sourceIds = selection.Value.GetObjectIds(); if (sourceIds.Length == 0) return 0; var created = 0;
>>>>>>> origin/agent/review-hardening-20260810
            ...
            using (var transaction = document.Database.TransactionManager.StartTransaction())
            {
                ...
                {
                    var line = transaction.GetObject(id, OpenMode.ForRead, false) as Line; if (line == null) continue;
...
<<<<<<< origin/main
                    ...
                    catch
                    {
                        ...
                        throw;
=======
                    var element = project.Elements.FirstOrDefault(x => x.Category == ElementCategory.ArchitecturalWall && x.SourceHandles.Any(h => string.Equals(h, sourceHandle, StringComparison.OrdinalIgnoreCase))); if (element == null) continue;
                    var family = project.FindFamily(element.FamilyId); var thicknessM = Number(element, family, "ThicknessM", .2d); var heightM = Number(element, family, "HeightM", 3.6d); var bottomOffsetM = Number(element, family, "BottomOffsetM", 0d);
                    var dx = line.EndPoint.X - line.StartPoint.X; var dy = line.EndPoint.Y - line.StartPoint.Y; var length = Math.Sqrt(dx * dx + dy * dy); if (length <= 1e-6 || thicknessM <= 0d || heightM <= 0d) continue;
                    var thickness = CadUnitService.MetersToDrawingUnits(document, thicknessM); var height = CadUnitService.MetersToDrawingUnits(document, heightM); var bottomOffset = CadUnitService.MetersToDrawingUnits(document, bottomOffsetM); var angle = Math.Atan2(dy, dx);
                    var mid = new Point3d((line.StartPoint.X + line.EndPoint.X) / 2d, (line.StartPoint.Y + line.EndPoint.Y) / 2d, line.StartPoint.Z + bottomOffset + height / 2d);
                    GeneratedGeometryService.ErasePrevious(document, transaction, element);
                    var solid = new Solid3d();
                    try
                    {
                        solid.SetDatabaseDefaults(document.Database); solid.CreateBox(length, thickness, height);
                        solid.TransformBy(Matrix3d.Displacement(new Vector3d(-length / 2d, -thickness / 2d, -height / 2d)));
                        solid.TransformBy(Matrix3d.Rotation(angle, Vector3d.ZAxis, Point3d.Origin));
                        solid.TransformBy(Matrix3d.Displacement(new Vector3d(mid.X, mid.Y, mid.Z))); solid.Layer = line.Layer;
                        modelSpace.AppendEntity(solid); transaction.AddNewlyCreatedDBObject(solid, true);
                        GeneratedGeometryService.Track(element, solid.Handle.ToString(), ElementCategory.ArchitecturalWall);
                        element.Properties["LengthM"] = CadUnitService.DrawingUnitsToMeters(document, length).ToString("R", CultureInfo.InvariantCulture);
                        element.Properties["ThicknessM"] = thicknessM.ToString("R", CultureInfo.InvariantCulture); element.Properties["HeightM"] = heightM.ToString("R", CultureInfo.InvariantCulture); created++;
>>>>>>> origin/agent/review-hardening-20260810
                    }
                }
                transaction.Commit();
