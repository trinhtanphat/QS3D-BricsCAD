...
namespace QS3D.BricsCAD.V25.Updates
{
    ...
    internal sealed class UpdateManifestProbe
    {
        ...
        {
            if (release == null) return UpdateManifestProbeResult.Rejected("Release cập nhật V26 không hợp lệ.");
            var manifestUri = release.ManifestUri;
            if (manifestUri == null) return UpdateManifestProbeResult.Rejected("Release V26 không có update manifest.");
            if (!IsExpectedReleaseAssetUri(manifestUri, release.Tag, GitHubReleaseClient.UpdateManifestAssetName))
            {
                return UpdateManifestProbeResult.Rejected("Update manifest V26 không thuộc đúng repository/tag/asset GitHub đã chọn.");
            }
        ...
        private static UpdateManifestProbeResult ValidateManifest(UpdateReleaseInfo release, UpdateManifestDto manifest, string expectedSigner)
        {
            ...
            if (!string.Equals(manifest.Target, Target, StringComparison.Ordinal))
                return UpdateManifestProbeResult.Rejected("Update manifest không dành cho BricsCAD V26 x64.");
            ...
            if (string.IsNullOrEmpty(packageUriText) ||
                ...
                !IsExpectedReleaseAssetUri(packageUri, release.Tag, "QS3D-BricsCAD-V26.zip"))
            {
                return UpdateManifestProbeResult.Rejected("V26 update package URL không thuộc đúng repository/tag/asset GitHub đã chọn.");
            }
