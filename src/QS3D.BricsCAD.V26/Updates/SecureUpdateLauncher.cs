    ...
    internal static class SecureUpdateLauncher
    {
        ...
        internal static bool TryGetCurrentSignerThumbprint(out string thumbprint, out string reason)
        {
            ...
            try
            {
                ...
                {
                    reason = "Không xác định được DLL QS3D V26 đang chạy.";
                    return false;
                }
                ...
                using (var signer = new X509Certificate2(certificate))
                {
                    ...
                    {
                        reason = "Bản QS3D V26 hiện tại không có publisher Authenticode hợp lệ cho auto-update.";
                        return false;
                    }
            ...
            {
                reason = "Bản QS3D V26 hiện tại chưa có Authenticode hợp lệ; cần cài thủ công một bản signed hợp lệ trước khi bật one-click update.";
                return false;
            }
        }
        ...
        internal static bool TrySchedule(UpdateReleaseInfo? release, out string error)
        {
            ...
            if (manifestUri == null)
            {
                error = "Release V26 này không có signed update manifest.";
                return false;
            }
            ...
            {
                error = "V26 update manifest không thuộc host GitHub HTTPS được phép.";
                return false;
            }
            ...
            try
            {
                ...
                var updaterPath = Path.Combine(installDirectory, "update-v26.ps1");
                if (!File.Exists(updaterPath))
                    throw new FileNotFoundException("Không tìm thấy update-v26.ps1 cạnh plugin QS3D V26.", updaterPath);
                ...
                var readyEventName = mutexName + "-Ready-" + handoffId;
                var cancelEventName = mutexName + "-Cancel-" + handoffId;
                ...
                using (var cancelEvent = new EventWaitHandle(false, EventResetMode.ManualReset, cancelEventName))
                {
                    ...
                    if (updater == null) throw new InvalidOperationException("Không thể khởi động tiến trình updater V26 tách rời.");
                    try
                    {
                        if (!readyEvent.WaitOne(WorkerReadyTimeoutMilliseconds))
                        {
                            ...
                            throw new InvalidOperationException(
                                "Updater V26 worker không xác nhận readiness trong 5 giây. QS3D đã hủy worker trước khi cài đặt; BricsCAD vẫn mở và bạn có thể thử lại.");
                        }
                    }
                    finally
                    {
        ...
        internal static bool TryRequestGracefulHostClose(out string error)
        {
            ...
            try
            {
                ...
                error = "BricsCAD chưa chấp nhận yêu cầu đóng cửa sổ chính. Hãy lưu bản vẽ và đóng BricsCAD bình thường; updater V26 sẽ tiếp tục chờ.";
                return false;
            }
            catch (Exception ex)
            {
                error = "Không gửi được yêu cầu đóng BricsCAD: " + ex.Message + " Hãy đóng BricsCAD bình thường; updater V26 sẽ tiếp tục chờ.";
                return false;
            }
        }
        ...
        private static bool TryAcquireCrossProcessReservation(out string mutexName, out string error)
        {
            ...
            try
            {
                ...
                {
                    error = "Không xác định được Windows user SID để khóa updater V26 liên tiến trình.";
                    return false;
                }
                ...
                {
                    reservation.Dispose();
                    error = "Một tiến trình QS3D V26 updater khác của Windows user này đã được lên lịch hoặc đang chạy. Hãy hoàn tất lần cập nhật đó trước.";
                    return false;
                }
            ...
            {
                error = "Không tạo được khóa updater V26 liên tiến trình: " + ex.Message;
                return false;
            }
        }
        ...
        private static void TryTerminateUnreadyWorker(Process updater)
        {
            ...
            {
                // Best effort only. Readiness timeout signals the worker cancellation event
                // before this cleanup, so a surviving child remains fail-closed before install.
            }
        }
        ...
        private static bool TryVerifyAuthenticode(string filePath, out string reason)
        {
            ...
            try
            {
                ...
                reason = "Authenticode của DLL QS3D V26 hiện tại không hợp lệ (WinVerifyTrust 0x" + unchecked((uint)status).ToString("X8") + ").";
                return false;
            }
            catch (Exception ex)
            {
                reason = "Không xác minh được Authenticode của DLL QS3D V26 hiện tại: " + ex.Message;
                return false;
            }
            finally
            {
            ...
            string cancelEventName)
        {
            ...
            script.AppendLine("$cancelEventName = " + PsLiteral(cancelEventName));
            ...
            script.AppendLine("  $cancelEvent = [System.Threading.EventWaitHandle]::OpenExisting($cancelEventName)");
            script.AppendLine("  $readyEvent.Set() | Out-Null");
            script.AppendLine("  $readyEvent.Dispose()");
            script.AppendLine("  $readyEvent = $null");
            script.AppendLine("  if ($cancelEvent.WaitOne(0)) { throw 'QS3D V26 updater was cancelled before mutex ownership.' }");
            ...
            script.AppendLine("  if (-not $ownsUpdateMutex) { throw 'Could not acquire the QS3D V26 cross-process update reservation.' }");
            script.AppendLine("  if ($cancelEvent.WaitOne(0)) { throw 'QS3D V26 updater was cancelled after mutex ownership.' }");
            script.AppendLine("  while (Get-Process -Name bricscad -ErrorAction SilentlyContinue) {");
            script.AppendLine("    if ($cancelEvent.WaitOne(0)) { throw 'QS3D V26 updater was cancelled while waiting for BricsCAD to close.' }");
            script.AppendLine("    Start-Sleep -Seconds 2");
            script.AppendLine("  }");
            script.AppendLine("  $hostClosed = $true");
            script.AppendLine("  if (-not (Test-Path -LiteralPath $updater -PathType Leaf)) { throw 'Installed QS3D V26 updater script is missing.' }");
            script.AppendLine("  $signature = Get-AuthenticodeSignature -LiteralPath $updater");
            script.AppendLine("  if ($signature.Status -ne [System.Management.Automation.SignatureStatus]::Valid -or -not $signature.SignerCertificate) { throw ('Installed V26 updater signature is not valid: ' + $signature.Status) }");
            ...
            script.AppendLine("    catch { Write-Warning ('QS3D V26 update failed and BricsCAD recovery restart also failed: ' + $_.Exception.Message) }");
            script.AppendLine("  }");
            ...
            script.AppendLine("  if ($cancelEvent) { try { $cancelEvent.Dispose() } catch { } }");
