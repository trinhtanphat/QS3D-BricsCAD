        ...
        public static bool TryFindOwner(ProjectState project, string handle, out ProjectElement? owner, out string propertyKey)
        {
            ...
            foreach (var element in project.Elements)
            {
                foreach (var entry in EnumerateOwnerHandles(element))
                {
                    ...
                    if (owner != null && !string.Equals(owner.Id, element.Id, StringComparison.OrdinalIgnoreCase))
                        throw new InvalidOperationException("Generated CAD handle " + normalized + " is ambiguously owned by semantic elements " + owner.Id + " (" + propertyKey + ") and " + element.Id + " (" + entry.Value + ").");
                    owner = element;
