...
namespace QS3D.Core.Diagnostics
{
    ...
    public sealed class ModelHealthIssue { public ModelHealthIssue(string code, HealthSeverity severity, string message, string elementId="") { Code=code??string.Empty; Severity=severity; Message=message??string.Empty; ElementId=elementId??string.Empty; } public string Code{get;} public HealthSeverity Severity{get;} public string Message{get;} public string ElementId{get;} }
...
=======
            if(project==null) throw new ArgumentNullException(nameof(project)); var issues=new List<ModelHealthIssue>(); if(project.Metadata.TryGetValue("QS3D.ReadOnlyRecoveryRequired",out var recoveryRequired)&&string.Equals(recoveryRequired,"true",StringComparison.OrdinalIgnoreCase)){var detail=project.Metadata.TryGetValue("QS3D.LoadWarning",out var warning)?warning:"QSDB could not be loaded.";issues.Add(new ModelHealthIssue("PROJECT_LOAD_FAILED",HealthSeverity.Error,"Project đang ở chế độ bảo vệ và sẽ không ghi đè .qsdb: "+detail));} else if(project.Metadata.TryGetValue("QS3D.RecoveredFromBackup",out var recovered)&&string.Equals(recovered,"true",StringComparison.OrdinalIgnoreCase))issues.Add(new ModelHealthIssue("PROJECT_RECOVERED_BACKUP",HealthSeverity.Warning,"Project được khôi phục từ file .bak. Hãy kiểm tra dữ liệu rồi lưu lại project."));
            ...
            foreach(var element in project.Elements){if(!ids.Add(element.Id))issues.Add(new ModelHealthIssue("DUPLICATE_ID",HealthSeverity.Error,"Trùng mã cấu kiện.",element.Id));if(string.IsNullOrWhiteSpace(element.FamilyId)||!familyIds.Contains(element.FamilyId))issues.Add(new ModelHealthIssue("MISSING_FAMILY",HealthSeverity.Error,"Cấu kiện chưa liên kết Family hợp lệ.",element.Id));if(string.IsNullOrWhiteSpace(element.FloorId)||!floorIds.Contains(element.FloorId))issues.Add(new ModelHealthIssue("MISSING_FLOOR",HealthSeverity.Warning,"Cấu kiện chưa có tầng hợp lệ.",element.Id));if(string.IsNullOrWhiteSpace(element.ZoneId)||!zoneIds.Contains(element.ZoneId))issues.Add(new ModelHealthIssue("MISSING_ZONE",HealthSeverity.Warning,"Cấu kiện chưa có Zone hợp lệ.",element.Id));if(element.Dirty!=ElementDirtyFlags.None)issues.Add(new ModelHealthIssue("DIRTY",HealthSeverity.Info,"Khối lượng/cấu kiện cần cập nhật.",element.Id));if((element.Category==ElementCategory.WallOpening||element.Category==ElementCategory.Door)&&!element.Properties.ContainsKey("HostWallId"))issues.Add(new ModelHealthIssue("MISSING_HOST",HealthSeverity.Error,"Cửa/lỗ mở chưa có Host Wall.",element.Id));if(RequiresMaterial(element.Category)&&!HasMaterial(project,element))issues.Add(new ModelHealthIssue("MISSING_MATERIAL",HealthSeverity.Warning,"Cấu kiện chưa có vật liệu.",element.Id));ValidateDimensions(project,element,issues);ValidateRebar(element,issues);foreach(var handle in element.SourceHandles.Where(x=>!string.IsNullOrWhiteSpace(x))){var normalized=handle.Trim();if(handles.TryGetValue(normalized,out var owner)&&!string.Equals(owner,element.Id,StringComparison.OrdinalIgnoreCase))issues.Add(new ModelHealthIssue("DUPLICATE_HANDLE",HealthSeverity.Warning,"CAD Handle đang được nhiều QS3D element sử dụng; element khác: "+owner,element.Id));else handles[normalized]=element.Id;}if(liveHandles!=null&&element.SourceHandles.Count>0&&element.SourceHandles.All(x=>!liveHandles.Contains(x)))issues.Add(new ModelHealthIssue("ORPHAN_HANDLE",HealthSeverity.Error,"Không còn tìm thấy đối tượng CAD nguồn.",element.Id));}
            return issues;
        }
        private static void ValidateDimensions(ProjectState project,ProjectElement element,ICollection<ModelHealthIssue> issues)
        {
            switch(element.Category){case ElementCategory.Beam:Need(project,element,issues,"LengthM","Dầm thiếu chiều dài.");Need(project,element,issues,"WidthM","Dầm thiếu bề rộng.");Need(project,element,issues,"HeightM","Dầm thiếu chiều cao.");break;case ElementCategory.Slab:Need(project,element,issues,"AreaM2","Sàn thiếu diện tích.");Need(project,element,issues,"ThicknessM","Sàn thiếu chiều dày.");Need(project,element,issues,"PerimeterM","Sàn thiếu chu vi để tính cốp pha biên.");break;case ElementCategory.Column:Need(project,element,issues,"HeightM","Cột thiếu chiều cao.");if(!((Number(project,element,"AreaM2")>0d&&Number(project,element,"PerimeterM")>0d)||(Number(project,element,"WidthM")>0d&&Number(project,element,"DepthM")>0d)))issues.Add(new ModelHealthIssue("MISSING_DIMENSION",HealthSeverity.Error,"Cột cần Area+Perimeter hoặc Width+Depth hợp lệ.",element.Id));break;case ElementCategory.StructuralWall:Need(project,element,issues,"LengthM","Vách thiếu chiều dài.");Need(project,element,issues,"HeightM","Vách thiếu chiều cao.");Need(project,element,issues,"ThicknessM","Vách thiếu chiều dày.");break;case ElementCategory.Foundation:NeedEitherArea(project,element,issues,"Móng thiếu diện tích đáy.");Need(project,element,issues,"ThicknessM","Móng thiếu chiều dày.","HeightM");Need(project,element,issues,"PerimeterM","Móng thiếu chu vi để tính cốp pha.");break;case ElementCategory.Stair:Need(project,element,issues,"AreaM2","Cầu thang thiếu diện tích.");Need(project,element,issues,"ThicknessM","Cầu thang thiếu chiều dày.");break;case ElementCategory.Railing:Need(project,element,issues,"LengthM","Lan can thiếu chiều dài.");break;case ElementCategory.Earthwork:NeedEitherArea(project,element,issues,"Đào đất thiếu diện tích.","ExcavationAreaM2");Need(project,element,issues,"DepthM","Đào đất thiếu chiều sâu.");break;}
>>>>>>> origin/ci/full-domain-integration-final-20260810
        }
        private static void Need(ProjectState project,ProjectElement element,ICollection<ModelHealthIssue> issues,string key,string message,string? alternate=null){if(Number(project,element,key)>0d)return;var alternateKey=alternate??string.Empty;if(alternateKey.Length>0&&Number(project,element,alternateKey)>0d)return;issues.Add(new ModelHealthIssue("MISSING_DIMENSION",HealthSeverity.Error,message,element.Id));}
        private static void NeedEitherArea(ProjectState project,ProjectElement element,ICollection<ModelHealthIssue> issues,string message,string? alternate=null){var alternateKey=alternate??string.Empty;if(Number(project,element,"AreaM2")>0d||Number(project,element,"BaseAreaM2")>0d||(alternateKey.Length>0&&Number(project,element,alternateKey)>0d))return;issues.Add(new ModelHealthIssue("MISSING_DIMENSION",HealthSeverity.Error,message,element.Id));}
        private static double Number(ProjectState project,ProjectElement element,string key){if(element.Properties.TryGetValue(key,out var value)&&double.TryParse(value,NumberStyles.Float,CultureInfo.InvariantCulture,out var own)&&!double.IsNaN(own)&&!double.IsInfinity(own))return own;var family=project.FindFamily(element.FamilyId);if(family!=null&&family.Properties.TryGetValue(key,out value)&&double.TryParse(value,NumberStyles.Float,CultureInfo.InvariantCulture,out var inherited)&&!double.IsNaN(inherited)&&!double.IsInfinity(inherited))return inherited;return 0d;}
        private static void ValidateRebar(ProjectElement element,ICollection<ModelHealthIssue> issues){if(!element.Properties.TryGetValue("RebarNotation",out var notation)||string.IsNullOrWhiteSpace(notation))return;try{RebarNotationParser.Parse(notation);}catch(Exception ex)when(ex is FormatException||ex is ArgumentException||ex is OverflowException){issues.Add(new ModelHealthIssue("INVALID_REBAR",HealthSeverity.Error,"Ký hiệu thép không hợp lệ: "+ex.Message,element.Id));return;}var hasLength=element.Properties.TryGetValue("RebarCuttingLengthM",out var cutting)&&!string.IsNullOrWhiteSpace(cutting);if(!hasLength)hasLength=element.Properties.TryGetValue("LengthM",out var length)&&!string.IsNullOrWhiteSpace(length);if(!hasLength)hasLength=element.Quantities.TryGetValue("LengthM",out var quantityLength)&&quantityLength>0d;if(!hasLength)issues.Add(new ModelHealthIssue("REBAR_LENGTH_MISSING",HealthSeverity.Warning,"Thép có notation nhưng chưa có chiều dài cắt/chiều dài cấu kiện.",element.Id));}
        private static bool HasMaterial(ProjectState project,ProjectElement element){if(element.Properties.TryGetValue("Material",out var own)&&!string.IsNullOrWhiteSpace(own))return true;var family=project.FindFamily(element.FamilyId);return family!=null&&family.Properties.TryGetValue("Material",out var familyMaterial)&&!string.IsNullOrWhiteSpace(familyMaterial);}
        private static bool RequiresMaterial(ElementCategory category){switch(category){case ElementCategory.Beam:case ElementCategory.Slab:case ElementCategory.Column:case ElementCategory.StructuralWall:case ElementCategory.Foundation:case ElementCategory.Stair:case ElementCategory.Railing:case ElementCategory.ArchitecturalWall:case ElementCategory.GlassWall:case ElementCategory.WallPier:case ElementCategory.FloorFinish:case ElementCategory.Waterproofing:case ElementCategory.Skirting:case ElementCategory.WallFinish:case ElementCategory.CeilingFinish:case ElementCategory.Door:return true;default:return false;}}
    }
}
