            ...
            foreach (var element in project.Elements)
            {
                ...
                foreach (var item in raw.Split(new[] { ';' }, StringSplitOptions.None))
                {
                    ...
                    if (ownership.IsConflicted(identity, expectedOwner))
                        issues.Add(new ModelHealthIssue("BEAM_STIRRUP_GENERATED_OWNERSHIP_CONFLICT", HealthSeverity.Error, "Generated beam stirrup solid xung đột owner/project handle khác: " + ownership.Describe(identity), element.Id));
