...
{
    public enum InterchangeIdentityKind
    {
        Zone = 0,
        Floor = 1,
        Family = 2,
    ...
    public enum InterchangeIdentityDisposition
    {
        New = 0,
        ExistingNeedsPolicy = 1,
        ExistingIncompatible = 2
    }

    public enum InterchangeDrawingFingerprintRelation
    {
        Unknown = 0,
        Match = 1,
        Different = 2
    ...
    {
        public InterchangeImportPreviewItem(
            InterchangeIdentityKind kind,
            string id,
            InterchangeIdentityDisposition disposition,
            string reason,
    ...
    {
        internal ProjectInterchangeImportPreviewResult(
            ProjectInterchangeValidationResult validation,
            string sourceProjectId,
            string targetProjectId,
            bool sameProjectId,
            InterchangeDrawingFingerprintRelation drawingFingerprintRelation,
            int totalIdentityCount,
            int newIdentityCount,
            int policyCollisionCount,
            int incompatibleCollisionCount,
            bool detailsTruncated,
            IEnumerable<InterchangeImportPreviewItem> items)
        {
    ...
    public static class ProjectInterchangeImportPreview
    {
        ...
        public static ProjectInterchangeImportPreviewResult Plan(ProjectState targetProject, string json)
        {
            ...
            {
                return new ProjectInterchangeImportPreviewResult(
                    validation,
                    string.Empty,
                    ...
                    0,
                    false,
                    Array.Empty<InterchangeImportPreviewItem>());
            }
            ...
            var fingerprintRelation = CompareFingerprint(
                manifest.Project == null ? null : manifest.Project.DrawingFingerprint,
            ...
            foreach (var family in manifest.Families ?? new List<CategorizedIdentityContract>())
            {
                ...
                else if (existing.Category != sourceCategory)
                {
                    AddDetail(new InterchangeImportPreviewItem(
                        ...
                        "The target Family uses the same id with a different category; automatic merge must fail closed.",
                        sourceCategory.ToString(),
                        existing.Category.ToString()), items);
                    total++;
                    incompatibleCount++;
                }
                else
                {
                    AddDetail(new InterchangeImportPreviewItem(
                        ...
                        "The target already contains this Family id. Property/name merge semantics require an explicit import policy.",
                        sourceCategory.ToString(),
                        existing.Category.ToString()), items);
                    total++;
                    policyCount++;
            ...
            foreach (var element in manifest.Elements ?? new List<CategorizedIdentityContract>())
            {
                ...
                else if (existing.Category != sourceCategory)
                {
                    AddDetail(new InterchangeImportPreviewItem(
                        ...
                        id,
                        InterchangeIdentityDisposition.ExistingIncompatible,
                        "The target element uses the same id with a different category; automatic merge must fail closed.",
                        sourceCategory.ToString(),
                        existing.Category.ToString()), items);
                    total++;
                    incompatibleCount++;
                ...
                else
                {
                    AddDetail(new InterchangeImportPreviewItem(
                        ...
                        id,
                        InterchangeIdentityDisposition.ExistingNeedsPolicy,
                        "The target already contains this semantic element id. Geometry/provenance/property merge semantics require an explicit import policy.",
                        sourceCategory.ToString(),
                        existing.Category.ToString()), items);
                    total++;
                    policyCount++;
        ...
        private static void AddSimple<T>(
            ...
            string id,
            bool exists,
            ICollection<InterchangeImportPreviewItem> items,
            ref int total,
            ref int newCount,
            ref int policyCount)
        {
            AddDetail(new InterchangeImportPreviewItem(
                kind,
                id,
                exists ? InterchangeIdentityDisposition.ExistingNeedsPolicy : InterchangeIdentityDisposition.New,
                exists
                    ? "The target already contains this semantic id; rename/merge/replace behavior requires an explicit import policy."
                    : "No target semantic definition uses this id."), items);
            total++;
            if (exists) policyCount++;
            else newCount++;
        }
        ...
        {
            if (!Enum.TryParse<ElementCategory>((raw ?? string.Empty).Trim(), true, out var category))
        ...
        private static Dictionary<string, T> UniqueIndex<T>(IEnumerable<T> source, Func<T, string> idSelector, string label) where T : class
        {
            ...
            foreach (var item in source)
            {
                ...
                if (result.ContainsKey(id))
                    throw new InvalidOperationException("Project contains duplicate " + label + " id: " + id + ". Import preview refuses ambiguous target identity.");
                result[id] = item;
            }
            return result;
        }
        ...
        private static ManifestContract ParseValidatedManifest(string json)
        {
            try
            {
                var serializer = new DataContractJsonSerializer(typeof(ManifestContract), new DataContractJsonSerializerSettings
                {
