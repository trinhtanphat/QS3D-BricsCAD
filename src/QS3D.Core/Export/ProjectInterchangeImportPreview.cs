...
namespace QS3D.Core.Export
{
    ...
    public sealed class InterchangeImportPreviewItem
    {
        ...
        {
            Kind = kind; Id = id ?? string.Empty; Disposition = disposition; Reason = reason ?? string.Empty;
            SourceCategory = sourceCategory ?? string.Empty; TargetCategory = targetCategory ?? string.Empty;
        }
    ...
    public sealed class ProjectInterchangeImportPreviewResult
    {
        ...
        {
            Validation = validation ?? throw new ArgumentNullException(nameof(validation)); SourceProjectId = sourceProjectId ?? string.Empty;
            TargetProjectId = targetProjectId ?? string.Empty; SameProjectId = sameProjectId; DrawingFingerprintRelation = drawingFingerprintRelation;
            TotalIdentityCount = totalIdentityCount; NewIdentityCount = newIdentityCount; PolicyCollisionCount = policyCollisionCount;
            IncompatibleCollisionCount = incompatibleCollisionCount; DetailsTruncated = detailsTruncated;
    ...
    public static class ProjectInterchangeImportPreview
    {
        ...
        public static ProjectInterchangeImportPreviewResult Plan(ProjectState targetProject, string json)
        {
            ...
            var total = 0; var newCount = 0; var policyCount = 0; var incompatibleCount = 0;
            ...
            foreach (var family in manifest.Families ?? new List<CategorizedIdentityContract>())
            {
                ...
                var id = Required(family.Id, "Family id"); var sourceCategory = ParseCategory(family.Category, "Family " + id);
                if (!targetFamilies.TryGetValue(id, out var existing)) { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Family, id, InterchangeIdentityDisposition.New, "No target Family uses this semantic id.", sourceCategory.ToString()), items); total++; newCount++; }
                else if (existing.Category != sourceCategory) { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Family, id, InterchangeIdentityDisposition.ExistingIncompatible, "The target Family uses the same id with a different category; automatic merge must fail closed.", sourceCategory.ToString(), existing.Category.ToString()), items); total++; incompatibleCount++; }
                else { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Family, id, InterchangeIdentityDisposition.ExistingNeedsPolicy, "The target already contains this Family id. Property/name merge semantics require an explicit import policy.", sourceCategory.ToString(), existing.Category.ToString()), items); total++; policyCount++; }
            }
            foreach (var element in manifest.Elements ?? new List<CategorizedIdentityContract>())
            {
                ...
                var id = Required(element.Id, "element id"); var sourceCategory = ParseCategory(element.Category, "element " + id);
                if (!targetElements.TryGetValue(id, out var existing)) { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Element, id, InterchangeIdentityDisposition.New, "No target semantic element uses this id.", sourceCategory.ToString()), items); total++; newCount++; }
                else if (existing.Category != sourceCategory) { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Element, id, InterchangeIdentityDisposition.ExistingIncompatible, "The target element uses the same id with a different category; automatic merge must fail closed.", sourceCategory.ToString(), existing.Category.ToString()), items); total++; incompatibleCount++; }
                else { AddDetail(new InterchangeImportPreviewItem(InterchangeIdentityKind.Element, id, InterchangeIdentityDisposition.ExistingNeedsPolicy, "The target already contains this semantic element id. Geometry/provenance/property merge semantics require an explicit import policy.", sourceCategory.ToString(), existing.Category.ToString()), items); total++; policyCount++; }
            }
        ...
        private static void AddSimple(InterchangeIdentityKind kind, string id, bool exists, ICollection<InterchangeImportPreviewItem> items, ref int total, ref int newCount, ref int policyCount)
        {
            ...
            total++; if (exists) policyCount++; else newCount++;
        }
        private static void AddDetail(InterchangeImportPreviewItem item, ICollection<InterchangeImportPreviewItem> items) { if (items.Count < MaxDetailedItems) items.Add(item); }
        private static InterchangeDrawingFingerprintRelation CompareFingerprint(string? source, string? target)
        {
            var left = (source ?? string.Empty).Trim(); var right = (target ?? string.Empty).Trim();
        ...
        {
            if (!Enum.TryParse<ElementCategory>((raw ?? string.Empty).Trim(), true, out var category)) throw new InvalidDataException("Validated interchange " + label + " contains an unsupported category.");
            return category;
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(value)) throw new InvalidDataException("Required identity value is empty: " + label + "."); return value.Trim();
        }
        private static ManifestContract ParseValidatedManifest(string json)
        {
            try
            {
                ...
                using (var stream = new MemoryStream(Encoding.UTF8.GetBytes(json), false))
                {
                    ...
                    if (result == null) throw new InvalidDataException("Validated interchange snapshot did not deserialize into an import-preview manifest."); return result;
                }
            }
            catch (Exception ex) when (ex is SerializationException || ex is FormatException || ex is InvalidCastException) { throw new InvalidDataException("Interchange validator passed but import-preview manifest parsing failed.", ex); }
        }

        [DataContract] private sealed class ManifestContract { [DataMember(Name = "project")] public ProjectContract? Project { get; set; } [DataMember(Name = "zones")] public List<IdentityContract>? Zones { get; set; } [DataMember(Name = "floors")] public List<IdentityContract>? Floors { get; set; } [DataMember(Name = "families")] public List<CategorizedIdentityContract>? Families { get; set; } [DataMember(Name = "elements")] public List<CategorizedIdentityContract>? Elements { get; set; } }
        [DataContract] private sealed class ProjectContract { [DataMember(Name = "id")] public string? Id { get; set; } [DataMember(Name = "drawingFingerprint")] public string? DrawingFingerprint { get; set; } }
        [DataContract] private sealed class IdentityContract { [DataMember(Name = "id")] public string? Id { get; set; } }
        [DataContract] private sealed class CategorizedIdentityContract { [DataMember(Name = "id")] public string? Id { get; set; } [DataMember(Name = "category")] public string? Category { get; set; } }
    }
}
