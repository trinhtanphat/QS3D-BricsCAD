        ...
        public int AssignFamily(ProjectState project, IEnumerable<string> elementIds, string familyId)
        {
            ...
            foreach (var id in elementIds)
            {
                ...
                if (previousFamily != null)
                {
                    ...
                    {
                        if (element.Properties.TryGetValue(property.Key, out var current) &&
                            string.Equals(current, property.Value ?? string.Empty, StringComparison.Ordinal))
                ...
                element.MarkGeneratedGeometryStale("Family changed: " + family.Id);
                element.MarkDirty(ElementDirtyFlags.Properties | ElementDirtyFlags.Quantity);
                count++;
            }
