...
namespace QS3D.Core.Services
{
    public static class SemanticHandleOwnershipResolver
    {
        ...
        public static ProjectElement? ResolveCaptureTarget(ProjectState project, string sourceHandle, ElementCategory category, string canonicalId)
        {
            ...
            var sourceOwner = ResolveUniqueSourceOwner(project, sourceHandle);
            if (sourceOwner != null && sourceOwner.Category != category)
                throw new InvalidOperationException(
                    "CAD source handle " + sourceHandle.Trim() + " is already tracked as " + sourceOwner.Category +
            ...
            if (sourceOwner != null)
            {
                if (canonical != null && !ReferenceEquals(sourceOwner, canonical))
                    throw new InvalidOperationException(
                        "CAD source handle " + sourceHandle.Trim() + " belongs to " + sourceOwner.Id +
            ...
            if (canonical.SourceHandles.Any(x => !string.Equals(x, sourceHandle, StringComparison.OrdinalIgnoreCase)))
                throw new InvalidOperationException(
