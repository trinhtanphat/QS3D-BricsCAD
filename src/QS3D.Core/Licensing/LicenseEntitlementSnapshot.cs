...
namespace QS3D.Core.Licensing
{
    ...
    public sealed class LicenseEntitlementSnapshot
    {
        ...
        private static string RequireBounded(string value, string parameterName, int maxBytes)
        {
            ...
            if (Encoding.UTF8.GetByteCount(normalized) > maxBytes) throw new ArgumentException("Value exceeds the persistence bound.", parameterName);
            return normalized;
        }
        ...
        private static string RequirePayload(string payload)
        {
            ...
            if (Encoding.UTF8.GetByteCount(payload) > MaxPayloadBytes) throw new ArgumentException("Entitlement payload exceeds the persistence bound.", nameof(payload));
            return payload;
        }
