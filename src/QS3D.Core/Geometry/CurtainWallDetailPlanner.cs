...
namespace QS3D.Core.Geometry
{
    ...
    {
        internal CurtainWallDetail(CurtainWallLayout layout, IReadOnlyList<CurtainWallRect> panels, IReadOnlyList<CurtainWallRect> verticalFrames, IReadOnlyList<CurtainWallRect> horizontalFrames)
        {
    ...
    public static class CurtainWallDetailPlanner
    {
        ...
        private static List<CurtainWallRect> BuildPanelCells(CurtainWallLayoutInput input, CurtainWallLayout layout)
        {
            ...
            {
                var bottom = row == 0 ? input.PerimeterFrameWidthM : Add(Multiply(row, layout.BayHeightM, "curtain panel bottom grid"), halfTransom, "curtain panel bottom");
                var top = row + 1 == layout.Rows ? Subtract(input.HeightM, input.PerimeterFrameWidthM, "curtain panel top perimeter") : Subtract(Multiply(row + 1d, layout.BayHeightM, "curtain panel top grid"), halfTransom, "curtain panel top");
                var height = SubtractPositive(top, bottom, "curtain panel height");
                for (var column = 0; column < layout.Columns; column++)
                {
                    var left = column == 0 ? input.PerimeterFrameWidthM : Add(Multiply(column, layout.BayWidthM, "curtain panel left grid"), halfMullion, "curtain panel left");
                    var right = column + 1 == layout.Columns ? Subtract(input.LengthM, input.PerimeterFrameWidthM, "curtain panel right perimeter") : Subtract(Multiply(column + 1d, layout.BayWidthM, "curtain panel right grid"), halfMullion, "curtain panel right");
        ...
        {
            x = Finite(x, label + " X"); z = Finite(z, label + " Z"); width = Positive(width, label + " width"); height = Positive(height, label + " height");
            if (x < -1e-12d || z < -1e-12d) throw new InvalidOperationException(label + " starts outside the curtain wall extent.");
            var normalizedX = Math.Max(0d, x); var normalizedZ = Math.Max(0d, z);
            var right = Finite(normalizedX + width, label + " right"); var top = Finite(normalizedZ + height, label + " top");
            if (!(right > normalizedX)) throw new OverflowException(label + " width is below the representable coordinate resolution.");
            if (!(top > normalizedZ)) throw new OverflowException(label + " height is below the representable coordinate resolution.");
            return new CurtainWallRect(normalizedX, normalizedZ, width, height);
        }

        private static double Positive(double value, string label) { value = Finite(value, label); if (!(value > 0d)) throw new InvalidOperationException(label + " must be greater than zero."); return value; }
        private static double Add(double left, double right, string label) { left = Finite(left, label + " left"); right = Finite(right, label + " right"); var result = Finite(left + right, label); if (left > 0d && right > 0d && (result == left || result == right)) throw new OverflowException(label + " lost a positive contribution at floating-point precision."); return result == 0d ? 0d : result; }
        private static double Subtract(double left, double right, string label) { left = Finite(left, label + " left"); right = Finite(right, label + " right"); var result = Finite(left - right, label); if (right > 0d && result == left) throw new OverflowException(label + " lost a positive deduction at floating-point precision."); return result == 0d ? 0d : result; }
        private static double Multiply(double left, double right, string label) { left = Finite(left, label + " left"); right = Finite(right, label + " right"); var result = Finite(left * right, label); if (result == 0d && left != 0d && right != 0d) throw new OverflowException(label + " underflowed to zero."); return result == 0d ? 0d : result; }
        private static double SubtractPositive(double left, double right, string label) { var result = Subtract(left, right, label); if (!(result > 0d)) throw new InvalidOperationException(label + " must be positive."); return result; }
        private static double Finite(double value, string label) { if (double.IsNaN(value) || double.IsInfinity(value)) throw new OverflowException(label + " must be finite."); return value; }
    }
}
