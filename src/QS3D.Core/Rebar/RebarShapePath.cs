...
namespace QS3D.Core.Rebar
{
    ...
    public sealed class RebarShapePath
    {
        public RebarShapePath(string shapeCode, IReadOnlyList<RebarShapePoint> points)
        {
            ...
            if (Points.Count < 2) throw new ArgumentException("Rebar shape path requires at least two points.", nameof(points));
        }
    ...
    public static class RebarShapePathBuilder
    {
        ...
        public static RebarShapePath Build(string? shapeCode, double cuttingLengthM, string? legsText = null, string? turnsText = null)
        {
            ...
            if (IsStraight(code) && string.IsNullOrWhiteSpace(legsText))
                return new RebarShapePath(code, new[] { new RebarShapePoint(0d, 0d), new RebarShapePoint(cuttingLengthM, 0d) });
        ...
        private static IReadOnlyList<double> PresetTurns(string code, int legCount)
        {
            if (IsStraight(code))
            {
                if (legCount != 1) throw new InvalidOperationException("Straight rebar shape 00 must contain exactly one leg when RebarShapeLegsM is supplied.");
                return Array.Empty<double>();
            }
            if (code == "11" || code == "L")
            {
                if (legCount != 2) throw new InvalidOperationException("L/11 rebar shape requires exactly two legs.");
                return new[] { 90d };
            }
            if (code == "21" || code == "U")
            {
                if (legCount != 3) throw new InvalidOperationException("U/21 rebar shape requires exactly three legs.");
                return new[] { 90d, 90d };
            }
            if (code == "31" || code == "Z")
            {
                if (legCount != 3) throw new InvalidOperationException("Z/31 rebar shape requires exactly three legs.");
                return new[] { 90d, -90d };
            }
        ...
        private static List<double> ParsePositiveList(string? text, string label)
        {
            ...
            {
                if (!double.TryParse(token, NumberStyles.Float, CultureInfo.InvariantCulture, out var value) || double.IsNaN(value) || double.IsInfinity(value) || value <= 0d)
                    throw new FormatException(label + " contains an invalid positive number: " + token);
                values.Add(value);
            }
            return values;
        }
        ...
        private static IReadOnlyList<double> ParseTurns(string? text)
        {
            ...
            foreach (var token in Split(text))
            {
                if (!double.TryParse(token, NumberStyles.Float, CultureInfo.InvariantCulture, out var value) || double.IsNaN(value) || double.IsInfinity(value) || Math.Abs(value) > 180d)
                    throw new FormatException("RebarShapeTurnsDeg contains an invalid turn angle: " + token);
                values.Add(value);
            }
            return values;
        }
        ...
        {
            var total = 0d;
            foreach (var leg in legs) total = AddFinite(total, leg, "rebar shape total length");
            var tolerance = Math.Max(1e-6d, cuttingLengthM * 1e-6d);
            if (Math.Abs(total - cuttingLengthM) > tolerance)
                throw new InvalidOperationException("RebarShapeLegsM total " + total.ToString("R", CultureInfo.InvariantCulture) + " m does not match BBS cutting length " + cuttingLengthM.ToString("R", CultureInfo.InvariantCulture) + " m.");
        }
