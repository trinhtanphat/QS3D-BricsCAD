...
namespace QS3D.Core.BenchmarkParity
{
    ...
    /// <summary>
    /// Fail-closed handoff from reviewed Cubicost reconstruction quantities into the
    /// existing inventory boundary and transport-neutral estimate/tender/procurement
    /// integration points. Quantity arithmetic remains owned by
    /// CubicostConcreteFormworkWorkflow.
    /// </summary>
    public sealed class CubicostReviewedQuantityDownstreamBridge
    {
        public IReadOnlyList<CubicostDownstreamLine> Admit(
            IEnumerable<CubicostQuantityLine> lines,
            ...
            bool allowProposed)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (line.ReviewStatus == ComponentRecognitionStatus.Proposed && !allowProposed)
                    throw new InvalidOperationException("Unreviewed Cubicost quantity cannot enter the commercial handoff: " + line.ComponentId + ".");
                if (line.ReviewStatus != ComponentRecognitionStatus.Proposed &&
                    line.ReviewStatus != ComponentRecognitionStatus.Accepted &&
                    line.ReviewStatus != ComponentRecognitionStatus.Corrected)
                    throw new InvalidOperationException("Unsupported Cubicost review state: " + line.ReviewStatus + ".");

                CubicostDownstreamBinding binding;
                if (!bindingMap.TryGetValue(line.Classification, out binding!))
                    throw new InvalidOperationException("Missing Cubicost downstream classification/formula binding: " + line.Classification + ".");
            ...
            return new ReadOnlyCollection<CubicostDownstreamLine>(admitted
        ...
        public IReadOnlyList<TakeoffInventoryLine> BuildInventory(IEnumerable<CubicostDownstreamLine> admitted)
        {
            ...
            return new ReadOnlyCollection<TakeoffInventoryLine>(snapshot
                .GroupBy(x => new { x.InventoryClassification, x.Unit })
                .Select(g => new TakeoffInventoryLine(g.Key.InventoryClassification, g.Key.Unit, g.Sum(x => x.Quantity), g.Select(x => x.ComponentId).Distinct(StringComparer.OrdinalIgnoreCase).Count()))
        ...
        public IReadOnlyList<CubicostCommercialHandoff> BuildCommercialHandoffs(IEnumerable<CubicostDownstreamLine> admitted)
        {
            ...
            return new ReadOnlyCollection<CubicostCommercialHandoff>(destinations
                .SelectMany(destination => snapshot.Select(line => new CubicostCommercialHandoff(destination, line)))
                .OrderBy(x => x.Destination, StringComparer.Ordinal)
                .ThenBy(x => x.Line.InventoryClassification, StringComparer.OrdinalIgnoreCase)
                .ThenBy(x => x.Line.ComponentId, StringComparer.OrdinalIgnoreCase)
                .ToList());
        }
        ...
        private static void ValidateAdmitted(IEnumerable<CubicostDownstreamLine> lines)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (line.ReviewStatus == ComponentRecognitionStatus.Rejected || line.ReviewStatus == ComponentRecognitionStatus.Proposed)
                    throw new InvalidOperationException("Only accepted/corrected Cubicost quantities may be published downstream.");
        ...
        private static void ValidateQuantity(double quantity, string componentId, string kind)
        {
            if (double.IsNaN(quantity) || double.IsInfinity(quantity) || quantity < 0d)
                throw new InvalidOperationException("Invalid " + kind + " quantity for Cubicost component " + componentId + ".");
        }
    }
}
