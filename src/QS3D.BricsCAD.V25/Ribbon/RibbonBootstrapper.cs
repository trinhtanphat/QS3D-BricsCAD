...
namespace QS3D.BricsCAD.V25.Ribbon
{
    internal static class RibbonBootstrapper
    {
        ...
        {
            public RibbonButtonSpec(string text, string command) { Text = text; Command = command; }
        ...
        {
            public RibbonTabSpec(string id, string title, params RibbonButtonSpec[] buttons) { Id = id; Title = title; Buttons = buttons; }
        ...
        public static bool TryInitialize()
        {
            ...
            try
            {
                ...
                foreach (var spec in CreateSpecs())
                {
                    ...
                    SetProperty(tab, "Id", spec.Id);
                    SetProperty(tab, "Name", spec.Id);
                    SetProperty(tab, "Title", spec.Title);
                    var panels = GetProperty(tab, "Panels") ?? throw new InvalidOperationException("RibbonTab.Panels was not available.");

                    var source = Create("Bricscad.Windows.RibbonPanelSource");
                    SetProperty(source, "Id", spec.Id + "_PANEL_SOURCE");
                    SetProperty(source, "Name", spec.Title);
                    ...
                    foreach (var buttonSpec in spec.Buttons)
                    {
                        ...
                        SetProperty(button, "Id", spec.Id + "_" + Normalize(buttonSpec.Text));
        ...
        private static IEnumerable<RibbonTabSpec> CreateSpecs()
        {
            ...
            yield return new RibbonTabSpec("QS3D_BIM", "MÔ HÌNH BIM", new RibbonButtonSpec("Phòng", "QS3DROOM"), new RibbonButtonSpec("Tường KT", "QS3DWALL"), new RibbonButtonSpec("Lỗ mở", "QS3DOPENING"), new RibbonButtonSpec("Cửa", "QS3DDOOR"), new RibbonButtonSpec("HT_Phòng", "QS3DFINISH"));
        ...
        private static void SetProperty(object target, string name, object value)
        {
            ...
            if (value == null || property.PropertyType.IsInstanceOfType(value) || property.PropertyType == value.GetType()) property.SetValue(target, value, null);
        }
        private static void Add(object collection, object item)
        {
            var method = collection.GetType().GetMethods(BindingFlags.Instance | BindingFlags.Public).FirstOrDefault(x => x.Name == "Add" && x.GetParameters().Length == 1 && x.GetParameters()[0].ParameterType.IsAssignableFrom(item.GetType()));
