...
namespace QS3D.BricsCAD.V25
{
    public sealed class GeometryExtensionsCommands
    {
        ...
        public void ShowGeometryExtensions()
        {
            ...
            try
            {
                ...
                if (published != null)
                {
                    ...
                    {
                        try { PaletteCoordinator.SetStatus("Geometry Extensions hiện có đã được kích hoạt cho đúng bản vẽ."); } catch { }
                    }
                    return;
                }
                ...
                if (IsActiveDocumentGeneration(document, nativeDatabaseIdentity))
                {
                    try { PaletteCoordinator.SetStatus("Đã mở Geometry Extensions cho đúng bản vẽ."); } catch { }
                }
            }
            catch
            {
        ...
        {
            if (!IsActiveDocumentGeneration(document, nativeDatabaseIdentity)) return;
            const string blockedStatus = "Geometry Extensions hiện tại chưa thể đóng an toàn; không mở bản sao thứ hai.";
            try { document.Editor.WriteMessage("\nQS3DGEOMETRYEXT: cửa sổ hiện tại chưa đạt terminal Closed; không mở bản sao thứ hai."); } catch { }
        ...
        {
            if (!IsActiveDocumentGeneration(document, nativeDatabaseIdentity)) return;
            const string message = "QS3DGEOMETRYEXT không thể mở Geometry Extensions an toàn; trạng thái hiện tại được giữ nguyên.";
        ...
        private static IntPtr GetNativeDatabaseIdentity(Document document)
        {
            var database = document.Database;
            if (database == null)
                throw new InvalidOperationException("Geometry Extensions requires a BricsCAD document database.");

            var identity = database.UnmanagedObject;
            if (identity == IntPtr.Zero)
                throw new InvalidOperationException("Geometry Extensions requires a live native BricsCAD database.");
            return identity;
        }

        private sealed class PublishedWindow
        {
            internal PublishedWindow(GeometryExtensionsWindow window, Document document, IntPtr nativeDatabaseIdentity)
            {
            ...
            internal GeometryExtensionsWindow Window { get; }
