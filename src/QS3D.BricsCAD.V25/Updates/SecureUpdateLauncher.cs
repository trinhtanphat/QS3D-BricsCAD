...
namespace QS3D.BricsCAD.V25.Updates
{
    internal static class SecureUpdateLauncher
    {
        ...
        private const string DemandLoadRegistryRoot = @"Software\Bricsys\BricsCAD\V25x64";
        ...
        private static bool TryResolveRegisteredLoadMode(string pluginPath, out string loadMode, out string reason)
        {
            ...
            try
            {
                ...
                {
                    reason = "Không xác định được DLL QS3D đang chạy để bảo toàn chế độ DemandLoad.";
                    return false;
                }
                ...
                using (var hostKey = Registry.CurrentUser.OpenSubKey(DemandLoadRegistryRoot, writable: false))
                {
                    if (hostKey == null)
                    {
                        reason = "Không tìm thấy registry BricsCAD V25 để bảo toàn chế độ DemandLoad QS3D.";
                        return false;
                    }
                    ...
                    {
                        reason = "Registry BricsCAD V25 có quá nhiều nhánh con; one-click update đã dừng để tránh đọc trạng thái DemandLoad mơ hồ.";
                        return false;
                    }
                    ...
                    foreach (var languageName in languageNames)
                    {
                        ...
                        using (var appKey = hostKey.OpenSubKey(languageName + @"\Applications\QS3D", writable: false))
                        {
                            ...
                            {
                                reason = "DemandLoad QS3D V25 phải dùng Loader REG_SZ và LoadCtrls REG_DWORD; one-click update đã dừng.";
                                return false;
                            }
                            ...
                            {
                                reason = "DemandLoad QS3D V25 không trỏ đúng DLL đang chạy; hãy cài lại bản hiện tại trước khi dùng one-click update.";
                                return false;
                            }
                            ...
                            {
                                reason = "DemandLoad QS3D V25 chỉ chấp nhận LoadCtrls 2 (OnStartup) hoặc 4 (OnCommand).";
                                return false;
                            }
                            if (registeredLoadCtrls.HasValue && registeredLoadCtrls.Value != loadCtrls)
                            {
                                reason = "Các ngôn ngữ BricsCAD V25 đang đăng ký QS3D với chế độ DemandLoad khác nhau; one-click update đã dừng.";
                                return false;
                            }
                    ...
                    {
                        reason = "Không tìm thấy DemandLoad QS3D V25 hợp lệ trỏ tới DLL đang chạy.";
                        return false;
                    }
            ...
            {
                reason = "Không đọc được DemandLoad QS3D V25 để bảo toàn chế độ hiện tại: " + ex.Message;
                return false;
            }
        }
            ...
            string cancelEventName)
        {
            ...
            script.AppendLine("  $readyEvent = $null");
            script.AppendLine("  if ($cancelEvent.WaitOne(0)) { throw 'QS3D updater was cancelled before mutex ownership.' }");
            ...
            script.AppendLine("  if ($cancelEvent.WaitOne(0)) { throw 'QS3D updater was cancelled after mutex ownership.' }");
            ...
            script.AppendLine("  if ($cancelEvent.WaitOne(0)) { throw 'QS3D updater was cancelled before installer execution.' }");
