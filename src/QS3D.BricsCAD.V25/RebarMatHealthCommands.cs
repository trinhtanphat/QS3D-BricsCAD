...
{
    public sealed class RebarMatHealthCommands
    {
        [CommandMethod("QS3DREBARMATHEALTH", CommandFlags.Modal)]
        public void ReviewRebarMat()
        {
            ...
            try
            {
                ...
                var issues = new GeneratedRebarMatHealthService().Inspect(project, live);
                var summary = new HealthSummary(issues);
                var message = "Rebar Mat Health: " + summary.Errors + " lỗi • " + summary.Warnings + " cảnh báo • " + summary.Info + " thông tin";
                ...
                Application.ShowModelessWindow(IntPtr.Zero, new ModelHealthWindow(issues, issue =>
                {
                    ...
                    PaletteCoordinator.SetStatus("Rebar Mat Health Định vị " + element.Id + " • " + count + " solid(s)");
                    if (count > 0) document.SendStringToExecute("QS3DZOOMSELECTED ", true, false, false);
                }), true);
            }
            catch (Exception ex)
            {
                var message = "QS3DREBARMATHEALTH lỗi: " + ex.Message;
                PaletteCoordinator.SetStatus(message);
                document.Editor.WriteMessage("\n" + message);
            }
        }
        ...
        {
            if (!element.Properties.TryGetValue(RebarMatSolidBuilder.HandlesKey, out var raw) || string.IsNullOrWhiteSpace(raw)) return Array.Empty<string>();
