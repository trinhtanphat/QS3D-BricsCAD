...
namespace QS3D.BricsCAD.V25
{
    public sealed class ModelReviewCommands
    {
        ...
        [CommandMethod("QS3DFOCUS", CommandFlags.UsePickSet)] public void Focus() { var document = Active(); if (document == null) return; Guard(document, "QS3DFOCUS", () => { var count = ModelReviewService.HighlightSelection(document, true); if (count == 0) return; document.SendStringToExecute("QS3DZOOMSELECTED ", true, false, false); PaletteCoordinator.SetStatus("Focus + highlight " + count + " object."); }); }
        ...
        private static void Guard(Document document, string operation, Action action) { try { action(); } catch (Exception ex) { var message = operation + " lỗi: " + ex.Message; PaletteCoordinator.SetStatus(message); document.Editor.WriteMessage("\n" + message); } }
    }
}
