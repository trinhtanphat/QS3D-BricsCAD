...
namespace QS3D.BricsCAD.V25
{
    public sealed class DirectDrawOpeningCommands
    {
        ...
        public void DrawDoor() => DrawOpening(ElementCategory.Door, "Cửa Đi", 0d);

        [CommandMethod("QS3DDRAWOPENING", CommandFlags.Modal)]
        public void DrawWallOpening() => DrawOpening(ElementCategory.WallOpening, "Lỗ Mở Vách", 0d);
        ...
        private static void DrawOpening(ElementCategory category, string label, double defaultSillM)
        {
            ...
            Guard(document, "QS3DDRAW" + (category == ElementCategory.Door ? "DOOR" : "OPENING"), () =>
            {
                ...
                var heightDefault = hasDefaultsProject ? FamilyPositiveNumber(defaultsProject, category, "HeightM", 2.2d) : 2.2d;
                var heightM = PromptPositiveMeters(document.Editor, "Chiều cao " + label + " (m)", heightDefault);
                if (!heightM.HasValue) return;

                var bottomOffsetDefault = hasDefaultsProject ? FamilyNonNegativeNumber(defaultsProject, category, "BottomOffsetM", defaultSillM) : defaultSillM;
                var sillDefault = hasDefaultsProject ? FamilyNonNegativeNumber(defaultsProject, category, "SillHeightM", bottomOffsetDefault) : bottomOffsetDefault;
                var sillM = PromptNonNegativeMeters(document.Editor, "Cao độ bậu " + label + " so với đáy host (m)", sillDefault);
                if (!sillM.HasValue) return;

                var clearanceDefault = hasDefaultsProject ? FamilyNonNegativeNumber(defaultsProject, category, "BooleanClearanceM", 0.01d) : 0.01d;
        ...
        private static void Execute(Document document, ElementCategory category, string label, Point3d start, Point3d end, double widthM, double heightM, double sillM, double clearanceM)
        {
            ...
            try
            {
                ...
                createdElement = project.Elements.SingleOrDefault(x => x.Category == category && x.SourceHandles.Any(h => string.Equals(h, sourceHandle, StringComparison.OrdinalIgnoreCase)));
                ...
                if (createdElement == null) throw new InvalidOperationException(label + " vừa tạo không còn tồn tại sau Auto Host; operation được rollback.");
            ...
            catch (Exception operationError)
            {
                ...
                try { EraseSource(document, sourceId); } catch (Exception ex) { cleanupError = ex; }
        ...
        private static IReadOnlyList<Point3d>? AcquireTwoPoints(Document document, string label)
        {
            ...
            var secondOptions = new PromptPointOptions("\n" + label + " - chọn mép thứ hai: ") { UseBasePoint = true, BasePoint = first.Value };
            ...
            if (CadGeometryGuard.Hypot(dx, dy, label + "/plan width") <= 1e-9d) throw new InvalidOperationException(label + " có bề rộng plan bằng 0.");
            return points;
        }
        ...
        {
            var safeStart = new Point3d(CadGeometryGuard.Finite(start.X, "Direct Draw opening start X"), CadGeometryGuard.Finite(start.Y, "Direct Draw opening start Y"), CadGeometryGuard.Finite(start.Z, "Direct Draw opening start Z"));
            var safeEnd = new Point3d(CadGeometryGuard.Finite(end.X, "Direct Draw opening end X"), CadGeometryGuard.Finite(end.Y, "Direct Draw opening end Y"), CadGeometryGuard.Finite(end.Z, "Direct Draw opening end Z"));
        ...
        private static void EraseSource(Document document, ObjectId sourceId)
        {
            ...
            if (CadHandleService.GetLiveHandles(document, new[] { handle }).Count > 0) throw new InvalidOperationException("Rollback còn source CAD chưa xóa: " + handle + ".");
        }

        private static double? PromptPositiveMeters(Editor editor, string label, double defaultValue)
        {
            if (double.IsNaN(defaultValue) || double.IsInfinity(defaultValue) || defaultValue <= 0d) throw new InvalidOperationException(label + " default phải là số hữu hạn > 0.");
        ...
        {
            if (double.IsNaN(defaultValue) || double.IsInfinity(defaultValue) || defaultValue < 0d) throw new InvalidOperationException(label + " default phải là số hữu hạn >= 0.");
            var options = new PromptDoubleOptions("\n" + label + " <" + defaultValue.ToString("0.###", CultureInfo.InvariantCulture) + ">: ") { AllowNegative = false, AllowZero = true, AllowNone = true, DefaultValue = defaultValue, UseDefaultValue = true };
            var result = editor.GetDouble(options);
            if (result.Status == PromptStatus.Cancel) return null;
            if (result.Status != PromptStatus.OK && result.Status != PromptStatus.None) return null;
            var value = result.Status == PromptStatus.OK ? result.Value : defaultValue;
        ...
        private static void RequireModelSpace(Document document)
        {
            using (var transaction = document.Database.TransactionManager.StartOpenCloseTransaction())
            {
                ...
                if (!document.Database.CurrentSpaceId.Equals(modelSpaceId)) throw new InvalidOperationException("Direct Draw Cửa/Lỗ mở hiện chỉ hỗ trợ Model Space; chuyển sang Model rồi chạy lại.");
                transaction.Commit();
            }
        ...
        {
            var status = label + ": width=" + widthM.ToString("0.###", CultureInfo.InvariantCulture) + " m • host=" + hostId + " • regen=" + regenerated + ". Semantic + Auto Host hoàn tất; dùng QS3DCUTSELECTEDOPENINGS khi muốn khoét đúng Cửa/Lỗ đang chọn.";
            try
            {
