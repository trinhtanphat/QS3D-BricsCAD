    ...
    internal static class SemanticScheduleNativeTableBuilder
    {
        ...
        public static string Build(Document document, ProjectState project, SemanticScheduleDefinition definition, Point3d position)
        {
            ...
            if (semanticTable.Rows.Count == 0)
                throw new InvalidOperationException("Custom semantic schedule không có dòng để tạo native Table.");
            ...
            ICollection<ModelHealthIssue> issues)
        {
            ...
            if (definition == null)
            {
                issues.Add(Issue(
                    ...
                    "Native Table references removed custom schedule " + scheduleId + "; remove the orphan Table or restore the definition.",
                    scheduleId));
        ...
        private static ModelHealthIssue Issue(string code, HealthSeverity severity, string message, string scheduleId) =>
            new ModelHealthIssue(code, severity, message, scheduleId ?? string.Empty);
