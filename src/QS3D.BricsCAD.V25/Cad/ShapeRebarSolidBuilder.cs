    ...
    internal static class ShapeRebarSolidBuilder
    {
        ...
        public static ShapeRebarBuildResult BuildSelected(Document document, ProjectState project)
        {
            ...
            var elements = project.Elements
                .Where(x => x.SourceHandles.Any(selectedHandles.Contains) && x.Properties.TryGetValue("RebarNotation", out var n) && !string.IsNullOrWhiteSpace(n))
                .ToList();
            ...
            var rows = ProjectRebarScheduleBuilder.Build(project)
                .Where(x => elementIds.Contains(x.ElementId))
                    ...
                    foreach (var row in elementRows)
                    {
                        ...
                        for (var index = 0; index < row.Quantity; index++)
                        {
                            ...
                            var origin = new Point3d(
                                placement.Origin.X + placement.Distribution.X * offset,
                                placement.Origin.Y + placement.Distribution.Y * offset,
                                placement.Origin.Z + lift);
        ...
        private static Placement ResolvePlacement(Document document, ProjectState project, ProjectElement element, Entity source)
        {
            ...
            var bottom = CadGeometryGuard.ToDrawingUnits(document, bottomM, element.Id + "/bottom");
            if (source is Line line)
            {
                ...
                var spanM = element.Category == ElementCategory.StructuralWall || element.Category == ElementCategory.ArchitecturalWall
                    ? CadGeometryGuard.Number(element, family, "ThicknessM", .2d)
                    : CadGeometryGuard.Number(element, family, "WidthM", .3d);
            ...
            var width = extents.MaxPoint.X - extents.MinPoint.X; var depth = extents.MaxPoint.Y - extents.MinPoint.Y;
            var alongX = width >= depth;
            var origin = new Point3d(extents.MinPoint.X + cover, extents.MinPoint.Y + cover, extents.MinPoint.Z + bottom + cover);
            return new Placement { Origin = origin, Axis = alongX ? Vector3d.XAxis : Vector3d.YAxis, Distribution = alongX ? Vector3d.YAxis : Vector3d.XAxis, Span = Math.Max(1e-9, alongX ? depth : width), Cover = cover };
        }
        ...
        private static Solid3d BuildShape(Document document, Point3d origin, Vector3d axis, Vector3d distribution, RebarShapePath path, double radius, string label)
        {
            ...
            try
            {
                for (var i = 1; i < path.Points.Count; i++)
                {
                    ...
                    var before = i == 1 ? 0d : overlap; var after = i == path.Points.Count - 1 ? 0d : overlap;
                    var extendedStart = new Point3d(start.X - unit.X * before, start.Y - unit.Y * before, start.Z - unit.Z * before);
                    var part = Cylinder(document, extendedStart, vector, length + before + after, radius, label + "/s" + i);
        ...
        private static Solid3d Cylinder(Document document, Point3d start, Vector3d direction, double length, double radius, string label)
        {
            ...
            if (magnitude <= 1e-12 || double.IsNaN(magnitude) || double.IsInfinity(magnitude)) throw new InvalidOperationException("Rebar axis không hợp lệ: " + label);
            ...
            try
            {
                ...
                if (rotationAxis.Length > 1e-12) solid.TransformBy(Matrix3d.Rotation(angle, rotationAxis, Point3d.Origin));
        ...
        private static double DistributionOffset(int index, int count, double span, double cover, double radius)
        {
            ...
            var usable = Math.Max(0d, span - 2d * (cover + radius));
            return usable * index / (count - 1d);
        }

        private static Vector3d Normalize(Vector3d vector, string label)
        {
            var length = vector.Length;
            if (length <= 1e-12 || double.IsNaN(length) || double.IsInfinity(length)) throw new InvalidOperationException("Không xác định được " + label + ".");
        ...
        private static void ErasePrevious(Document document, Transaction transaction, ProjectElement element)
        {
            ...
            foreach (var text in raw.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                try
                {
                    var id = document.Database.GetObjectId(false, new Handle(value), 0); if (id.IsNull || !id.IsValid) continue;
                    var entity = transaction.GetObject(id, OpenMode.ForWrite, false) as Entity; if (entity != null && !entity.IsErased) entity.Erase();
                }
                catch { }
            }
        }
        ...
        private static Entity? OpenFirstSource(Document document, Transaction transaction, ProjectElement element)
        {
            foreach (var text in element.SourceHandles)
            {
                ...
                {
                    var id = document.Database.GetObjectId(false, new Handle(value), 0); if (id.IsNull || !id.IsValid) continue;
                    var entity = transaction.GetObject(id, OpenMode.ForRead, false) as Entity; if (entity != null && !entity.IsErased) return entity;
                }
                catch { }
            }
            return null;
