    ...
    internal static class GeneratedTieRebarOwnershipGuard
    {
        ...
        internal sealed class OwnershipIndex
        {
            ...
            public void EnsureTieOwned(string handle, ProjectElement element)
            {
                ...
                var normalized = CanonicalHandle(handle, "Generated tie handle");
                var expectedOwner = OwnerToken(element, HandlesKey);
                ...
                if (!string.Equals(actual, expectedOwner, StringComparison.OrdinalIgnoreCase))
                    throw new InvalidOperationException("Generated tie handle " + normalized + " belongs to " + actual + ", not " + expectedOwner + ". Refusing destructive erase.");

                EnsureCompleteLiveSet(element, expectedOwner);
            }

            private void EnsureCompleteLiveSet(ProjectElement element, string expectedOwner)
            {
                if (_validatedLiveSets.Contains(expectedOwner)) return;
                if (_document == null)
                    throw new InvalidOperationException("No active BricsCAD document is available to validate generated column ties before destructive replacement.");
                if (!ReferenceEquals(_document, Application.DocumentManager.MdiActiveDocument))
                    throw new InvalidOperationException("Active DWG changed before generated column tie replacement. Refusing destructive replacement before any tie is erased.");
                if (!element.Properties.TryGetValue(HandlesKey, out var raw) || string.IsNullOrWhiteSpace(raw))
                    throw new InvalidOperationException("Generated column tie owner metadata disappeared before destructive replacement: " + expectedOwner + ".");
                ...
                foreach (var token in SplitHandles(raw))
                {
                    ...
                    if (!seen.Add(canonical))
                        throw new InvalidOperationException("Generated column tie metadata contains duplicate canonical handle " + canonical + ". Refusing destructive replacement before any tie is erased.");
                    if (!_owners.TryGetValue(canonical, out var actual) || !string.Equals(actual, expectedOwner, StringComparison.OrdinalIgnoreCase))
                        throw new InvalidOperationException("Generated column tie handle " + canonical + " is not exclusively owned by " + expectedOwner + ". Refusing destructive replacement before any tie is erased.");
                    expectedHandles.Add(canonical);
                }

                if (expectedHandles.Count == 0)
                    throw new InvalidOperationException("Generated column tie metadata contains no valid handles for " + expectedOwner + ". Refusing destructive replacement before any tie is erased.");

                var ids = CadHandleService.Resolve(_document, expectedHandles);
                if (ids.Count != expectedHandles.Count)
                    throw new InvalidOperationException("Generated column tie live-handle set is incomplete for " + expectedOwner + ". Refusing destructive replacement before any tie is erased.");
                ...
                using (var transaction = _document.Database.TransactionManager.StartOpenCloseTransaction())
                {
                    foreach (var id in ids)
                    {
                        ...
                        if (!(entity is Solid3d solid) || solid.IsErased)
                            throw new InvalidOperationException("Generated column tie set contains a missing, erased, or non-Solid3d object for " + expectedOwner + ". Refusing destructive replacement before any tie is erased.");
                        GeneratedRebarNativeOwnershipService.RequireMatchingOwnership(
                            solid,
                            _project,
                            element,
                            HandlesKey,
                            "validate generated column tie replacement");
                    }
                    transaction.Commit();
                }
        ...
        private static void Add(ProjectElement element, string propertyKey, Dictionary<string, string> owners)
        {
            ...
            foreach (var handle in SplitHandles(raw))
            {
                ...
                if (owners.TryGetValue(canonical, out var existing) && !string.Equals(existing, token, StringComparison.OrdinalIgnoreCase))
                    throw new InvalidOperationException("Generated rebar ownership conflict: " + canonical + " is claimed by both " + existing + " and " + token + ". Refusing destructive erase.");
