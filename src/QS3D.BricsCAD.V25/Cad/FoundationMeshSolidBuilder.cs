...
namespace QS3D.BricsCAD.V25.Cad
{
    ...
    internal static class FoundationMeshSolidBuilder
    {
            ...
            using (var transaction = document.Database.TransactionManager.StartTransaction())
            {
                ...
                foreach (var element in elements)
                {
                    ...
                    var centerZ = CadGeometryGuard.Add(
                        polyline.Elevation,
                        CadGeometryGuard.ToDrawingUnits(document, bottomM + thicknessM / 2d, element.Id + "/foundation mesh center Z"),
        ...
        {
            if (!polyline.Closed || polyline.NumberOfVertices != 4) throw new InvalidOperationException(element.Id + ": QS3DFOUNDATIONREBAR3D yêu cầu closed 4-vertex rectangular POLYLINE.");
        ...
        private static Polyline? OpenSelectedFoundationSource(Document document, Transaction transaction, ProjectElement element, ISet<string> selectedHandles)
        {
            ...
            foreach (var text in element.SourceHandles.Where(selectedHandles.Contains))
            {
                ...
                if (entity == null || entity.IsErased) continue;
                if (!(entity is Polyline polyline)) throw new InvalidOperationException(element.Id + " cần source POLYLINE chữ nhật để dựng foundation mesh 3D.");
