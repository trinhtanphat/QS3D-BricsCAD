...
namespace QS3D.BricsCAD.V25.Cad
{
    internal static class ModelReviewService
    {
        ...
        public static int HighlightSelection(Document document, bool promptIfEmpty)
        {
            ...
            editor.SetImpliedSelection(selection.Value.GetObjectIds());
            ...
            lock (Gate) Highlighted[document] = handles;
        ...
        public static int ClearHighlight(Document document)
        {
            ...
            lock (Gate)
            {
                if (!Highlighted.TryGetValue(document, out handles)) return 0;
                Highlighted.Remove(document);
            }
            var count = 0;
            using (var transaction = document.Database.TransactionManager.StartOpenCloseTransaction())
            {
        ...
        public static void ForgetByName(string fileName)
        {
            ...
            lock (Gate)
            {
                foreach (var document in Highlighted.Keys.ToList())
                {
                    string name;
                    try { name = document.Name; }
                    catch { Highlighted.Remove(document); continue; }
                    if (string.Equals(name, fileName, StringComparison.OrdinalIgnoreCase)) Highlighted.Remove(document);
                }
            }
        }
    }
