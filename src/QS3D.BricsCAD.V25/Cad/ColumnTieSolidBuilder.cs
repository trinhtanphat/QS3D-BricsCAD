...
namespace QS3D.BricsCAD.V25.Cad
{
    internal static class ColumnTieSolidBuilder
    {
        ...
        public static int BuildSelected(Document document, ProjectState project)
        {
            ...
            using (var transaction = document.Database.TransactionManager.StartTransaction())
            {
                ...
                foreach (var id in ids)
                {
                    ...
                    if (totalTies + layout.ElevationsM.Count > MaxTiesPerBatch) throw new InvalidOperationException("Tie 3D batch vượt giới hạn " + MaxTiesPerBatch + " solid.");
                    ...
                    var radius = CadGeometryGuard.ToDrawingUnits(document, diameterMm / 2000d, element.Id + "/tie radius");
                    ...
                    foreach (var elevationM in layout.ElevationsM)
                    {
                        ...
                        var z = CadGeometryGuard.Add(polyline.Elevation, bottomOffset + elevation, element.Id + "/tie Z");
        ...
        private static RectGeometry RectangleGeometry(Polyline polyline, string label)
        {
            ...
            var e1x = p1.X - p0.X; var e1y = p1.Y - p0.Y; var e2x = p2.X - p1.X; var e2y = p2.Y - p1.Y;
            ...
            if (Math.Abs(ux * vx + uy * vy) > 1e-6d) throw new InvalidOperationException("Column footprint không vuông góc: " + label);
            var tolerance = Math.Max(width, depth) * 1e-6d + 1e-8d;
            if (Distance(p2.X, p2.Y, p0.X + e1x + e2x, p0.Y + e1y + e2y) > tolerance || Distance(p3.X, p3.Y, p0.X + e2x, p0.Y + e2y) > tolerance)
            ...
            {
                CenterX = (p0.X + p1.X + p2.X + p3.X) / 4d,
                CenterY = (p0.Y + p1.Y + p2.Y + p3.Y) / 4d,
        ...
        private static Solid3d BuildTie(Document document, RectGeometry geometry, ColumnTieLayout layout, double z, double radius, string label)
        {
            ...
            try
            {
                for (var i = 1; i < layout.ClosedPath.Count; i++)
                {
                    ...
                    var start = new Point3d(geometry.CenterX + geometry.Ux * ax + geometry.Vx * ay, geometry.CenterY + geometry.Uy * ax + geometry.Vy * ay, z);
                    var end = new Point3d(geometry.CenterX + geometry.Ux * bx + geometry.Vx * by, geometry.CenterY + geometry.Uy * bx + geometry.Vy * by, z);
                    var direction = new Vector3d(end.X - start.X, end.Y - start.Y, 0d);
                    var length = direction.Length;
                    if (double.IsNaN(length) || double.IsInfinity(length) || length <= 1e-9d) throw new InvalidOperationException("Tie path contains a degenerate segment: " + label);
                    var unit = new Vector3d(direction.X / length, direction.Y / length, 0d);
                    var overlap = Math.Min(radius * 0.75d, length * 0.1d);
                    var extendedStart = new Point3d(start.X - unit.X * overlap, start.Y - unit.Y * overlap, z);
                    var part = Cylinder(document, extendedStart, unit, length + overlap * 2d, radius, label + "/tie segment " + i);
        ...
        private static Solid3d Cylinder(Document document, Point3d start, Vector3d unit, double length, double radius, string label)
        {
            ...
            try
            {
                ...
                var axis = Vector3d.ZAxis.CrossProduct(unit);
