...
{
    internal static class GeneratedCurtainFrameNativeOwnershipService
    {
            ...
            string operation)
        {
            ...
            throw new InvalidOperationException(
                "Refusing to " + (string.IsNullOrWhiteSpace(operation) ? "modify generated curtain frame" : operation.Trim()) +
                " because its QS3D curtain-frame ownership marker does not match project " + project.ProjectId +
                ", element " + element.Id + ", owner slot " + GeneratedHandleOwnershipPolicy.CanonicalOwnerSlot(HandlesKey) + ". " +
                "Legacy/unmarked curtain-frame solids must not be destructively erased by handle alone.");
        }
