...
{
    public sealed class FoundationMeshHealthCommands
    {
        [CommandMethod("QS3DFOUNDATIONREBARHEALTH", CommandFlags.Modal)]
        public void ReviewFoundationMesh()
        {
            ...
            try
            {
                ...
                var message = "Foundation Rebar Health: " + summary.Errors + " lỗi • " + summary.Warnings + " cảnh báo • " + summary.Info + " thông tin";
        ...
        {
            if (!element.Properties.TryGetValue(FoundationMeshSolidBuilder.HandlesKey, out var raw) || string.IsNullOrWhiteSpace(raw)) return Array.Empty<string>();
