...
namespace QS3D.BricsCAD.V25
{
    public sealed class ReferenceSearchCommands
    {
        ...
        public void ShowReferenceSearch()
        {
            ...
            try
            {
                ...
                if (published != null)
                {
                    ...
                    {
                        try { PaletteCoordinator.SetStatus("Reference Search hiện có đã được kích hoạt cho đúng bản vẽ."); } catch { }
                    }
                    return;
                }
                ...
                if (IsActiveDocumentGeneration(document, nativeDatabaseIdentity))
                {
                    try { PaletteCoordinator.SetStatus("Tham khảo thi công: ảnh • web • video • mua sắm • video ngắn • tin tức."); } catch { }
                }
            }
            catch
            {
        ...
        {
            if (!IsActiveDocumentGeneration(document, nativeDatabaseIdentity)) return;
            const string blockedStatus = "Reference Search hiện tại chưa thể đóng an toàn; không mở bản sao thứ hai.";
        ...
        {
            if (!IsActiveDocumentGeneration(document, nativeDatabaseIdentity)) return;
            const string message = "QS3DREFSEARCH không thể mở Reference Search an toàn; trạng thái hiện tại được giữ nguyên.";
        ...
        {
            internal PublishedWindow(ReferenceSearchWindow window, Document document, IntPtr nativeDatabaseIdentity)
            {
            ...
            internal ReferenceSearchWindow Window { get; }
