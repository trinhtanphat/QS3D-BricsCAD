...
namespace QS3D.BricsCAD.V25
{
    ...
    internal static class McpBackgroundHostRuntime
    {
        ...
        internal static IEnumerable<string> ToolDescriptors()
        {
            return new[]
            {
                ...
                Tool("bricscad_interaction_policy_set",
                    "Set the remote interaction policy fail-closed. background_only is supported remotely; Foreground Control can only be enabled by the local Agent Center checkbox.",
        ...
        internal static void EnableForegroundFromLocalUser()
        {
            ...
            // Keep the legacy consent reason as a second local-only validation so existing
            // source guards stay compatible without reopening remote foreground activation.
