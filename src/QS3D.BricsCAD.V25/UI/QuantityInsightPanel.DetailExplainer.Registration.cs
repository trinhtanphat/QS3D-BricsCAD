...
namespace QS3D.BricsCAD.V25.UI
{
 public partial class QuantityInsightPanel
 {
  ...
  private StackPanel? _quantityDetailBody; private readonly Dictionary<string,TextBlock> _quantityDetailValues=new Dictionary<string,TextBlock>(StringComparer.Ordinal);
  private IReadOnlyList<QuantityInsightDetailOption> _quantityDetailOptions=Array.Empty<QuantityInsightDetailOption>();
  private static bool RegisterQuantityDetailExplainer(){EventManager.RegisterClassHandler(typeof(QuantityInsightPanel),FrameworkElement.LoadedEvent,new RoutedEventHandler(OnQuantityDetailExplainerLoaded),true);return true;}
  private static void OnQuantityDetailExplainerLoaded(object sender,RoutedEventArgs e){if(sender is QuantityInsightPanel panel)panel.InstallQuantityDetailExplainer();}
