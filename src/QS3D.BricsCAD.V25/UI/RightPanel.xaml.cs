    ...
    public partial class RightPanel : UserControl
    {
        ...
        public void Refresh()
        {
            ...
            try
            {
                ...
                _viewModel.Drawings.Add(new DrawingItemViewModel { Name = System.IO.Path.GetFileName(doc.Name), Path = doc.Name, Scale = "MODEL" });
                foreach (var item in DrawingCatalogReader.ReadReferences(doc)) _viewModel.Drawings.Add(new DrawingItemViewModel { Name = item.Name, Path = item.Path, Scale = "XREF" });
                RefreshLayers();
        ...
        private void RefreshLayers()
        {
            ...
            foreach (var item in DrawingCatalogReader.ReadLayers(doc).Where(x => search.Length == 0 || x.Name.IndexOf(search, StringComparison.CurrentCultureIgnoreCase) >= 0))
                _viewModel.Layers.Add(new LayerItemViewModel { Name = item.Name, IsVisible = item.IsVisible, ColorIndex = item.ColorIndex });
        }
        ...
        private void SetLayerFromCheckBox(object sender, bool visible)
        {
            ...
            var doc = Application.DocumentManager.MdiActiveDocument; if (doc == null) return;
            try { LayerVisibilityService.SetVisible(doc, new[] { item.Name }, visible); _viewModel.Status = (visible ? "Hiện " : "Ẩn ") + item.Name; }
            catch (Exception ex) { _viewModel.Status = ex.Message; }
        }
        ...
        private void SetSelectedLayers(bool visible)
        {
            ...
            try { var count = LayerVisibilityService.SetVisible(doc, names, visible); _viewModel.Status = (visible ? "Đã hiện " : "Đã ẩn ") + count + " layer"; RefreshLayers(); }
            catch (Exception ex) { _viewModel.Status = ex.Message; }
        }

        private static void OnAttachXrefClick(object sender, RoutedEventArgs e) => Send("_XATTACH");
        private static void OnReloadXrefClick(object sender, RoutedEventArgs e) => Send("_XREF");
        private static void OnOpenXrefManagerClick(object sender, RoutedEventArgs e) => Send("_XREF");
