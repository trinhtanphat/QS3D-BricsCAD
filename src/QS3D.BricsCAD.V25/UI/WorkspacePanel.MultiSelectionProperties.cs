...
namespace QS3D.BricsCAD.V25.UI
{
    public partial class WorkspacePanel
    {
            ...
            out string error)
        {
            ...
            {
                if (!requestedHandles.Add(handle))
                {
            ...
            var matchesByHandle = new Dictionary<string, List<ProjectElement>>(StringComparer.OrdinalIgnoreCase);
            foreach (var handle in requestedHandles) matchesByHandle.Add(handle, new List<ProjectElement>());
        ...
        {
            var ids = elements.Select(element => element.Id).ToArray();
            var inspection = SemanticSelectionInspector.Inspect(project, ids);
            if (inspection.Count < 2) throw new InvalidOperationException("Multi-selection presentation requires at least two semantic elements.");

            var scopeAnchor = elements.FirstOrDefault(element => !string.IsNullOrWhiteSpace(element.FamilyId) && project.FindFamily(element.FamilyId) != null);
            ...
            _viewModel.SelectedFamilyName = MultiSelectionHeader(project, inspection);
            LoadMultiSelectionRows(project, inspection);
            _viewModel.Status = inspection.Count + " cấu kiện semantic • Property Inspector đang hiển thị common/mixed của toàn bộ selection.";
        }
        ...
        private void LoadMultiSelectionRows(ProjectState project, SemanticSelectionInspection inspection)
        {
            ...
            AddMultiReferenceRow(project, inspection.Family, "Family");
            AddMultiReferenceRow(project, inspection.Floor, "Tầng");
            AddMultiReferenceRow(project, inspection.Zone, "Zone");

            foreach (var summary in inspection.Properties.OrderBy(item => MultiGroupFor(item.Name)).ThenBy(item => MultiDisplayNameFor(item.Name)))
            {
                ...
                var row = new PropertyRowViewModel
                {
                    ...
                    Name = MultiDisplayNameFor(key) + (summary.IsMixed ? " • Nhiều giá trị" : " • Chung"),
                    Unit = unit,
                    IsReadOnly = IsMultiSelectionReadOnlyKey(key),
                    CanReset = false,
                    EditorKind = MultiEditorKindFor(key, storageValue),
                    Choices = MultiChoicesFor(project, key, storageValue)
                };
                if (summary.IsMixed && row.EditorKind == PropertyRowViewModel.BooleanEditor)
                    row.EditorKind = PropertyRowViewModel.TextEditor;
            ...
            foreach (var quantity in inspection.Quantities.OrderBy(item => item.Name, StringComparer.CurrentCultureIgnoreCase))
            {
                AddMultiReadOnlyRow(
                    "KHỐI LƯỢNG / ĐO ĐẠC",
                    quantity.Name + (quantity.IsMixed ? " • Nhiều giá trị" : " • Chung"),
            ...
            string requestedValue)
        {
            ...
            try
            {
                ...
                {
                    SetStatus("Cập nhật multi-selection bị từ chối: Property Inspector đang giữ project stale sau reload/thay thế. Hãy Refresh Workspace.");
                    return CurrentMultiDisplayValue(presentedProject, presentedIds, key);
                }
                ...
                var currentIds = currentElements.Select(element => element.Id).OrderBy(id => id, StringComparer.OrdinalIgnoreCase).ThenBy(id => id, StringComparer.Ordinal).ToArray();
                var expectedIds = presentedIds.OrderBy(id => id, StringComparer.OrdinalIgnoreCase).ThenBy(id => id, StringComparer.Ordinal).ToArray();
                if (!currentIds.SequenceEqual(expectedIds, StringComparer.OrdinalIgnoreCase))
                {
                    SetStatus("Cập nhật multi-selection bị từ chối: CAD selection đã thay đổi sau khi Property Inspector được dựng.");
                    return CurrentMultiDisplayValue(currentProject, presentedIds, key);
                }
                ...
                if (!valid) return beforeSummary == null || beforeSummary.IsMixed ? string.Empty : MultiToDisplayValue(key, previousStorage);
                ...
                SetStatus(
                    result.ChangedCount == 0
                        ? "Multi-selection: " + MultiDisplayNameFor(key) + " không thay đổi; stale/project guards vẫn đã được revalidate."
        ...
        private void AddMultiReferenceRow(ProjectState project, SemanticSelectionTextValue summary, string label)
        {
            var value = summary.IsMixed ? "Nhiều giá trị" : ResolveReferenceDisplay(project, summary.Name, summary.Value);
            AddMultiReadOnlyRow("SELECTION", label, value);
        }
        ...
        private static string ResolveReferenceDisplay(ProjectState project, string referenceName, string? value)
        {
            var id = (value ?? string.Empty).Trim();
            if (id.Length == 0) return "—";
            if (string.Equals(referenceName, "FamilyId", StringComparison.OrdinalIgnoreCase))
            {
                var family = project.FindFamily(id);
                return family == null ? id : family.Name;
            }
            if (string.Equals(referenceName, "FloorId", StringComparison.OrdinalIgnoreCase))
            {
                var floor = project.FindFloor(id);
                return floor == null ? id : floor.Name;
            }
            if (string.Equals(referenceName, "ZoneId", StringComparison.OrdinalIgnoreCase))
            {
                var zone = project.FindZone(id);
                return zone == null ? id : zone.Name;
            }
            return id;
        }

        private static string MultiSelectionHeader(ProjectState project, SemanticSelectionInspection inspection)
        {
            ...
            var family = inspection.Family.IsMixed
                ? "nhiều Family"
                : ResolveReferenceDisplay(project, inspection.Family.Name, inspection.Family.Value);
        ...
        private static bool IsMultiSelectionReadOnlyKey(string key)
        {
            ...
            if (normalized.EndsWith("Id", StringComparison.OrdinalIgnoreCase) ||
                ...
                normalized.EndsWith("RefId", StringComparison.OrdinalIgnoreCase) ||
                normalized.EndsWith("RefIds", StringComparison.OrdinalIgnoreCase)) return true;
            if (normalized.IndexOf("Handle", StringComparison.OrdinalIgnoreCase) >= 0 ||
        ...
        private static IReadOnlyList<string> MultiChoicesFor(ProjectState project, string key, string current)
        {
            if (!MultiIsChoiceProperty(key)) return Array.Empty<string>();
            return project.Families
                .SelectMany(family => family.Properties.Where(property => string.Equals(property.Key, key, StringComparison.OrdinalIgnoreCase)).Select(property => (property.Value ?? string.Empty).Trim()))
        ...
        private static string MultiEditorKindFor(string key, string current)
        {
            if (MultiIsBooleanProperty(key, current)) return PropertyRowViewModel.BooleanEditor;
            if (MultiIsChoiceProperty(key)) return PropertyRowViewModel.ChoiceEditor;
        ...
        private static bool MultiIsChoiceProperty(string key)
        {
            if (string.IsNullOrWhiteSpace(key)) return false;
        ...
        private static string MultiGroupFor(string key)
        {
            ...
            if (key.IndexOf("Material", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Classification", StringComparison.OrdinalIgnoreCase) >= 0 || key.EndsWith("Code", StringComparison.OrdinalIgnoreCase)) return "VẬT LIỆU / PHÂN LOẠI";
            if (key.IndexOf("Offset", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Elevation", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Level", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Sill", StringComparison.OrdinalIgnoreCase) >= 0) return "VỊ TRÍ / CAO ĐỘ";
            if (key.IndexOf("Display", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Color", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Layer", StringComparison.OrdinalIgnoreCase) >= 0) return "HIỂN THỊ";
            if (key.IndexOf("Length", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Width", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Height", StringComparison.OrdinalIgnoreCase) >= 0 ||
                key.IndexOf("Depth", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Thickness", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Area", StringComparison.OrdinalIgnoreCase) >= 0 ||
                key.IndexOf("Volume", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Perimeter", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Profile", StringComparison.OrdinalIgnoreCase) >= 0 ||
                key.IndexOf("Axis", StringComparison.OrdinalIgnoreCase) >= 0 || key.IndexOf("Radius", StringComparison.OrdinalIgnoreCase) >= 0) return "HÌNH HỌC";
            return "THUỘC TÍNH";
        }
