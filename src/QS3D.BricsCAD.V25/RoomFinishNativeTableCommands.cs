...
{
    public sealed class RoomFinishNativeTableCommands
    {
        private const double UcsAxisTolerance = 1e-9d;

        [CommandMethod("QS3DFINISHTABLE", CommandFlags.Modal)]
        public void Build()
        {
            ...
            try
            {
                ...
                {
                    Report(document, "Room Finish Table: BLOCKED • chưa có QS3D project state/sidecar; đặt Table không tạo project mới.");
                    return;
                }
                ...
                if (!string.Equals(project.ProjectId, expectedProjectId, StringComparison.OrdinalIgnoreCase) ||
                    ...
                    throw new InvalidOperationException("Room Finish Table: QS3D project/state đã thay đổi trong lúc chọn điểm đặt. Hãy chạy lại lệnh.");
            ...
            catch (Exception ex) { Report(document, "QS3DFINISHTABLE lỗi: " + ex.Message); }
        }

        [CommandMethod("QS3DFINISHTABLEREFRESH", CommandFlags.Modal)]
        public void Refresh()
        {
            var document = Application.DocumentManager.MdiActiveDocument;
            if (document == null) return;
            ...
            catch (Exception ex) { Report(document, "QS3DFINISHTABLEREFRESH lỗi: " + ex.Message); }
        }

        [CommandMethod("QS3DFINISHTABLEREMOVE", CommandFlags.Modal)]
        public void Remove()
        {
            var document = Application.DocumentManager.MdiActiveDocument;
            if (document == null) return;
            ...
            catch (Exception ex) { Report(document, "QS3DFINISHTABLEREMOVE lỗi: " + ex.Message); }
        }

        [CommandMethod("QS3DFINISHTABLEHEALTH", CommandFlags.Modal)]
        public void Health()
        {
            var document = Application.DocumentManager.MdiActiveDocument;
            if (document == null) return;
            try
            {
                ...
                {
                    Report(document, "Room Finish Table health: BLOCKED • chưa có QS3D project state/sidecar; health check không tạo project mới.");
                    return;
                }

                var issues = RoomFinishNativeTableBuilder.Inspect(document, project);
                if (issues.Count == 0)
                {
                    Report(document, "Room Finish Table health: không phát hiện ownership/stale/live CAD drift issue.");
                    return;
                }
        ...
        private static void RequireModelSpace(Document document)
        {
            if (!document.Database.TileMode)
                throw new InvalidOperationException("Room Finish Table P0 chỉ hỗ trợ ModelSpace.");
        }
        ...
        private static void RequireSupportedUcs(Document document)
        {
            ...
            if (Math.Abs(x) > UcsAxisTolerance || Math.Abs(y) > UcsAxisTolerance || Math.Abs(z - 1d) > UcsAxisTolerance)
                throw new InvalidOperationException("Room Finish Table P0 chỉ hỗ trợ UCS có mặt phẳng XY song song WCS XY khi chọn điểm đặt mới.");
        }
