...
=======
        public static int BuildSelected(Document document, ProjectState project, ElementCategory category)
        {
            ...
            if (!Supports(category)) return 0; var selection = document.Editor.SelectImplied(); if (selection.Status != PromptStatus.OK || selection.Value == null) return 0; var ids = selection.Value.GetObjectIds(); if (ids.Length == 0) return 0; var created = 0;
            using (document.LockDocument()) using (var transaction = document.Database.TransactionManager.StartTransaction())
            {
                ...
                foreach (var id in ids)
                {
                    var entity = transaction.GetObject(id, OpenMode.ForRead, false) as Entity; if (entity == null) continue; var handle = entity.Handle.ToString(); var element = project.Elements.FirstOrDefault(x => x.Category == category && x.SourceHandles.Any(h => string.Equals(h, handle, StringComparison.OrdinalIgnoreCase))); if (element == null) continue;
                    Solid3d? solid = null;
                    try
                    {
                        ...
                        if (solid == null) continue; solid.Layer = entity.Layer; modelSpace.AppendEntity(solid); transaction.AddNewlyCreatedDBObject(solid, true); element.Properties["GeneratedSolidHandle"] = solid.Handle.ToString(); element.Properties["GeneratedSolidCategory"] = category.ToString(); created++;
                    }
...
<<<<<<< origin/main
            ...
            return solid;
=======
            var widthM = category == ElementCategory.Beam ? Number(element, family, "WidthM", .2d) : Number(element, family, "ThicknessM", .2d); var heightM = Number(element, family, "HeightM", category == ElementCategory.Beam ? .4d : 3.6d); var bottomM = Number(element, family, "BottomOffsetM", 0d); var dx = line.EndPoint.X - line.StartPoint.X; var dy = line.EndPoint.Y - line.StartPoint.Y; var length = Math.Sqrt(dx * dx + dy * dy); if (length <= 1e-6 || widthM <= 0d || heightM <= 0d) throw new InvalidOperationException("Kích thước structural LINE không hợp lệ.");
            var width = widthM * 1000d; var height = heightM * 1000d; var angle = Math.Atan2(dy, dx); var mid = new Point3d((line.StartPoint.X + line.EndPoint.X) / 2d, (line.StartPoint.Y + line.EndPoint.Y) / 2d, line.StartPoint.Z + bottomM * 1000d + height / 2d); var solid = new Solid3d(); solid.SetDatabaseDefaults(document.Database); solid.CreateBox(length, width, height); solid.TransformBy(Matrix3d.Rotation(angle, Vector3d.ZAxis, Point3d.Origin)); solid.TransformBy(Matrix3d.Displacement(new Vector3d(mid.X, mid.Y, mid.Z))); return solid;
>>>>>>> origin/agent/full-domain-20260810
        }
...
=======
            var heightM = category == ElementCategory.Slab ? Number(element, family, "ThicknessM", .12d) : Number(element, family, "HeightM", category == ElementCategory.Column ? 3.6d : .5d); var bottomM = Number(element, family, "BottomOffsetM", 0d); if (heightM <= 0d) throw new InvalidOperationException("Chiều cao extrusion phải lớn hơn 0."); var height = heightM * 1000d; var solid = new Solid3d(); solid.SetDatabaseDefaults(document.Database); solid.CreateExtrudedSolid(polyline, new Vector3d(0d, 0d, height), new SweepOptions()); if (Math.Abs(bottomM) > 1e-12) solid.TransformBy(Matrix3d.Displacement(new Vector3d(0d, 0d, bottomM * 1000d))); return solid;
        }
        ...
        private static double Number(ProjectElement element, ProjectFamily? family, string name, double fallback) { if (element.Properties.TryGetValue(name, out var value) && double.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out var direct)) return direct; if (family != null && family.Properties.TryGetValue(name, out value) && double.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out var inherited)) return inherited; return fallback; }
>>>>>>> origin/agent/full-domain-20260810
    }
}
