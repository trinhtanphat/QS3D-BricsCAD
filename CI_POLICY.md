...
> **Owner override — 2026-08-14:** every new AI agent/chat-session lane must register through a visible GitHub issue or claim PR before substantive implementation, keep claim/status/implementation writes off `main`, publish a concrete plan, implement on a dedicated agent/recovery branch, and run/observe applicable task-scoped non-destructive CI until the lane's required checks are green. Ordinary prompts such as `fix bug`, `update code`, `commit push git`, `continue all`, `implement all`, `run CI`, `fix CI` or `loop until success` do **not** grant `main` authority. Only explicit owner integration authorization may change `main`. CI authority is separate from integration authority and release/publish authority. See `docs/AI-SESSION-WORKFLOW.md` and `docs/AGENT-WORK-REGISTRATION.md`. This override supersedes older claim-only/status-only direct-`main` wording while preserving all stronger release, exact-SHA, LOCAL_ONLY and integration rules below.
...
1. publish a visible GitHub issue or claim PR that reserves the lane without modifying `main`;
...
8. merge the integration branch into `main` **once**, and only with explicit owner integration authority;
...
Claim/status documentation for new lanes stays on issues/claim PRs/agent branches rather than being pushed directly to `main`. Release-preparation commits pushed by `github-actions[bot]` are ignored so the release workflow cannot recursively trigger itself.

This section supersedes older repository wording that told implementation agents or claim publishers to push their own batch directly to `main`. New claim publication uses issues/claim PRs; implementation landing uses agent branches plus a single authorized integration branch.
...
- Claim publication through a visible issue/claim PR does not authorize arbitrary Actions operations or any `main` write.
- For the registered lane, applicable non-destructive branch/PR CI may be run/observed/retried as part of the mandatory completion loop. A red relevant run must be diagnosed against its exact SHA, fixed on the agent/recovery branch, pushed, and followed by a fresh relevant run until required/applicable checks are green.
- A normal `continue all`, `fix bug`, `update code`, `commit`, review or handoff assignment does not authorize manually dispatching/re-running/cancelling **unrelated** workflows, publishing releases, or changing `main`.
- The automatic post-integration dispatcher requires no per-run agent approval after a valid authorized integration landing; it is standing owner policy.
- Manual release operations outside that automatic path still require explicit owner release authorization and remain agent/scope-specific.
...
8. merge the integration branch to `main` once, only with explicit owner integration authority;
...
## Mandatory session completion verdict

Every AI agent/chat session must end with the explicit verdict defined in `docs/AI-SESSION-WORKFLOW.md`:

- `PROMPT/LANE STATUS: 100% COMPLETE` or `NOT 100% COMPLETE`;
- `SESSION CAN BE CLOSED/DELETED: YES` or `NO`;
- `MERGED TO MAIN: YES` or `NO`;
- exact issue/PR/branch references, implementation SHA(s), validation/CI results and remaining blockers.

If the prompt did not grant integration authority, a fully implemented and green branch/PR lane may be 100% complete while `MERGED TO MAIN: NO`. If required/applicable lane CI is red and actionable work remains, the session must continue the diagnose -> fix -> push -> fresh-run loop instead of stopping at a checkpoint. Required LOCAL_ONLY evidence that cannot be produced remotely must be handed off precisely and must never be fabricated as PASS.
...
Related documentation: `AGENTS.md`, `docs/AI-SESSION-WORKFLOW.md`, `docs/AGENT-WORK-REGISTRATION.md`, `README.md`, `docs/CI.md`, `docs/CI-READINESS.md`, `docs/MANUAL-BUILD-RELEASE.md`, `docs/MANUAL-BUILD-RELEASE-V26.md`, `docs/LOCAL-V25-QUALIFICATION.md`, `docs/LOCAL-V26-QUALIFICATION.md`.
